...
//		$log->fatal($res->sql);
	...
	private function update($data,$id){
		...
		if ($this->skip_entity)
			return; //se c'è qualche motivo per ignorarlo salto!
...
//			array_walk($arr,'sanitize_array_sql');
	...
	private function create($data){
		...
		if ($this->module == 'Users') {
			fwrite($this->sql_file_update[$table_prefix.'_users'],"insert into ".$table_prefix."_user2role values ($id,'".$this->cache[$this->module]['roleid']."');\n");
		}
	}
	...
	private function get_checkbox_value($field,$data,$table,$field,$mode){
		$value = $data[$this->mapping_inverse[$this->fields_name[$table][$field]]];
		return $value; //il valore è già formattato con 0=>no 1=>si
	}
	
	private function get_assigned_user($field,$data,$table,$field,$mode){
		$value = $data[$this->mapping_inverse[$this->fields_name[$table][$field]]];
		if ($this->users[$value] != ''){
	...
	private function get_date_value($field,$data,$table,$field,$mode){
		$value = $data[$this->mapping_inverse[$this->fields_name[$table][$field]]];
		if (trim($value) == ''){
			return 'NULL';
		}
	...
	private function get_date_short_value($field,$data,$table,$field,$mode){
		$value = $data[$this->mapping_inverse[$this->fields_name[$table][$field]]];
		if ($value != '') {
	...
	private function get_valuta($field,$data,$table,$field,$mode){
		global $adb;
	...
	private function get_ui10_value($field,$data,$table,$field,$mode){
		global $adb;
		$value = $data[$this->mapping_inverse[$this->fields_name[$table][$field]]];
		if ($value == ''){
			return $value;
		...
		switch($this->module){
			case 'Users':
				...
				foreach ($this->entity_created as $id){
					...
					$query = "UPDATE ".$table_prefix."_users SET user_password=?, crypt_type=? where id=?";
					$adb->pquery($query,array($encrypted_password, $crypt_type, $id));
...
//				if (count($entities)>0){
//					$sql = "update vtiger_invoice set taxtype = ?,subject = numero_fattura_erp,taxtype=? where invoiceid in (".generateQuestionMarks($entities).")";
...
//				$entities = array_merge($this->entity_created,$this->entity_updated);
//				if (count($entities)>0){
//					$sql = "update vtiger_salesorder set taxtype = ?,subject = numero_ordine_erp,taxtype=? where salesorderid in (".generateQuestionMarks($entities).")";
//					$params = Array('percent','group',$entities);
//					$adb->pquery($sql,$params);
//				}
				break;	
	...
	// crmv@91571
	private function launch_workflows(){
	...
	private function preprocess(&$row){
		...
		switch ($this->module){
			case 'Products':
				...
				$this->fields_runtime['vtiger_crmentity']['smownerid'] = $smownerid;
				$this->fields_runtime['vtiger_crmentity']['modifiedby'] = $smownerid;
				$this->fields_runtime['vtiger_crmentity']['smcreatorid'] = $smownerid;				
				$this->fields_runtime['vtiger_products']['productcode'] = $row['company'].$row['item'].$row['colour'].$row['size'];
				break;
			case 'SalesOrder':
				$smownerid = $this->get_smownerid($row['company']);
				$this->fields_runtime['vtiger_crmentity']['smownerid'] = $smownerid;
				$this->fields_runtime['vtiger_crmentity']['modifiedby'] = $smownerid;
				$this->fields_runtime['vtiger_crmentity']['smcreatorid'] = $smownerid;				
				$this->fields_runtime['vtiger_salesorder']['subtotal'] = $row['val_net'];
				break;
			case 'Invoice':
				$smownerid = $this->get_smownerid($row['company']);
				$this->fields_runtime['vtiger_crmentity']['smownerid'] = $smownerid;
				$this->fields_runtime['vtiger_crmentity']['modifiedby'] = $smownerid;
				$this->fields_runtime['vtiger_crmentity']['smcreatorid'] = $smownerid;				
				$this->fields_runtime['vtiger_invoice']['subtotal'] = $row['val_net'];
				$this->fields_runtime['vtiger_invoice']['total'] = $row['val_fat'];
				break;
			case 'Accounts':
				$smownerid = $this->get_smownerid($row['company']);
				$this->fields_runtime['vtiger_crmentity']['smownerid'] = $smownerid;
	...
	*/
	...
	function get_assigned_user_global(){
		...
		switch ($this->module){
			default:{
//				$sql =" select id,codice_agente_erp from vtiger_users where codice_agente_erp <> ''";
//				$res = $adb->query($sql);
//				if ($res){
//					while ($row=$adb->fetchByAssoc($res,-1,false)){

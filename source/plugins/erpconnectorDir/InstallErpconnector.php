<?php
$erpconnector_dir = '';	//ex. $erpconnector_dir = 'plugins/erpconnectorDir/';
if ($erpconnector_dir == '') {
...
@require_once($erpconnector_dir.'config.php');
global $adb,$table_prefix;

if(!Vtiger_Utils::CheckTable($log_script_state)) {
...
if(!Vtiger_Utils::CheckTable($log_script_content)) {
	$schema = '<?xml version="1.0"?>
				<schema version="0.3">
...
foreach($erpconnector_modules as $erpconnector_module) {
	...
	$moduleInstance = Vtiger_Module::getInstance($erpconnector_module);
	...
	if ($result && $adb->num_rows($result) > 0) {
		...
		if ($result1 && $adb->num_rows($result1) > 0) {
			...
			if ($blocklabel != '') {
				...
				SDK::setLanguageEntries($erpconnector_module, 'External Code', array('it_it'=>'Codice Esterno','en_us'=>'External Code','pt_br'=>'Código Externo'));
			}
		}
	}
	if (in_array($erpconnector_module,$erpconnector_modules_row)) {
		...
		if ($table_row != '' && !empty($fields_row)) {
			if(!Vtiger_Utils::CheckTable($table_row)) {
				$schema = '<?xml version="1.0"?>
							<schema version="0.3">

...
class Vtiger_Filter {
	...
	function addStandardRule($fieldInstance, $duration, $date_start = null, $date_end = null, $only_month= false) {
	    ...
	    if (!in_array($duration, $valid_durations)) {
	    	Vtiger_Utils::log("Error adding standard condition, unknown duration : $duration");
	    	return $this;
	    }

	    if ($duration == 'custom' && (empty($date_start) || empty($date_end))) {
	    	Vtiger_Utils::log("Error adding standard condition, dates cannot be null in custom mode");
	    	return $this;
	    }
	    ...
	    $params = array($this->id, $cvcolvalue, $duration, $date_start, $date_end, $only_month);
		...
		if ($metaLogs) $metaLogs->log($metaLogs::OPERATION_EDITFILTER, $this->id); // crmv@49398
	    Vtiger_Utils::Log("Adding Standard Condition $duration on {$fieldInstance->name} of {$this->name} filter ... DONE");

...
class ADODB_DataDict {
	...
	function NameQuote($name = NULL,$allowBrackets=false)
	{
		...
		//crmv@26666
		if ($this->connection->databaseType == 'oci8') {
	...
	*/
	function CreateTableSQL($tabname, $flds, $tableoptions=array(),$temporary = false)
	{
		...
		$tabname = $this->TableName ($tabname);
		$sql = $this->_TableSQL($tabname,$lines,$pkey,$taboptions,$temporary);
	...
	global $ADODB_FETCH_MODE;
		...
		foreach ( $lines as $id => $v ) {
			if ( isset($cols[$id]) && is_object($cols[$id]) ) {
				...
				$sql[] = $alter . $this->alterCol.' '.$v;	//crmv@21249
			} else {

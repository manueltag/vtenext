...
class ADODB_oci8 extends ADOConnection {
	...
	var $_initnumberformat = true; // init date to . as decimal separator
	...
	var $metaSequencesSQL = "select table_name,table_type from cat where table_type in ('SEQUENCE') and table_name not like 'BIN\$%'"; // bin$ tables are recycle bin tables
	var $existTable = "select table_name from cat where table_type in ('TABLE','VIEW') and table_name = '%s'";
  ...
  where owner='%s' and table_name='%s' order by column_id"; // when there is a schema
	...
	//crmv@13669
	var $NLS_COMP = 'LINGUISTIC'; //equal operator mode
	var $NLS_SORT = 'BINARY_CI'; //like operator mode
	//crmv@13669 end
...
*/
	function _connect($argHostname, $argUsername, $argPassword, $argDatabasename,$mode=0)
	{
		...
		//crmv@13669 crmv@47905bis
		$oci8_sessions = array();
		if ($this->_initdate) {
			$oci8_sessions['NLS_DATE_FORMAT'] = $this->NLS_DATE_FORMAT;
			$this->Execute("ALTER SESSION SET NLS_NUMERIC_CHARACTERS = '{$this->decimal_separator}'");
		}
		if ($this->NLS_COMP){
			$oci8_sessions['NLS_COMP'] = $this->NLS_COMP;
		}
		if ($this->NLS_SORT){
			$oci8_sessions['NLS_SORT'] = $this->NLS_SORT;
		}
		if (!empty($oci8_sessions)) {
			$query = 'ALTER SESSION SET';
			foreach($oci8_sessions as $p => $v) {
				$query .= " $p = '$v'";
			}
			$this->Execute($query);
		}
		//crmv@13669e crmv@47905bise
		// looks like: 
	...
	function DBTimeStamp($ts,$isfld=false)
	{
		...
		if ($isfld) return 'TO_DATE(substr('.$ts.",1,19),'".$this->NLS_DATE_FORMAT."')";
		if (is_string($ts)) $ts = ADORecordSet::UnixTimeStamp($ts);
		
		if (is_object($ts)) $tss = $ts->format("'Y-m-d H:i:s'");
		else $tss = adodb_date("'Y-m-d H:i:s'",$ts);
		return $tss;
		return 'TO_DATE('.$tss.",'".$this->NLS_DATE_FORMAT."')";
	}
...
"select constraint_name,r_owner,r_constraint_name 
	...
	function quote_field_name($fname){
		if (in_array($key,Array('comment','session')))
			$fname = '"'.$fname.'"';
		return $fname;
	}

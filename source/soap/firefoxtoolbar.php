...
function LogintoVtigerCRM($user_name,$password,$version)
{
	...
	$objuser = CRMEntity::getInstance('Users');
...
function create_account($username,$sessionid,$accountname,$email,$phone,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country)
{
	...
	if(isPermitted("Accounts","EditView") == "yes")
	{
		$query = "SELECT accountname FROM ".$table_prefix."_account,".$table_prefix."_crmentity WHERE accountname =? and ".$table_prefix."_account.accountid = ".$table_prefix."_crmentity.crmid and ".$table_prefix."_crmentity.deleted != 1";
...
{
	global $current_user,$log,$adb,$table_prefix;
	if(!validateSession($username,$sessionid))
	return null;
	...
	{
		$query = "select " . $adb->sql_escape_string($tablename) . " from ".$table_prefix."_". $adb->sql_escape_string($tablename);		
	...
	{
		$query = "select " .$adb->sql_escape_string($tablename) . " from ".$table_prefix."_". $adb->sql_escape_string($tablename) ." inner join ".$table_prefix."_role2picklist on ".$table_prefix."_role2picklist.picklistvalueid = ".$table_prefix."_". $adb->sql_escape_string($tablename) .".picklist_valueid where roleid=? and picklistid in (select picklistid from ".$table_prefix."_". $adb->sql_escape_string($tablename)." ) order by sortid";	
...
function unsetServerSessionId($id)
{
	...
	$adb->query("delete from ".$table_prefix."_soapservice where type='FireFox' and id=$id");
...
function getServerSessionId($id)
{
	...
	$query = "select * from ".$table_prefix."_soapservice where type='FireFox' and id={$id}";

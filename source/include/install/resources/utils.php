...
class Migration_Utils {
	...
	function migrate($migrationInfo){
		...
		require($migrationInfo['root_directory'].'/vteversion.php');
		if($adb->num_rows($res)) {
...
//crmv@10488
\$REMINDER_EMAIL_ID ='reminder@vte.com';
\$REMINDER_NAME = 'Notification System';
//crmv@10488 e\
...
\$PORTAL_URL = '{$this->siteUrl}/portal';
//crmv@18100
// root directory path
...
//show reflection on company logo
...
class Common_Install_Wizard_Utils {
	...
	public static $recommendedDirectives = array (
		...
		'allow_call_time_pass_reference' => 'On',	// crmv@114604 - not used for PHP >= 5.4
		'log_errors' => 'Off',
		'short_open_tag' => 'On',
...
//			$directiveValues['session.gc_maxlifetime'] = ini_get('session.gc_maxlifetime');
	...
	public static function installSelectedOptionalModules($selected_modules, $source_directory='', $destination_directory='') {
		...
		$languagePacks = array();
		if ($handle = opendir('packages/vte/optional')) {	//crmv@change packets path
		    while (false !== ($file = readdir($handle))) {
				...
				$packagepath = "packages/vte/optional/$file";	//crmv@change packets path
				$package = new Vtiger_Package();
				$module = $package->getModuleNameFromZip($packagepath);

...
if (typeof(GroupConditions) == 'undefined') {
	GroupConditions = {
		...
		groupLabel: false,
		groupGlue: true,
		relatedFields: true,	// if false fields of the related module are hidden and it is shown only the relation field in the primary module
		subGroup: false,
		subGroupLabel: '',
		...
		init: function($, moduleName, container, context, conditions, params, initCallback){
			var vtinst = new VtigerWebservices("webservice.php",undefined,undefined,true,false);	//crmv@120039
			var fieldValidator;
			var desc = null;
			var me = this;
			...
			var operations = function(){
				...
				var op = {
					...
					reference:["is", "is not", "contains", "does not contain", "starts with", "ends with"],
				...
				var mapping = [
					...
					['date', ['datetime','date']],
					['value', ['reference', 'picklist', 'multipicklist', 'time', 'boolean']],
					['reference', ['reference']],
					['table', ['table']],
					['sdk', ['sdk']]
				];
							...
							function filteredFields(fields){
								return filter(
						  			function(e){
						  				...
						  				if (me.relatedFields) var filter = ['reference', 'autogenerated', 'owner', 'password'];

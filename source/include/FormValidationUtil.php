...
function getDBValidationData($tablearray, $tabid='', &$otherInfo=null, $focus=null) {	//crmv@96450
	if($tabid != '') {
		...
		$query = "SELECT {$table_prefix}_field.fieldid, uitype, MIN($col) AS \"mandatory\"
		...
		$query = "SELECT {$table_prefix}_field.*, tmp.$col FROM {$table_prefix}_field INNER JOIN ($query) tmp ON tmp.fieldid = {$table_prefix}_field.fieldid";

...
class WebserviceField{
	...
	public function isMandatory($user=''){
		if (!empty($user)) {
			...
			$query = "select min($mandCol) as \"mandatory\" from {$table_prefix}_profile2field 
			...
			where {$table_prefix}_profile2field.fieldid = ? and {$table_prefix}_profile2field.visible = 0 and {$table_prefix}_profile2field.profileid IN (".generateQuestionMarks($profileList).")";
			//crmv@60969e
			$result = $adb->pquery($query, array($this->fieldId, $profileList));
			if ($result && $adb->num_rows($result) > 0) {
				$mandatory = $adb->query_result($result,0,'mandatory');
				if ($mandatory == '0') {
					return true;
				}
			}

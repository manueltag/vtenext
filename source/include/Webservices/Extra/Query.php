									...
									if (count($field_arr['type']['refersTo']) > 1){
										if (isset($function[$field_arr['name']]) && $function[$field_arr['name']] != '' && $row[$mapping[$function[$field_arr['name']]]] != ''){
											...
											if ($row[$mapping[$field_arr['name']]] != ''){
												$sql_search = "select setype from {$table_prefix}_crmentity where crmid = ?";
												$res_search = $adb->pquery($sql_search,Array($row[$mapping[$field_arr['name']]]));
												if ($res_search && $adb->num_rows($res_search)>0){
													$row_change[$field_arr['name']] = $module_ids[$adb->query_result_no_html($res_search,0,'setype')]."x".$row[$mapping[$field_arr['name']]];
												}
											}
										}
									}

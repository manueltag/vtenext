 ...
 *************************************************************************************/
	...
	class OperationManager{
		...
		private function fillOperationDetails($operationName){
			global $table_prefix;
			$sql = "select * from ".$table_prefix."_ws_operation where name=?";
		...
		private function fillOperationParameters(){
			global $table_prefix;
			$sql = "select * from ".$table_prefix."_ws_operation_parameters where operationid=? order by sequence";
			...
			try{
				...
				if(!$this->preLogin){
					...
					if(is_array($userDetails)){
						...
						$resp = array("sessionName"=>$this->sessionManager->getSessionId(),"userId"=>$userId,"version"=>$API_VERSION,"vtigerVersion"=>$vtigerVersion,"timezone"=>$userDetails->timezonediff);	//crmv@2390m
						return $resp;
					}
				}

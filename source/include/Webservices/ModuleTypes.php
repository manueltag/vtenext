 ...
 *************************************************************************************/
	...
	function vtws_listtypes($fieldTypeList, $user=false){
		...
		try{
			...
			if(!empty($fieldTypeList)) {
				...
				"".$table_prefix."_field.uitype=".$table_prefix."_ws_fieldtype.uitype
				 ...
				 and ".$table_prefix."_user2role.userid=? and fieldtype in (".
				...
				$table_prefix."_ws_entity_tables.table_name where fieldtype=(".
				generateQuestionMarks($fieldTypeList).')';
				$result = $db->pquery($sql, $params);
				$it = new SqlResultIterator($db, $result);
				$entityList = array();
		...
		//crmv@28498
		if (in_array('Newsletter',$accessibleModules) && $_REQUEST['operation'] != '') {
			unset($accessibleModules[array_search('Newsletter',$accessibleModules)]);
		}
		//crmv@28498e

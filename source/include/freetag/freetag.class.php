...
/*
	...
	function get_objects_with_tag($tag, $offset = 0, $limit = 100, $tagger_id = NULL) {
		...
		//crmv@limit query	
		$res = $adb->limitpQuery($sql,$offset,$limit,$params) or die("Error: $sql");
		$retarr = array();
	...
	function get_objects_with_tag_all($tag, $tagger_id = NULL) {
		...
		$sql = "SELECT DISTINCT object_id
        	...
        	//echo $sql;
        //crmv@limit query	
		$res = $adb->pquery($sql, $params) or die("Error: $sql");
		$retarr = array();
		if ($res && $adb->num_rows($res) > 0){
			while($row = $adb->fetchByAssoc($res)) {
				$retarr[] = $row['object_id'];
	...
	function get_objects_with_tag_id($tag_id, $offset = 0, $limit = 100, $tagger_id = NULL) {
		...
		$sql = "SELECT DISTINCT object_id
			...
			ORDER BY object_id ASC ";
		//crmv@limit query	
		$res = $adb->limitpQuery($sql,$offset,$limit) or die("Error: $sql");
		$retarr = array();
		if ($res && $adb->num_rows($res) > 0){
			while($row = $adb->fetchByAssoc($res)) {
				$retarr[] = $row['object_id'];
	...
	function safe_tag($tagger_id, $object_id, $tag, $module) {
		...
		//crmv@limit query
		$res = $adb->pquery($sql, $params) or die("Syntax Error: $sql");
		if ($res && $adb->num_rows($res) > 0){	
		...
		if(!$rs->EOF) {
			...
			// Add new tag! 
			$tag_id = $adb->getUniqueId("${prefix}freetags"); //crmv@49274
		...
		$sql = "INSERT INTO ${prefix}freetagged_objects
			(tag_id, tagger_id, object_id, tagged_on, module) VALUES (?,?,?,?,?)";
		$params = array($tag_id, $tagger_id, $object_id,$adb->formatDate(date('Y-m-d H:i:s'), true),$module);
		$rs = $adb->pquery($sql, $params) or die("Syntax error: $sql");
		//crmv@fix date end
		return true;
	}
	...
	function delete_object_tag($tagger_id, $object_id, $tag) {
		...
		if($tag_id > 0) {
			...
			$sql = "DELETE FROM ${prefix}freetagged_objects
				WHERE tagger_id = ? AND object_id = ? AND tag_id = ?";
			$params = array($tagger_id, $object_id, $tag_id);
			$rs = $adb->limitpQuery($sql,$offset,1,$params) or die("Syntax Error: $sql");	
			//crmv@limit query end
			return true;
		} else {
			return false;	
	...
	function get_tag_id($tag) {
		...
		$sql = "SELECT id FROM ${prefix}freetags
			WHERE tag = ?";	
		$params = Array($tag);	
		$res = $adb->limitpQuery($sql,0,1,$params) or die("Syntax Error: $sql");	
		if ($res && $adb->num_rows($res) > 0)	
	...
	function get_most_popular_tags($tagger_id = NULL, $offset = 0, $limit = 25) {
		...
		//crmv@limit query/fix assoc	
		$res = $adb->limitpQuery($sql,$offset,$limit,$params) or die("Syntax Error: $sql");
		$retarr = array();
		while($row = $adb->fetchByAssoc($res)) {
			$retarr[] = array(
					'tag' => $row['tag'],
		...
		//crmv@limit query/fix assoc end
		return $retarr;
	...
	function count_tags($tagger_id = NULL) {
		...
		$res = $adb->pquery($sql, $params) or die("Syntax Error: $sql");
		if($res && $adb->num_rows($res) > 0) {
			return $adb->query_result($res,0,'count');
		}
		//crmv@fix assoc end
		return false;
	...
	function get_tag_cloud_tags($max = 100, $tagger_id = NULL,$module = "",$obj_id = NULL) {
		...
		$res = $adb->limitpQuery($sql,0,$max,$params) or die("Syntax Error: $sql");
		$retarr = array();
		while($row = $adb->fetchByAssoc($res)) {
	...
	function similar_objects($object_id, $threshold = 1, $max_objects = 5, $tagger_id = NULL) {
		...
		//crmv@limit query	
		$params = Array($tagArray, $threshold);
		$res = $adb->limitpQuery($sql,0,$max_objects,$params) or die("Syntax Error: $sql, Error: " . $adb->ErrorMsg());
		while($row = $adb->fetchByAssoc($res)) {
			$retarr[] = array (

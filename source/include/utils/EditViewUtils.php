...
function getOutputHtml($uitype, $fieldname, $fieldlabel, $maxlength, $col_fields,$generatedtype,$module_name,$mode='',$readonly='',$typeofdata='',$dynaform_info=array())	//crmv@96450
{
	...
	elseif($uitype == 5 || $uitype == 6 || $uitype ==23)
	{
		...
		//crmv@96450 crmv@116011 crmv120769
		...
		elseif ((in_array($_REQUEST['file'],array('ProcessMaker/actions/UpdateForm','ProcessMaker/actions/CreateForm')) || $_REQUEST['cycle_action'] == 'InsertTableRow') && strpos($value,'$') !== false)	// old mode
			$disp_value = $value;
		//crmv@96450e crmv@116011e crmv@120769e
		else
		{
	...
	elseif($uitype == 52 || $uitype == 77 || $uitype == 54)
	{
		...
		{
			if ($uitype == 54)
				$combo = get_select_options_array(get_group_array($add_blank, "Active", $assigned_user_id,'private'), $assigned_user_id);
			else
				$combo = get_select_options_array(get_user_array($add_blank, "Active", $assigned_user_id,'private'), $assigned_user_id);
		}
		else
		{
			if ($uitype == 54)
				$combo = get_select_options_array(get_group_array($add_blank, "Active", $assigned_user_id), $assigned_user_id);
			else
				$combo = get_select_options_array(get_user_array($add_blank, "Active", $assigned_user_id), $assigned_user_id);
		}
		$fieldvalue [] = $combo;
	}
	//crmv@101683e
	...
	elseif($uitype == 56)
	{
		$editview_label[]=getTranslatedString($fieldlabel, $module_name);
		$fieldvalue[] = $value;
		//crmv@55961 crmv@78396
		if ($mode == '' && in_array($module_name,array('Accounts','Leads','Contacts')) && $fieldname == 'newsletter_unsubscrpt' && $_REQUEST['module'] != 'CustomView') {
			$readonly = 100;
		}
		//crmv@55961e crmv@78396e
		//crmv@96450 crmv@115268
		if (in_array($_REQUEST['file'],array('ProcessMaker/actions/UpdateForm','ProcessMaker/actions/CreateForm')) || $_REQUEST['cycle_action'] == 'InsertTableRow') {
			$options = array();
	...
	elseif($uitype == 30)
	{
		...
		$fieldvalue[] = array(array(0,32,'remdays',getTranslatedString('LBL_DAYS'),$rem_days),array(0,24,'remhrs',getTranslatedString('LBL_HOURS'),$rem_hrs),array(1,60,'remmin',getTranslatedString('LBL_MINUTES'),$rem_min)); // crmv@98866
	...
	elseif($uitype == 1020){
		...
		$temp_val = $value;
		//crmv@36509
		switch ($sla_config_global[time_measure]){
			case 'minutes':

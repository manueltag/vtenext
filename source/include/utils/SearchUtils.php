...
function getAllModulesForTag(){
	...
	while($modulerow = $adb->fetch_array($moduleres)) {
		if((is_admin($current_user) || isPermitted($modulerow['name'], 'DetailView') == 'yes') && !in_array($modulerow['name'],array('Messages','Emails'))) {
...
function getdashboardcondition()
{
	...
	{
		array_push($where_clauses, $table_prefix."_potential.closingdate >= ".$adb->quote($date_closed_start)." and ".$table_prefix."_potential.closingdate <= ".$adb->quote($date_closed_end));
	...
	if(isset($sales_stage) && $sales_stage!=''){
		...
		array_push($where_clauses, "(".$table_prefix."_potential.sales_stage <> 'Closed Won' and ".$table_prefix."_potential.sales_stage <> 'Closed Lost')");
		else
		array_push($where_clauses, $table_prefix."_potential.sales_stage = ".$adb->quote($sales_stage));
		$url_string .= "&sales_stage=".$sales_stage;
	}
	if(isset($lead_source) && $lead_source != "") {
		array_push($where_clauses, $table_prefix."_potential.leadsource = ".$adb->quote($lead_source));
		$url_string .= "&leadsource=".$lead_source;
	}
	if(isset($date_closed) && $date_closed != "") {
		array_push($where_clauses, $adb->getDBDateString($table_prefix."_potential.closingdate")." like ".$adb->quote($date_closed.'%')."");
		$url_string .= "&date_closed=".$date_closed;
	}
	if(isset($owner) && $owner != ""){
		$user_qry="select ".$table_prefix."_users.id from ".$table_prefix."_users where ".$table_prefix."_users.user_name = ?";
		$res = $adb->pquery($user_qry, array($owner));
		$uid = $adb->query_result($res,0,'id');
		array_push($where_clauses, $table_prefix."_crmentity.smownerid = ".$uid);
		//ds@39
	...
	{
		array_push($where_clauses, $table_prefix."_campaigncontrel.campaignid = ".$campaign);
                $url_string .= "&campaignid=".$campaign;
	}
	if(isset($quote) && $quote != "")
	{
		array_push($where_clauses, $table_prefix."_inventoryproductrel.id = ".$quote);
		$url_string .= "&quoteid=".$quote;
	}
	if(isset($invoice) && $invoice != "")
	{
		array_push($where_clauses, $table_prefix."_inventoryproductrel.id = ".$invoice);
		$url_string .= "&invoiceid=".$invoice;
	}
	if(isset($po) && $po != "")
	{
		array_push($where_clauses, $table_prefix."_inventoryproductrel.id = ".$po);

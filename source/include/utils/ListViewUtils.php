...
class ListViewUtils extends SDKExtendableUniqueClass {
	...
	*/
	public function getListViewHeader($focus, $module,$sort_qry='',$sorder='',$order_by='',$relatedlist='',$oCv='',$relatedmodule='',$skipActions=false, $nohtml = false) // crmv@31780
	{
		...
		if ($module == 'Users' || ($module == 'Projects' && isProjectLeader() && !isProjectAdmin())) {
					...
					//crmv@9433 end
						...
						if(($module == 'Calendar' || $module == 'Emails' || $module == 'Fax' || $module == 'Sms' || $module == 'HelpDesk' || $module == 'Invoice' || $module == 'Leads' || $module == 'Contacts') && (($fieldname=='parent_id') || ($name=='Contact Name') || ($name=='Close') || ($fieldname == 'firstname'))) {
							...
							//crmv@23515e
								...
								if($name=='Contact Name') {
									...
									if(($contact_name != "") && ($contact_id !='NULL')) {
										...
										$value =  "<a href='index.php?module=Contacts&action=DetailView&parenttab=".$tabname."&record=".$contact_id."' style='".$P_FONT_COLOR."'>".$contact_name."</a>"; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
									}
								}
							...
							if($module == 'Accounts') {
								...
								// Fredy Klammsteiner, 4.8.2005: changes from 4.0.1 migrated to 4.2
								$value = '<a href="index.php?module=Accounts&action=DetailView&record='.$account_id.'&parenttab='.$tabname.'" style="'.$P_FONT_COLOR.'">'.$account_name.'</a>'; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
							}
							/*
							//crmv@12035
							...
							*/
							elseif($module == 'Projects') {
								...
								$value = '<a href="index.php?module=Accounts&action=DetailView&record='.$account_id.'&parenttab='.$tabname.'">'.$accountname.'</a>'; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
							} else {
								$account_id = $adb->query_result($list_result,$i-1,"accountid");
								$account_name = getAccountName($account_id);
								$acc_name = textlength_check($account_name);
								// Fredy Klammsteiner, 4.8.2005: changes from 4.0.1 migrated to 4.2
								$value = '<a href="index.php?module=Accounts&action=DetailView&record='.$account_id.'&parenttab='.$tabname.'" style="'.$P_FONT_COLOR.'">'.$acc_name.'</a>'; // Armando Luscher 05.07.2005 -> priority -> Desc: inserted style="$P_FONT_COLOR"
							}
	...
	*/
	function getSearchListViewEntries($focus, $module,$list_result,$navigation_array,$form='',$oCv='')
	{
		...
		$ui_col_array=Array();
		$readonly_array = array();	//crmv@sdk-18508
		$webserviceFields = array();	//crmv@124736
		//crmv@124736
		$query = "SELECT *
			FROM ".$table_prefix."_field
			WHERE tabid=?
		...
		for($i=0;$i<$num_rows;$i++)
		{
			...
			$readonly_array[$field_name]=$readonly;
			//crmv@sdk-18508 e
			$webserviceFields[$field_name] = WebserviceField::fromQueryResult($adb,$result,$i);	//crmv@124736
		}
		//end
						...
						//crmv@7216e
							...
							*/
							...
							elseif(isset($focus->popup_fields) && in_array($fieldname, $focus->popup_fields)) {
								...
								if(isset($forfield) && $forfield != '' && $focus->popup_type != 'detailview') {
									...
									if (($p = strpos($value, "...")) !== false) {
										...
										$autoValue = substr($value, 0, $p+3);
									} else {
										// use the original value
										$autoValue = $value;
									}
									$autocomplete_return_function[$entity_id] = "vtlib_setvalue_from_popup($entity_id, \"$autoValue\", \"$forfield\");";
									$value = "<a href='javascript:void(0);' onclick='{$autocomplete_return_function[$entity_id]}closePopup();'>$value1</a>"; //crmv@21048m
									//crmv@16312 end
									//crmv@29190e
								}
							...
							else
							{
								...
								//crmv@124736
								if (in_array($webserviceFields[$fieldname]->getFieldDataType(),array('reference','owner'))) {
				...
				if($mode == "search")
				{
					...
					else
					{
						...
						elseif ($_REQUEST['fromCalendar'] == 'fromCalendar') {	//crmv@54926 crmv@72993
							...
							// js function
							$autocomplete_return_function[$entity_id] = "top.addInvitee($args);";
						...
						//crmv@26807e
							...
							$autocomplete_return_function[$entity_id] = 'set_return("'.$entity_id.'", "'.str_replace(array("\r","\n"),array(' ',' '),decode_html($temp_val)).'");';	//crmv@53108
							$value = '<a href="javascript:void(0);" onclick=\''.$autocomplete_return_function[$entity_id].'closePopup();\'>'.$temp_val.'</a>'; //crmv@21048m
						}
					}
					//crmv@29190e
	...
	public function getListQuery($module,$where='')
	{
		...
		Case "Leads":
			$query = "SELECT ".$table_prefix."_crmentity.crmid, ".$table_prefix."_crmentity.smownerid,
				...
				$reportFilterJoin $extraJoin";
			$query .= getNonAdminAccessControlQuery($module,$current_user);
			$query .= "WHERE ".$table_prefix."_crmentity.deleted = 0 $setypeCond AND ".$table_prefix."_leaddetails.converted = 0 ".
				$where;
				break;
		Case "Products":
			$query = "SELECT ".$table_prefix."_crmentity.crmid, ".$table_prefix."_crmentity.description, ".$table_prefix."_products.*, ".$table_prefix."_productcf.*
				...
				$query .= ' '.getNonAdminAccessControlQuery($module,$current_user); // crmv@109663
				$query .= " WHERE ".$table_prefix."_crmentity.deleted = 0 $setypeCond ".$where;
				break;
		Case "Documents":
			// crmv@30967
		...
		Case "Emails":
			...
			$query .= getNonAdminAccessControlQuery($module,$current_user);
			$query .= "AND ".$table_prefix."_crmentity.deleted = 0 $setypeCond ".$where;
				break;
		Case "Faq":
		...
		Case "Vendors":
			$query = "SELECT ".$table_prefix."_crmentity.crmid, ".$table_prefix."_vendor.*
				...
				$reportFilterJoin $extraJoin
				WHERE ".$table_prefix."_crmentity.deleted = 0 $setypeCond ".$where;
				break;
		Case "PriceBooks":
...
<ul class="pagination pagination-sm">
	...
	<li><a href="javascript:void(0);"><label for="pagination_page">Pagina</label> <input id="pagination_page" type="text" class="form-control pagenumber" maxlength="5" value="{$navigation_array['current']}" onchange="{$pageLink}" onkeypress="{$jsHandler}"> <label for="pagination_page">{$app_strings['LBL_LIST_OF']} {$navigation_array['verylast']}</label></a></li>
...
PAGIN;
	...
	//		$output = '<td align="right" style="padding: 5px;">';
		...
		if ($module == 'Messages') { //crmv@2963m
			...
			$output .= '<a href="javascript:;" onClick="'.$firstLink.'" alt="'.$app_strings['LBL_FIRST'].'" title="'.$app_strings['LBL_FIRST'].'"><img src="' . vtiger_imageurl('start.gif', $theme) . '" border="0" align="absmiddle"></a>&nbsp;';
			$output .= '<a href="javascript:;" onClick="'.$prevLink.'" alt="'.$app_strings['LNK_LIST_PREVIOUS'].'"title="'.$app_strings['LNK_LIST_PREVIOUS'].'"><img src="' . vtiger_imageurl('previous.gif', $theme) . '" border="0" align="absmiddle"></a>&nbsp;';
		}else{
		...
		if(($navigation_array['next']) !=0){
			...
			if ($module == 'Messages'){
				...
				$output .= '<a href="javascript:;" id="appendNextListViewEntries" onClick="appendListViewEntries_js(\''.$module.'\',\'parenttab='.$tabname.'&start='.$navigation_array['next'].$url_string.'\');" alt="'.$app_strings['LNK_LIST_NEXT'].'" title="'.$app_strings['LNK_LIST_NEXT'].'"><img src="' . vtiger_imageurl('next.gif', $theme) . '" border="0" align="absmiddle"></a>&nbsp;';
			} else {
				$output .= '<a href="javascript:;" onClick="'.$nextLink.'" alt="'.$app_strings['LNK_LIST_NEXT'].'" title="'.$app_strings['LNK_LIST_NEXT'].'"><img src="' . vtiger_imageurl('next.gif', $theme) . '" border="0" align="absmiddle"></a>&nbsp;';
				$output .= '<a href="javascript:;" onClick="'.$lastLink.'" alt="'.$app_strings['LBL_LAST'].'" title="'.$app_strings['LBL_LAST'].'"><img src="' . vtiger_imageurl('end.gif', $theme) . '" border="0" align="absmiddle"></a>&nbsp;';
			}
			//crmv@2963me
		}else{
...
function getRelatedTo($module,$list_result,$rset)
{
    ...
    {
    	global $theme;
    	$module_icon = '<img src="themes/images/modulesimg/'.$parent_module.'.png" alt="'.$app_strings[$parent_module].'" title="'.$app_strings[$parent_module].'" border=0 align=center height="20"> ';
    }
...
function textlength_check($field_val) {
	...
	if ($listview_max_textlength && $listview_max_textlength > 0) {
		...
		if (function_exists('mb_substr')) {
			if(mb_strlen($temp_val,$default_charset) > $listview_max_textlength) {

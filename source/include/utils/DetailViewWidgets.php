...
include_once('include/utils/crmv_utils.php');
...
class AccountsHierarchy extends DefaultWidget {
	...
	function install() {
		...
		Vtiger_Link::addLink($moduleInstance->id, 'DETAILVIEWWIDGET', 'AccountsHierarchy', 'block://DetailViewWidgets:include/utils/DetailViewWidgets.php');
	}
...
class PendingHistory extends DefaultWidget {
	...
	function install() {
		global $adb, $table_prefix;
		$result = $adb->query("SELECT tabid FROM {$table_prefix}_tab WHERE isentitytype = 1 AND presence = 0 AND name not in ('Emails','Fax','Sms','PBXManager','ModComments','ModNotifications','ChangeLog','Charts','Messages','MyNotes')");
		if ($result && $adb->num_rows($result) > 0) {
			while($row=$adb->fetchByAssoc($result)) {
				Vtiger_Link::addLink($row['tabid'], 'DETAILVIEWWIDGET', 'PendingHistory', 'block://DetailViewWidgets:include/utils/DetailViewWidgets.php');
			}
		}
	}
	...
	function getRecords($module,$record) {
		...
		$relations = $rm->getRelatedIds($module, $record, null, array('ModComments','ChangeLog'), false, true);

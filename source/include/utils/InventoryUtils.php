...
// crmv@31780
// restiuisce vero se il modulo ha i prodotti
...
//			} //crmv@43358
	...
	function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='') {
		...
		if ($focus->mode == 'edit') {
			...
			$this->deleteInventoryProductDetails($focus);
		}
		...
		for ($i=1; $i<=$tot_no_prod; ++$i) {
			...
			$lineitem_id = $adb->getUniqueID($table_prefix.'_inventoryproductrel');
	...
	function getAssociatedProducts($module,$focus,$seid='') {
		...
		$_SESSION['query_show'] .= $result->sql."\n";	//crmv@show_query

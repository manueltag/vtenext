...
class CRMVUtils extends SDKExtendableUniqueClass {
	...
	function getJSGlobalVars() {
		...
		$jsvar = array(
			...
			'theme_config' => $TU->getAll(), // crmv@120738
		);
	...
	//crmv@3085m
	function getPinRelatedLists($tabid) {
		global $adb, $table_prefix, $current_user;
		$query = "select {$table_prefix}_relatedlists_pin.relation_id, {$table_prefix}_tab.name, {$table_prefix}_relatedlists.label
					from {$table_prefix}_relatedlists_pin
					inner join {$table_prefix}_relatedlists on {$table_prefix}_relatedlists.relation_id = {$table_prefix}_relatedlists_pin.relation_id
					inner join {$table_prefix}_tab on {$table_prefix}_relatedlists.related_tabid = {$table_prefix}_tab.tabid
					where userid = ? and {$table_prefix}_relatedlists.tabid = ?";
		...
		//crmv@61012
		$query = "select {$table_prefix}_relatedlists_pin.relation_id, {$table_prefix}_relatedlists.label
		from {$table_prefix}_relatedlists_pin
		inner join {$table_prefix}_relatedlists on {$table_prefix}_relatedlists.relation_id = {$table_prefix}_relatedlists_pin.relation_id
		left join {$table_prefix}_tab on {$table_prefix}_relatedlists.related_tabid = {$table_prefix}_tab.tabid
		where {$table_prefix}_tab.tabid is null and userid = ? and {$table_prefix}_relatedlists.tabid = ?";
		$result = $adb->pquery($query,array($current_user->id,$tabid));
		if ($result && $adb->num_rows($result)) {
			while($row=$adb->fetchByASsoc($result)) {
	...
	//crmv@62415
	function getPinRelationIds($tabid) {
		$relatedlists = Array();
		global $adb, $table_prefix, $current_user;
	...
	function getHolidays($from, $to, $return_mode=null) {
		include_once('modules/SLA/SLA.php');
		$sla_config_global = SLA::get_config();
		$holidayDays = array();
		if (!empty($from) && !empty($to)) {	//crmv@106740
			$pasqua = date("d-m",easter_date(date('Y')));
			$pasquetta = date("d-m",strtotime("+ 1 day",easter_date(date('Y'))));
			$holidays = $sla_config_global['HelpDesk']['holidays'];
			foreach($holidays as $h) {
				list($d,$m) = explode('-',$h);
				if ($return_mode == 'jQueryGantt') {
					($h == $pasqua || $h == $pasquetta) ? $holidayDays[] = date('Y')."_{$m}_{$d}" : $holidayDays[] = "{$m}_{$d}";
				} else {
					($h == $pasqua || $h == $pasquetta) ? $holidayDays[] = date('Y')."-{$m}-{$d}" : $holidayDays[] = "*-{$m}-{$d}";
				}
			}
			// add other easter date
			// TODO cyrcle for all years between "from" to "to"
			($return_mode == 'jQueryGantt') ? $formt = 'Y_m_d' : $formt = 'Y-m-d';			
			if (substr($from,0,4) != date('Y')) {
				$y = substr($from,0,4);
				$holidayDays[] = date($formt,easter_date($y));
				$holidayDays[] = date($formt,strtotime('+ 1 day',easter_date($y)));
			}
			if (substr($to,0,4) != date('Y')) {
				$y = substr($to,0,4);
				$holidayDays[] = date($formt,easter_date($y));
				$holidayDays[] = date($formt,strtotime('+ 1 day',easter_date($y)));
			}
...
function getUserOptionsHTML($selected_user_id,$module_name,$parenttab,$file='') {	//crmv@OPER6288
	...
	//crmv@28496e
	//crmv@18592 crmv@OPER6288
	if (!empty($file)) $file = ",\"{$file}\"";
	$htmlStr = "<select name='lv_user_id' class='detailedViewTextBox' id='lv_user_id' onChange='showDefaultCustomView(null,\"$module_name\",\"$parenttab\", \"{$_REQUEST['folderid']}\"{$file});'>"; // crmv@30967
	//crmv@18592e crmv@OPER6288e
...
function getDuplicateQuery($module,$field_values,$ui_type_arr)
{
	...
	{
		//crmv@36508
		if ($_SESSION['duplicateshandling_empty_flag'] === true){
			add_empty_clause($field_values,$sec_parameter);
		}
		unset($_SESSION['duplicates_'.$module]);
		...
		if($module == 'Contacts')
		{
			...
			if ($res_insert){
				...
				if ($res_count){
					$_SESSION['duplicates_'.$module] = $adb->query_result_no_html($res_count,0,'count');
				}
...
function get_on_clause($field_list,$uitype_arr,$module)
{
	...
	foreach($field_array as $fld)
	{
		...
		$fld_name = $sub_arr[2];
		//crmv@36508
		if (($module == 'Accounts' || $module == 'Contacts' || $module == 'Leads') && $_SESSION['duplicateshandling_empty_flag'] === true){ // crmv@77469
...
function getEnterpriseProject() {
	...
	if (empty($enterprise_project)) {
		if (!empty($_SESSION['enterprise_project'])) {
			$enterprise_project = $_SESSION['enterprise_project'];
		} else {
			global $adb, $table_prefix;
			$result = $adb->query("SELECT enterprise_project FROM {$table_prefix}_version");
			$_SESSION['enterprise_project'] = $enterprise_project = $adb->query_result_no_html($result, 0, 'enterprise_project');
		}
	}
...
function getMenuModuleList($otherAll=false) {	/* crmv@30356	crmv@32217	crmv@31250	crmv@42707	crmv@47905bis crmv@124735 */
	...
	if ($tmp === false) {
		$tmp = array();
		if (!isMobile()) $sql_cond = ' AND '.$table_prefix.'_tab.name NOT IN (\'Messages\',\'Processes\')';
		$sql = 'SELECT '.$table_prefix.'_tab.tabid,'.$table_prefix.'_tab.name,tbl_s_menu_modules.fast,tbl_s_menu_modules.sequence FROM '.$table_prefix.'_tab
			...
			WHERE '.$table_prefix.'_tab.presence = 0 '.$sql_cond.'
	...
	if (is_array($columns_to_check)) {
		...
		if ($found_users == true) {
			if (stripos($query," join {$table_prefix}_users")===false && stripos($query," from {$table_prefix}_users")===false){
				$join_check = " inner join {$table_prefix}_users on {$table_prefix}_users.id = {$table_prefix}_crmentity.smownerid";
				$query = preg_replace('/\swhere\s/i', " $join_check where ", $query, 1);
			}
			if (stripos($query," join {$table_prefix}_groups")===false && stripos($query," from {$table_prefix}_groups")===false){
				$join_check = " inner join {$table_prefix}_groups on {$table_prefix}_groups.groupid = {$table_prefix}_crmentity.smownerid";
				$query = preg_replace('/\swhere\s/i', " $join_check where ", $query, 1);
			}
		}
		//crmv@82220e
...
function getDefaultUserAvatar($mode='') {
	global $theme;
	if ($mode == 'menu') {
		return vtiger_imageurl("no_avatar_$mode.png",$theme);
	} else {
		return vtiger_imageurl('no_avatar.png',$theme);
	}
}
...
function getDefaultGroupAvatar($mode='') {
	global $theme;
	return vtiger_imageurl('ico-groups_small.png',$theme);
}
//crmv@100731e
//crmv@29615
...
function requestFromMobile() {
	return (preg_match('/^WSMobile.*/', $_SESSION['app_unique_key']) > 0);
}
// crmv@39110e
// crmv@91082
...
function getLVS($module,$param='',$modhomeid='') {
	...
	if (in_array($module,$oldMode)) {
		if (empty($param)) {
			return $_SESSION['lvs'][$module];
		} else {
			return $_SESSION['lvs'][$module][$param];
		}
	} else {
		if (empty($modhomeid)) {
		...
		if (empty($param)) {
			return $_SESSION['lvs'][$module][$modhomeid];
		} else {
			return $_SESSION['lvs'][$module][$modhomeid][$param];
		}
	}
}
function setLVS($module,$value,$param='',$modhomeid='') {
	...
	if (in_array($module,$oldMode)) {
		if (empty($param)) {
			$_SESSION['lvs'][$module] = $value;
		} else {
			$_SESSION['lvs'][$module][$param] = $value;
		}
	} else {
		if (empty($modhomeid)) {
			require_once('include/utils/ModuleHomeView.php');
		...
		if (empty($param)) {
			$_SESSION['lvs'][$module][$modhomeid] = $value;
		} else {
			$_SESSION['lvs'][$module][$modhomeid][$param] = $value;
		}
	}		
}
function unsetLVS($module='',$param='') {
	if (!empty($param)) {
		$oldMode = oldModeSessionModules();
		if (in_array($module,$oldMode)) {
			$_SESSION['lvs'][$module][$param] = '';
		} else {
			require_once('include/utils/ModuleHomeView.php');
			global $current_user;
			$MHW = ModuleHomeView::getInstance($module, $current_user->id);
			$modhomeid = $MHW->getModHomeId();
			$_SESSION['lvs'][$module][$modhomeid][$param] = '';
		}
	} elseif (!empty($module))
...
function getLVSDetails($module,$viewid,$param='') {
	...
	if (in_array($module,$oldMode)) {
		if (!empty($param)) {
			return $_SESSION['lvs'][$module][$viewid][$param];
		} else {
			return $_SESSION['lvs'][$module][$viewid];
		}
	} else {
		require_once('include/utils/ModuleHomeView.php');
		global $current_user;
		...
		$modhomeid = $MHW->getModHomeId();
		if (!empty($param)) {
			return $_SESSION['lvs'][$module][$modhomeid][$viewid][$param];
		} else {
			return $_SESSION['lvs'][$module][$modhomeid][$viewid];
		}
	}	
}
function setLVSDetails($module,$viewid,$value,$param='') {
	...
	if (in_array($module,$oldMode)) {
		if (!empty($param)) {
			$_SESSION['lvs'][$module][$viewid][$param] = $value;
		} else {
			$_SESSION['lvs'][$module][$viewid] = $value;
		}
	} else {
		require_once('include/utils/ModuleHomeView.php');
		global $current_user;
		...
		$modhomeid = $MHW->getModHomeId();
		if (!empty($param)) {
			$_SESSION['lvs'][$module][$modhomeid][$viewid][$param] = $value;
		} else {
			$_SESSION['lvs'][$module][$modhomeid][$viewid] = $value;
		}
	}
}
//crmv@102334e

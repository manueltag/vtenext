...
function getRelatedLists($module,$focus,$turbolift=false)	//crmv@26896
{
	...
	if ($turbolift) {
		...
		$sql1 = "select ".$table_prefix."_relatedlists.*, p2r.visible as profile_visible, p2r.actions as profile_actions, {$table_prefix}_tab.name as related_tabname
				...
				LEFT JOIN {$table_prefix}_profile2related p2r ON p2r.profileid = ? AND p2r.tabid = ? AND p2r.relationid = {$table_prefix}_relatedlists.relation_id
				WHERE {$table_prefix}_relatedlists.tabid=? $condition1
					and {$table_prefix}_tab.presence != 1 and {$table_prefix}_relatedlists.presence != 1
					AND (p2r.visible IS NULL OR p2r.visible = 1)
				ORDER BY COALESCE(p2r.sequence, 0) ASC, coalesce(tb_count,0) DESC, {$table_prefix}_relatedlists.sequence ASC";
	} else {
	//crmv@26896e
		$sql1 = "select {$table_prefix}_relatedlists.*, p2r.visible as profile_visible, p2r.actions as profile_actions, {$table_prefix}_tab.name as related_tabname
		   FROM {$table_prefix}_relatedlists
		   LEFT join {$table_prefix}_tab ON {$table_prefix}_tab.tabid = {$table_prefix}_relatedlists.related_tabid
		   LEFT JOIN {$table_prefix}_profile2related p2r ON p2r.profileid = ? AND p2r.tabid = ? AND p2r.relationid = {$table_prefix}_relatedlists.relation_id
		   WHERE
		    {$table_prefix}_relatedlists.tabid = ?
		    AND ({$table_prefix}_tab.tabid IS NULL OR {$table_prefix}_tab.presence != 1) and {$table_prefix}_relatedlists.presence != 1
		    AND (p2r.visible IS NULL OR p2r.visible = 1)
		   ORDER BY COALESCE(p2r.sequence, {$table_prefix}_relatedlists.sequence)";
	}
	//crmv@26896
	$params = array($firstUserProfile , $cur_tab_id, $cur_tab_id);

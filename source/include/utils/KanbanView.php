...
class KanbanView extends SDKExtendableUniqueClass {
	...
	function getList($columnId,$user_id='',$page=0) {
		...
		if (!empty($orberby)) {
			...
			if(!empty($order_by) && $order_by != '' && $order_by != null) {
				if($order_by == 'smownerid') {
					...
					$list_query .= $focus->getFixedOrderBy($module,$order_by,$sorder);
				}
			}
		}
		$limit_start_rec = $page * $this->list_max_entries_per_page;
		...
		$entries = $controller->getListViewEntries($focus,$module,$list_result,null,true,'',array('KanbanView'=>true,'doNotEvaluate'=>array('assigned_user_id')));
...
class KanbanLib extends SDKExtendableUniqueClass {
	...
	public $excludeModules = array(
		...
		'Charts','M','Myfiles','ModNotifications',
		'ChangeLog','MyNotes','ProductLines','Messages'
	);
	...
	public function save($cvid, $json, $relation='') {
		...
		if (!empty($relation) && !is_numeric($relation)) {
			...
			$relModuleInstance = Vtiger_Module::getInstance($relation);
			...
			while($rw=$adb->fetchByAssoc($res)){
				...
				if ($result && $adb->num_rows($result) > 0) {
					...
					if ($fieldname != '') {
						...
						if (Vtiger_Utils::CheckTable($picktable)) {

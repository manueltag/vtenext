...
class WizardUtils extends SDKExtendableUniqueClass {
	...
	public function addLinkToParents($wizardid) {
		...
		/*
		$moduleInstance = Vtiger_Module::getInstance($mainMod);
		if ($moduleInstance) {
			Vtiger_Link::addLink($moduleInstance->id, 'LISTVIEWBASIC', $label, "javascript:Wizard.openWizard('{$mainMod}',{$wizardid});");
		}
		*/
	...
	public function getAllParentModules() {
		...
		$skipMods = array('MyNotes', 'Users', 'Emails', 'Myfiles', 'Charts', 'Messages', 'PBXManager', 'ChangeLog', 'ModComments', 'ModNotifications');
		$res = $adb->pquery(
...
class WizardGenerator extends SDKExtendableUniqueClass {
	...
	public function getAvailableRelations($module) {
		...
		$skipMods = array('ModComments', 'ChangeLog', 'ModNotifications', 'Messages', 'MyNotes');

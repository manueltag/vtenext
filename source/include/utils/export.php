...
function export($type){
	...
	$query = $focus->create_export_query($where,$oCustomView,$viewid);	//crmv@31775
	//crmv@21916
	/*
	...
	*/
	...
	if(($search_type == 'withoutsearch' || $search_type == 'includesearch') && $export_data == 'selecteddata'){
		$idstring = explode(";", vtlib_purify($_REQUEST['idstring'])); // crmv@37463
		//crmv@14086 - start
		$list_max_entries_per_page=count($idstring);
		//crmv@14086 - end
		if($type == 'Accounts' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_account.accountid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Contacts' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_contactdetails.contactid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Potentials' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_potential.potentialid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Leads' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_leaddetails.leadid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Products' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_products.productid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Documents' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_notes.notesid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'HelpDesk' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_troubletickets.ticketid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Vendors' && count($idstring) > 0) {
			$query .= ' and '.$table_prefix.'_vendor.vendorid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
	...
	if(isset($order_by) && $order_by != ''){
		if($order_by == 'smownerid'){
			...
			$query .= $focus->getFixedOrderBy($type,$order_by,$sorder);	//crmv@38949
		}
	}
    ...
    while($val = $adb->fetchByAssoc($result, -1, false)){
		...
		$line = implode("\";\"",$new_arr);	//crmv@27473
		$line = "\"" .$line;
		$line .= "\"\r\n";

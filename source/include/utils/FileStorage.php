...
class FileStorage extends SDKExtendableUniqueClass {
	...
	/**
	* This function is used to add the vtiger_attachments. This will call the function uploadAndSaveFile 
	...
	*/
	...
	public function insertAttachment($attid, $path, $module, &$file_details, $binFile, &$focus = null) {
		...
		$params1 = array($attid, $current_user->id, $ownerid, $setype, $focus->column_fields['description'], $adb->formatDate($date_var, true), $adb->formatDate($date_var, true));
	...
	public function downloadFile($attid) {
		...
		if ($adb->num_rows($result) == 1) {
			...
			$this->rawFileOutput($path, $dlname, $fileType);
		} else {
			echo "Attachment does not exist";
		}
	}
	
	public function rawFileOutput($path, $downloadName = null, $fileType = null) {

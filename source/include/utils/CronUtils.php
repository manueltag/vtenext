...
class CronManager {
	...
	public function run($idlist = array()) {
		global $adb, $table_prefix;
	...
	function checkRunnable($job) {
		...
		$runnableStatuses = array(CronJob::$STATUS_ERROR, CronJob::$STATUS_TIMEOUT_END, CronJob::$STATUS_EMPTY);
		$res = $adb->pquery(
			"SELECT cronid FROM {$table_prefix}_cronjobs WHERE cronid = ? AND (status IS NULL OR status in (".generateQuestionMarks($runnableStatuses)."))",
			array($job->getId(), $runnableStatuses)
		);
	...
	static function errorHandler(&$job, $logfile = null) {
		global $adb, $table_prefix;
	...
	public function runJob(&$job) {
		...
		$job->setPid();
		$job->setStatus(CronJob::$STATUS_PROCESSING, $now, 0); // crmv@102956
...
class CronJob {
	...
	public function shouldRunNow($cronInterval = 60) {
		/*
		...
		// and now with hours
			...
			foreach ($this->runHours as $rh) {
				...
				if (preg_match('/\d{1,2}:\d{1,2}(:\d{1,2})?/', $rh)) {
					...
					$hoursArrays[0][] = $yesterday->setTime($h, $m, $s);
					$hoursArrays[1][] = $today->setTime($h, $m, $s);
					$hoursArrays[2][] = $tomorrow->setTime($h, $m, $s);
				}
			}

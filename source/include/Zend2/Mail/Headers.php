...
class Headers implements Countable, Iterator
{
    ...
    public static function fromString($string, $EOL = self::EOL)
    {
        $headers     = new static();
        $currentLine = '';
        $currentKey = '';	//crmv@50124

        // iterate the header lines, some might be continuations
        foreach (explode($EOL, $string) as $line) {
        	//crmv@50124
        	if (preg_match('/^(?P<name>[\x21-\x39\x3B-\x7E]+):.*$/', $line, $matches)) {
        		$currentKey = $matches['name'];
        	} elseif (!preg_match('/^\s+.*$/', $line, $matches) && !preg_match('/^\s*$/', $line) && in_array($currentKey,array('To','From','Cc','Ccn'))) {
        		$line = ' '.$line;
        	}
        	//crmv@50124e

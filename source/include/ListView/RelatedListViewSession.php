...
class RelatedListViewSession {
	...
	public static function removeRelatedModuleFromSession($relationId, $header) {
		...
		if (!empty($relationId)) {
			...
			unset($_SESSION['relatedlist'][$currentModule]);
		}
	}
	...
	function getListViewNavigation($relationId, $currentRecordId){
		...
		if(!empty($_SESSION[$relationId.'_RelatedList_Navigation'])){
			$recordNavigationInfo = Zend_Json::decode($_SESSION[$relationId.'_RelatedList_Navigation']);
			$pageNumber =0;
		...
		if($reUseData === false){
			...
			$list_query = $_SESSION[$mod_listquery];
			...
			$_SESSION[$relationId.'_RelatedList_Navigation'] =

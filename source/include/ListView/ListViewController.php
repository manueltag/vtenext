...
class ListViewController extends SDKExtendableClass {
	 ...
	 */
	function getListViewEntries($focus,$module,$result,$navigationInfo,$skipActions=false,$listview_entries_other='',$otherActions='') {	//crmv@34627	//crmv@OPER6288
			...
			if ($module != 'Messages') {
				...
				if(isPermitted($module,"Delete","") == 'yes'){
					...
					if($actionLinkInfo != "" && $del_link != "")
						$actionLinkInfo .=  "&nbsp;";
					if($del_link != "")
						$actionLinkInfo .=	"<a href='javascript:confirmdelete(\"".addslashes(urlencode($del_link))."\")'><i class='vteicon' title='".getTranslatedString("LBL_DELETE",$module)."' >clear</i></a>";
				}
				...
				// END
				//crmv@56233
				if ($editPermission == 'yes' && $module == 'HelpDesk') {
			...
			foreach ($listViewFields as $fieldName) {
				...
				if(strpos($fieldName,'::') !== false) {
					...
					if (empty($tmp_value) && !empty($listview_entries_other[$tmp[0]])) {
						...
						if (!empty($tmp_value['fields']['indirect'])) {	//1->N
							foreach($tmp_value['fields']['indirect'] as $fieldid=>$fieldinfo) {
								...
								} elseif (!empty($fieldinfo['ids']) && count($fieldinfo['ids']) == 1) {
							...
							foreach($tmp_value['related'] as $rel_module=>$rel_ids) {
								if (!empty($rel_ids) && count($rel_ids) > 1) {
									$tmp_value = '<a href="index.php?module='.$this->queryGenerator->getModule().'&action=DetailView&record='.$rawValue.'">'.getTranslatedString('LBL_MULTIPLE').'</a>';
								} elseif (!empty($rel_ids) && count($rel_ids) == 1) {
				...
				//crmv@28670e
					...
					// crmv@67339 - fix for newsletter unsubscription
					if ($field->getFieldName() == 'newsletter_unsubscrpt') {
						...
						if (array_key_exists($module, $mailFields)) {
							$mailField = $mailFields[$module];
							$sql = "SELECT {$mailField} as email FROM {$focusnl->table_name} WHERE {$focusnl->table_index} = ?";
				...
				//crmv@17471 end
					...
					if(!empty($value) && !empty($this->nameList[$fieldName]) && !empty($parentModule)) {
						...
						if ($parentModule == 'Users') {
							...
							if (isset($groupNameList[$tmp_reference_value])) {
					...
					if (isset($otherActions['doNotEvaluate']) && in_array($fieldName,$otherActions['doNotEvaluate'])) {
						...
						$tmp_userid = $value;
						$value = textlength_check($this->ownerNameList[$value]);
						if (isset($groupNameList[$tmp_userid])) {
			...
			case 'Messages':
				if ($result){
					...
					while($row=$adb->fetchByAssoc($result,-1,false)){
						...
						$ret_arr[$row['messagesid']]= Array(
							'subject'=>textlength_check(to_html($row['subject'])),
							'mdate'=>$focus->getFriendlyDate($row['mdate']),
	...
	public function getGridSearch($focus,$module,$sort_qry='',$sorder='',$orderBy='',$skipActions=false,&$grid_search_js_array) {
		...
		if(!empty($_REQUEST['GridSearchCnt'])) {
			...
			for($i=0;$i<$_REQUEST['GridSearchCnt'];$i++) {
				...
				$value = function_exists(iconv) ? @iconv("UTF-8",$default_charset,$value) : $searchValue;
	...
	function createtempresultq_tmp($query,$viewid){ // crmv@63349
		...
		if($table_exist){
			...
			Vtiger_Utils::CreateTable($tableName,"crmid I(19) key");			
		}			
		if ($db->isMysql()){
	...
	function getMoreInformationsDiv($recordId,$fieldName,$moreInformations) {
		$value = '&nbsp;<a href="javascript:;"><i class="vteicon md-sm valign-bottom" onmouseout="getObj(\'content_'.$fieldName.'_'.$recordId.'\').hide();" onmouseover="getObj(\'content_'.$fieldName.'_'.$recordId.'\').show();checkDivPosition(getObj(\'content_'.$fieldName.'_'.$recordId.'\'));">chat</i></a>

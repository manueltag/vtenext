...
class Homestuff{
	...
	function addStuff(){
		...
		if($this->stufftype=="Module"){
			...
			$querydb="insert into ".$table_prefix."_homedashbd values(?,?,?)";
		...
		// crmv@30014e
	 	...
	 	return "loadAddedDiv($stuffid,'".$this->stufftype."', 1)"; // crmv@30014
	}
	...
	private function getModuleFilters($sid){
		...
		if(isPermitted($modname,'index') == "yes"){
			if($adb->num_rows($cvid_check_query)>0){
				...
				list($order_by,$sort_order) = $oCustomView->getOrderByFilterSQL($cvid);
				$query = $queryGenerator->getQuery();
				if(!empty($order_by) && $order_by != '' && $order_by != null) {
					if($order_by == 'smownerid') $list_query .= ' ORDER BY user_name '.$sort_order;
					else {
						$query .= $focus->getFixedOrderBy($modname,$order_by,$sort_order); //crmv@25403
						if (!in_array($order_by, $newFields)) {
							$newFields[] = $order_by;
						}
					}
				}
	...
	function getDashDetails($did,$chart=''){
		global $adb,$table_prefix;
		$qry="select * from ".$table_prefix."_homedashbd where stuffid=?";
	...
	function getChartDetails($did){
		global $adb, $table_prefix;
		$qry="select hc.chartid,hs.size from {$table_prefix}_homestuff hs inner join {$table_prefix}_homecharts hc on hc.stuffid = hs.stuffid where hc.stuffid=?";
		$result = $adb->pquery($qry, array($did));
		$row = $adb->fetchByAssoc($result, -1, false);
		return array('chartid'=>$row['chartid'], 'size'=>$row['size']);
	}
	// crmv@30014e

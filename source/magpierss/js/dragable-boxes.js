	...
	************************************************************************************************************/		
	...
	var src_rightImage = 'magpierss/images/arrow_right.gif';
	var src_downImage = 'magpierss/images/arrow_down.gif';
	var src_refreshSource = 'magpierss/images/refresh.gif';
	var src_smallRightArrow = 'magpierss/images/small_arrow.gif';
	...
	var nameOfCookie = 'login_vtiger';	// Name of cookie
	...
	*/	
	...
	function createColumns()
	{
		...
		for(var no=0;no<numberOfColumns;no++){
			...
			if(no==(numberOfColumns-1))columnWidth = 99.7 - sumWidth;
	...
	function addRSSEditContent(parentObj)
	{
		...
		var content = '<form><table cellpadding="1" cellspacing="2" class="small"><tr><td>Source:<\/td><td><input type="text" id="rssUrl[' + boxIndex + ']" value="' + dragableBoxesArray[boxIndex]['rssUrl'] + '" size="40" maxlength="255"><\/td><\/tr>'
...
// 			createARSSBox('http://rss.cnn.com/rss/cnn_topstories.rss',2,false,5,1);			
			//createARSSBox('http://rss.slashdot.org/Slashdot/slashdot',1,false,5);			
			//createARSSBox('http://www.repubblica.it/rss/homepage/rss2.0.xml',2,false,5,1);			
			
			
//			createARSSBox('http://www.tagesschau.de/xml/rss2',2,false,20,10);			
// 			createARSSBox('http://web14.difsol.different-solutions.de/index.php?id=137&type=100',1,false,10,10);			
		}
		
		/* Create static boxes */
		
		
		var htmlContentOfNewBox ='<table class="backimg small"><tr align="center" class="hrow"><td>New York</td><td>Berlin</td><td>Hong Kong</td></tr><tr align="center"><td><span>' + worldClock(-5, "NAmerica") + '</span></td><td><span>' + worldClock(1, "Europe") + '</span></td><td><span>' + worldClock(8, "HongKong") + '</span></td></tr></table>';	// HTML content of new box
		var titleOfNewBox = 'World Clock';
		if(!staticObjectArray['staticObject2']){	// The box is not stored in cookie - we need to create it.
			var newIndex = createABox(2,90,false,'staticObject2');
		...
		var htmlContentOfNewBox = '<div style="padding-left:13px;"><a href="plugins/vtigercrm-outlook-plugin-5.0.4.exe">Outlook Plugin<br></a><a href="plugins/vtigercrm-office-plugin-5.0.4.exe">Office Plugin<br></a><a href="plugins/vtigercrm-tbird-plugin-5.0.4.xpi">Thunderbird Plugin<br></a><a href="plugins/vtigercrm-firfox-plugin-5.0.4.xpi">Firefox Plugin<br><br></a></div>';	// HTML content of new box
		var titleOfNewBox = 'vTiger Plugins';
		if(!staticObjectArray['staticObject1']){	// The box is not stored in cookie - we need to create it.
			var newIndex = createABox(1,90,false,'staticObject1');
			document.getElementById('dragableBoxContent' + newIndex).innerHTML = htmlContentOfNewBox;		
			document.getElementById('dragableBoxHeader_txt' + newIndex).innerHTML = titleOfNewBox;		
		}else{	// Box is stored in cookie - all we have to do is to move content into it.

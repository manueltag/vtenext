...
$cloudtag = Array ('SO_vendtl', 'X-CEED', 'X-CEED', 'vte_50usr');
...
for($i = 0; $i < $company_name_count; $i++) {
	...
	if ($i > 3) {
		$freetag = $adb->getUniqueId($table_prefix.'_freetags');
		$query = "insert into ".$table_prefix."_freetags values (?,?,?)";
		$qparams = array($freetag, $cloudtag[1], $cloudtag[1]);
		$res = $adb->pquery($query, $qparams);

		$date = $adb->formatDate(date('Y-m-d H:i:s'), true);
		$query_tag = "insert into ".$table_prefix."_freetagged_objects values (?,?,?,?,?)";
...
$portalname = array ("VTECRM");
$portalurl = array ("http://www.vtecrm.com");
for($i=0;$i<1;$i++)
{

  ...
  function deleteRelation($table_name)
  {
	...
	function getSortOrder($module = '', $useSession = true)	{
		...
		elseif ($_REQUEST['override_orderby'] == 'true')
			...
			$sorder = (($useSession && $_SESSION[$module.'_SORT_ORDER'] != '')?($_SESSION[$module.'_SORT_ORDER']):($use_default_sort_order));
	...
	function getOrderBy($module = '', $useSession = true) {
		...
		$order_by = $use_default_order_by;
			...
			$order_by = (($useSession && $_SESSION[$module.'_ORDER_BY'] != '')?($_SESSION[$module.'_ORDER_BY']):($use_default_order_by));
	...
	*/
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		//crmv@46974 crmv@63349
			...
			$tabname = $this->setupTemporaryRelTable($currentModule,$related_module,$id);
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($numOfFields > 0 || $inventoryModule) {	
			...
			if ($inventoryModule) {
				...
				$query .= " INNER  JOIN $this->table_name $aliasTable ON $aliasTable.$this->table_index = $other->table_name.$dependentColumn";
				//crmv@102767e
			}
			$query .= $more_relation;
			$query .= " LEFT  JOIN ".$table_prefix."_users        ON ".$table_prefix."_users.id = ".$table_prefix."_crmentity.smownerid";
	 ...
	 */
	function generateReportsQuery($module, $reportid = 0, $joinProducts = false) { // crmv@63349
		global $adb,$table_prefix;
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid, $skip_check=false) {
		...
		if (!$skip_check) {
			...
			$result = $adb->pquery("SELECT * FROM {$table_prefix}_relatedlists WHERE (tabid = ? AND related_tabid = ?) OR (tabid = ? AND related_tabid = ?)",array($tabId,$relTabId,$relTabId,$tabId));
...
//			}
			...
			foreach($reference_modules as $reference_module){
				...
				if (is_array($order_by) && !empty($order_by)) {
					foreach ($order_by as $oby) {
						$res = $adb->pquery("SELECT columnname FROM {$table_prefix}_field WHERE tabid = ? AND fieldname = ?", array(getTabId($reference_module), $oby));
						if ($res && $adb->num_rows($res) > 0) {
	...
	function getListQuery($module, $where='') {
		...
		for($i=0; $i<$linkedFieldsCount; $i++) {
			...
			//crmv@26198 crmv@107669: fix for uitype 10, parentid
	...
	function getMessagePopupLinkQuery(&$queryGenerator, $module, $relatedIds = array(), $searchstr = '', $excludeIds = array()) {
		...
		$linkModules = array('Contacts', 'Accounts');
...
//		$queryGenerator->appendToWhereClause(" AND {$table_prefix}_messagesrel.messagehash IS NULL");
	...
	function get_messages_list($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT $tmpSelects
			...
			INNER JOIN {$table_prefix}_messages_account ON {$table_prefix}_messages_account.id = {$table_prefix}_messages.account
			INNER JOIN {$table_prefix}_crmentity ON {$table_prefix}_crmentity.crmid = {$table_prefix}_messages.messagesid
			$joinCondition
			LEFT JOIN {$table_prefix}_users ON {$table_prefix}_users.id = {$table_prefix}_crmentity.smownerid
			LEFT JOIN {$table_prefix}_groups ON {$table_prefix}_groups.groupid = {$table_prefix}_crmentity.smownerid";
		...
		$query .= " AND {$table_prefix}_messages.draft <> 1";	//crmv@84628
	...
	function get_newsletter_emails_count($id, $cur_tab_id, $rel_tab_id) {
		...
		$query = $_SESSION[strtolower($title)."_listquery"];
		if (!empty($query)) {
	...
	function getEntityPreview($id,$module='') {
		...
		$name = array_values(getEntityName($module,$id));
		$name = $name[0];
		$details = array();
		$focus = CRMEntity::getInstance($module);
		...
		$preview = array(
			...
			'name'=>$name,
			'img'=>$img,
			'link'=>"index.php?module=$module&action=DetailView&record=$id",
		);
		if ($focus->retrieve_entity_info($id,$mod, false)) return ''; // crmv@38592
		//crmv@77702
		if ($module == 'Calendar' && !is_admin($current_user) && $focus->column_fields['assigned_user_id'] != $current_user->id && $focus->column_fields['visibility'] == 'Private') {
		...
		if (!empty($fieldnames)) {
			...
			if ($result && $adb->num_rows($result) > 0) {
				...
				while($row=$adb->fetchByAssoc($result)) {
					...
					if ($info[1] != '') {
						//crmv@77702
						if ($module == 'Calendar' && !is_admin($current_user) && $focus->column_fields['assigned_user_id'] != $current_user->id && $focus->column_fields['visibility'] == 'Private' && !in_array($row['fieldname'],array('assigned_user_id','date_start','time_start','time_end','due_date','activitytype','visibility','duration_hours','duration_minutes'))) {
	...
	function setupTemporaryRelTable_notmp($module,$secmodule,$id=0,$relationinfo=Array()) {
		...
		if (empty($relationinfo) || $relationinfo['reltab'] == $table_prefix.'_crmentityrel'){
			if ($id > 0) {
	...
	function getStdDetailTabs() {
		...
		// crmv@107341e
		$tabs = Vtecrm_Panel::getAllForModule($mod);
		if (is_array($tabs)) {
		...
		if (in_array($currentModule, $relatedToProcesses)) {
			...
			if (!empty($ids)) {
				...
				foreach($ids as $id) {
					...
					$selectionProcesses[] = array(getEntityName('Processes',$id,true), $id, $chk_val);
				}
				$focus = CRMEntity::getInstance('Processes');
				if (!empty($processesid)) $focus->retrieve_entity_info($processesid,'Processes');
				$extraBlock .= $focus->getExtraDetailBlock($selectionProcesses);
			}
		}

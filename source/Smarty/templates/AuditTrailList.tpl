...
function auditenabled(ochkbox)
{
	...
	{
		var status='enabled';
		$('audit_info').innerHTML = {/literal}'{$MOD.LBL_AUDIT_TRAIL_ENABLED}'{literal};
		$('audit_info').style.display = 'block';
	}
	else
	{
		var status = 'disabled';
		$('audit_info').innerHTML = {/literal}'{$MOD.LBL_AUDIT_TRAIL_DISABLED}'{literal};
		$('audit_info').style.display = 'block';
	}
...
function showAuditTrail()
{
	...
	openPopup("index.php?module=Settings&action=SettingsAjax&file=ShowAuditTrail&userid="+userid,"","width=645,height=750,resizable=0,scrollbars=1,left=100");//crmv@21048m

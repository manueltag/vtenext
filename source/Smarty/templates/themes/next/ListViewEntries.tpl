...
<form name="massdelete" method="POST" id="massdelete" onsubmit="VtigerJS_DialogBox.block();">
	...
	<table border=0 cellspacing=1 cellpadding=0 width=100% class="lvtBg">
		<tr>
			...
			<table width="100%" style="min-height:50px"> {* crmv@104013 *}
				...
				<tr>
					<td>
						...
						{else}
							{include file="Buttons_List3.tpl" REQUEST_ACTION=$smarty.request.file}	{* crmv@102334 *}
						{/if}
					</td>
		                ...
		                <table border=0 cellspacing=0 cellpadding=0 class="small"><tr>
		                    ...
		                    <td style="padding-left:5px;padding-right:5px">
								<div class="dvtCellInfo">
									<select name="viewname" id="viewname" class="detailedViewTextBox" onchange="showDefaultCustomView(this,'{$MODULE}','{$CATEGORY}', '{$FOLDERID}')">{$CUSTOMVIEW_OPTION}</select>
								</div>
							</td>
							<td>
            ...
            <div id="listViewCont">
            <table id="listView" border=0 cellspacing=1 cellpadding=3 width=100% class="lvt small">
            <!-- Table Headers -->
            <tr>
			...
			{foreach item=entity key=entity_id from=$LISTENTITY}
				...
				<tr bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'" id="row_{$entity_id}">
					...
					{foreach name="entityforeach" key=colname item=data from=$entity}
						{if ($colname neq 'clv_color' and $colname neq 'clv_status') or $colname eq '0'} {* crmv@106058 *}
							<td bgcolor="{$color}"{if $smarty.foreach.entityforeach.index eq 0} nowrap{/if}>{$data}</td>
						{/if}
					{/foreach}
				</tr>
				<!-- crmv@7230e -->
				...
				<tr>
					...
					<td width="33%" align="right">
						...
						<table border=0 cellspacing=0 cellpadding=0 class="small"><tr>
		                    ...
		                    <td style="padding-left:5px;padding-right:5px">
								<div class="dvtCellInfo">
									<SELECT NAME="counts" id="counts" class="detailedViewTextBox" onchange="showMoreEntries(this,'{$MODULE}', '{$FOLDERID}')">{$CUSTOMCOUNTS_OPTION}</SELECT>
								</div>
							</td>
...
{ldelim}
	...
	button_top.innerHTML = "{$APP.LBL_SELECT_ALL_IDS}";
{rdelim}
...
{ldelim}
	...
	if (button_top.innerHTML == "{$APP.LBL_SELECT_ALL_IDS}")
	{ldelim}
		button_top.innerHTML = "{$APP.LBL_UNSELECT_ALL_IDS}";

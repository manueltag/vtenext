...
<div id="{if !empty($UNIFIED_SEARCH_AREAS_ID)}{$UNIFIED_SEARCH_AREAS_ID}{else}UnifiedSearchAreas{/if}" class="{if !empty($UNIFIED_SEARCH_AREAS_CLASS)}{$UNIFIED_SEARCH_AREAS_CLASS}{else}drop_mnu_all{/if}">	{* crmv@59091 *}
	...
	{if !$SKIP_UNIFIED_SEARCH_AREAS}
		<tr id="UnifiedSearchAreasUnifiedRowInput" style="display:none;">
			<td colspan="{$MODS4ROW}" align="center">
				...
				<div class="globalSearch form-group basicSearch">
					<input type="text" id="unifiedsearchnew_query_string" name="query_string" value="" class="form-control fastSearchBox" placeholder="{'LBL_SEARCH_STRING'|getTranslatedString}" onclick="clearText(this,'unified_search_icn_canc')">	{* crmv@31197 *}
					<span class="cancelIcon" style="top:10px; right:5px">
		...
		<tr id="UnifiedSearchAreasUnifiedRow" style="display:none;">
			<td colspan="{$MODS4ROW}">
				...
				<button class="crmbutton" onClick="jQuery('#unifiedsearch_query_string').val(jQuery('#unifiedsearchnew_query_string').val());this.form.submit();" style="width:99%">{$APP.LBL_SEARCH_ALL}</button>
				</form>
			</td>
		</tr>
	{/if}

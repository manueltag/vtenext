	...
	{include file='Buttons_List.tpl'} {* crmv@30683 *} 
			...
			<tr>
				<td width="50%">
					<div class="dvtCellInfo">
						<select name="Screen" class="detailedViewTextBox" onChange="changemodules(this)">
						{foreach key=module item=modulelabel from=$FIELD_INFO}
							{if $module == $DEF_MODULE}
								<option selected value='{$module}'>{$modulelabel}</option>	<!-- crmv@16886 -->
							{else}		
								<option value='{$module}' >{$modulelabel}</option>	<!-- crmv@16886 -->
							{/if}
						{/foreach}
						</select>
					</div>
		...
		{/if}
	 	<table cellspacing=0 cellpadding=5 width=100% class="listTable small">
			...
			<b>{$CMOD.LBL_FIELDS_AVLBL} {$module|@getTranslatedString:$module}</b>	<!-- crmv@16886 -->
			</td>
		</tr>
		<tr>

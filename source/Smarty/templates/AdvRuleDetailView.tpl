					...
					<table border=0 cellspacing=0 cellpadding=5 width=100% class="settingsSelUITopLine">
						<tr>
							<td width=50 rowspan=2 valign=top>
								<img src="{$IMAGE_PATH}ico-adv_rule.gif" alt="{$MOD.LBL_USERS}" width="48" height="48" border=0 title="{$MOD.LBL_USERS}">
							</td>
	...
	jQuery.ajax({
		...
		success: function(response) {
			...
			if (response.indexOf('FAILED') > -1) {
				...
				var offset = jQuery('#tempdiv').outerHeight() - 400;
				if (offset > 0) {
					var top = parseInt(jQuery('#tempdiv').css('top'));					
					jQuery('#tempdiv').css('top', top + offset);
				}
			}
		}
	});

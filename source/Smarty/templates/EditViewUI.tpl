	...
	<table cellspacing="0" cellspacing="0" width="100%">
		...
		<div {if $fld_displayvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} {if $smarty.request.enable_editoptions eq 'yes' && $FIELDTYPE eq 'Other'}style="display:none"{/if} style="position:relative">	{* crmv@92272 *}
			...
			<div class="dvtCellInfoImgRx">
				...
				<i class="vteicon md-link" tabindex="{$vt_tab}" title="{$APP.LBL_SELECT}" onclick='openPopup("index.php?module="+document.{$editViewType}.{$fldname}_type.value+"&action=Popup&html=Popup_picker&form=vtlibPopupView&forfield={$fldname}&srcmodule={$MODULE}&forrecord={$ID}{$SDK->getPopupHiddenElements($MODULE,$fldname)}","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");'>view_list</i>
				{* crmv@37211 *}
				{if $fromlink eq 'qcreate'}
				...
				{else}
					<i class="vteicon md-link" tabindex="{$vt_tab}" title="{$APP.LBL_CLEAR}" onClick="document.EditView.{$fldname}.value=''; document.EditView.{$fldname}_display.value=''; enableReferenceField(document.EditView.{$fldname}_display); return false;">highlight_off</i>
				{/if}
				{* crmv@37211e *}
			...
			<td width="{if $OLD_STYLE eq true}70{else}80{/if}%" style="position:relative">	{* crmv@57221 *} {* crmv@98866 *}
				...
				<div {if $fld_displayvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} id="{$assign_user_div}" style="position:relative; {$style_user}">
					...
					<div class="dvtCellInfoImgRx">
						...
						{if $fromlink eq 'qcreate'}
							<i class="vteicon md-link" tabindex="{$vt_tab}" title="{$APP.LBL_CLEAR}" onClick="document.QcEditView.{$fldname}.value=''; document.QcEditView.{$fldname}_display.value=''; enableReferenceField(document.QcEditView.{$fldname}_display); return false;">highlight_off</i>
						{else}
							<i class="vteicon md-link" tabindex="{$vt_tab}" title="{$APP.LBL_CLEAR}" onClick="document.{$EditViewForm}.{$fldname}.value=''; document.{$EditViewForm}.{$fldname}_display.value=''; enableReferenceField(document.EditView.{$fldname}_display); return false;">highlight_off</i>
						{/if}
						{* crmv@37211e *}
					</div>
				</div>
				...
				{if $smarty.request.enable_editoptions eq 'yes'}
					<div {if $fld_displayvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} id="{$assign_other_div}" style="position:relative; {$style_other}">
						...
						<div class="dvtCellInfoImgRx">
							<i style="display:none" id="advanced_field_assignment_button_{$fldname}" class="vteicon md-link" title="{'LBL_PM_ADVANCED_FIELD_ASSIGNMENT'|getTranslatedString:'Settings'}" onClick="ActionTaskScript.openAdvancedFieldAssignment('{$smarty.request.id}','{$smarty.request.elementid}','{$smarty.request.action_id}','{$fldname}','{$MODULE}','popup',true)">build</i>
						</div>
					</div>
				{/if}
				{* crmv@92272 *}
...
{elseif $uitype eq 85}
	{include file="FieldHeader.tpl" uitype=$uitype mandatory=$keymandatory label=$fldlabel massedit=$MASS_EDIT}
	<div class="{$DIVCLASS}">
		<img src="{$IMAGE_PATH}skype.gif" alt="Skype" title="Skype" LANGUAGE=javascript align="absmiddle"></img><input class="detailedViewTextBox" type="text" tabindex="{$vt_tab}" name="{$fldname}" style="border:1px solid #bababa;" size="27" value="{$fldvalue}">
	</div>
	...
	<div class="{$DIVCLASS}">
		{if $MODULE eq 'Products'}
			...
			<div id="files_list" style="padding: 5px; -moz-background-clip: initial; -moz-background-origin: initial; -moz-background-inline-policy: initial; font-size: x-small">{$APP.Files_Maximum_6}
				...
				{if $maindata[3].0 && $maindata[3].0.name neq '' && $DUPLICATE neq 'true'} {* crmv@65080 *}

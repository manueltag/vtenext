      ...
      <div align=center>
  			{include file='SetMenu.tpl'}
  				...
  				{if $MODE eq "quotes"}
      				...
      				<tr>
      					<td width=50 rowspan=2 valign=top><img src="{$IMAGE_PATH}settingsInvNumber.gif" alt="{$MOD.LBL_CUSTOMIZE_QUOTES_NUMBER}" width="48" height="48" border=0 title="{$MOD.LBL_CUSTOMIZE_QUOTES_NUMBER}"></td>
  				...
  				{else}
      				<table border=0 cellspacing=0 cellpadding=5 width=100% class="settingsSelUITopLine">
      				<tr>
      					<td width=50 rowspan=2 valign=top><img src="{$IMAGE_PATH}settingsInvNumber.gif" alt="{$MOD.LBL_CUSTOMIZE_SALES_ORDER_NUMBER}" width="48" height="48" border=0 title="{$MOD.LBL_CUSTOMIZE_SALES_ORDER_NUMBER}"></td>
         ...
         $("status").style.display="block";
  	     new Ajax.Request(
                  ...
                  {queue: {position: 'end', scope: 'command'},
                          method: 'post',
                          {/literal}
                          postBody: 'module=Users&action=UsersAjax&file=UpdateCustomNo&ajax=true&no='+inv_no+'&str='+inv_str+'&mode=configure&status='+status + '&mod={$MODE}',
                          {literal}

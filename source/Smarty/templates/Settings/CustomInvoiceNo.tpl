...
<!-- quotes -->
		...
		<td>
			...
			<table border=0 cellspacing=0 cellpadding=5 width=100% >
					<tr>
					<td class="small" nowrap align=right><a href="#top">{$MOD.LBL_SCROLL}</a>
					</td>
					</tr>
			</table>
			<!-- end : quotes -->
...
function setquoteid()
{
	...
	new Ajax.Request(
			...
			{queue: {position: 'end', scope: 'command'},
			method: 'post',
			postBody: 'module=Users&action=UsersAjax&file=UpdateCustomQuoteNo&ajax=true&no='+quote_no+'&str='+quote_str+'&mode=configure_quoteno&status='+status,
			onComplete: function(response) {
				if((response.responseText != '')){
				...
				$('view_info_quote').style.display = 'block';		
				$("status").style.display="none";
				}
 			});
...
function setporderid()
{
 ...
 {queue: {position: 'end', scope: 'command'},
 method: 'post',
 postBody: 'module=Users&action=UsersAjax&file=UpdateCustomPorderNo&ajax=true&no='+porder_no+'&str='+porder_str+'&mode=configure_porderno&status='+status,
 onComplete: function(response) {
	 if((response.responseText != '')){
	 ...
	 $('view_info_porder').style.display = 'block';		
	 $("status").style.display="none";
 }
 });
...
function setsorderid()
{
 ...
 {queue: {position: 'end', scope: 'command'},
 method: 'post',
 postBody: 'module=Users&action=UsersAjax&file=UpdateCustomSorderNo&ajax=true&no='+sorder_no+'&str='+sorder_str+'&mode=configure_sorderno&status='+status,
 onComplete: function(response) {
	 if((response.responseText != '')){
	 ...
	 $('view_info_sorder').style.display = 'block';		
	 $("status").style.display="none";
 }
 });
...
function setnoteid()
{
 ...
 {queue: {position: 'end', scope: 'command'},
 method: 'post',
 postBody: 'module=Users&action=UsersAjax&file=UpdateCustomNoteNo&ajax=true&no='+note_no+'&str='+note_str+'&mode=configure_noteno&status='+status,
 onComplete: function(response) {
	 if((response.responseText != '')){
	 ...
	 $('view_info_note').style.display = 'block';		
	 $("status").style.display="none";
 }
 });

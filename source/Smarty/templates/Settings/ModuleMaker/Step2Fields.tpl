{* crmv@64542 crmv@102879 crmv@105127 *}

{if $PROCESSMAKERMODE eq true && $MODE eq 'openimportdynaformblocks'}
{else}
	<table>
		<tr>
			...
			<td align="right" width="7%" nowrap>
				...
				{if $PROCESSMAKERMODE eq true}
					<input type="button" class="crmButton create small" onclick="ProcessHelperScript.openImportDynaformBlocks()" alt="{$MOD.LBL_PM_IMPORT_BLOCK}" title="{$MOD.LBL_PM_IMPORT_BLOCK}" value="{$MOD.LBL_PM_IMPORT_BLOCK}..."/>
				{/if}
			</td>
			<td width="1%">&nbsp;</td>
		</tr>
...
{/if}
	...
	{foreach item=block key=blockno from=$STEPVARS.mmaker_blocks name=outer}
		...
		{if $smarty.foreach.outer.first neq true}
			<tr><td colspan="4"><img src="{'blank.gif'|@vtiger_imageurl:$THEME}" style="width:16px;height:16px;" border="0" />&nbsp;&nbsp;</td></tr>
		{/if}
				...
				<td width="20%" align = "right" class="colData small" >
					...
					{else}
						...
						{if $smarty.foreach.inner.first}
							{if $fieldno % 2 != 0}
								<img src="{'blank.gif'|@vtiger_imageurl:$THEME}" style="{$blankstyle}" border="0" />&nbsp;&nbsp;
							{/if}
							<img src="{'blank.gif'|@vtiger_imageurl:$THEME}" style="{$blankstyle}" border="0" />&nbsp;&nbsp;
		...
		<table border="0" cellpadding="5" cellspacing="1" width="98%" align="center">
			<tr>
				<td width="220" align="right">
					<div name="cfcombo" class="small" style="width:100%;height:200px; overflow-y:auto;overflow-x:hidden;overflow:auto; border:1px solid #CCCCCC; text-align: left;">
						<table border="0">
							{foreach key=nfieldno item=nfield from=$NEWFIELDS}
								<tr><td align="left"><a id="newfield_{$nfieldno}" href="javascript:void(0);" class="newFieldMnu" onclick="ModuleMakerFields.selectNewField('{$nfieldno}', '{","|implode:$nfield.properties}', '{$nfield.uitype}');">
									{if isset($nfield.vteicon)}
										<i class="vteicon valign-bottom" style="width:24px;text-align:center">{$nfield.vteicon}</i> {* crmv@102879 *}
									{elseif isset($nfield.vteicon2)}
										<i class="vteicon2 {$nfield.vteicon2} valign-bottom" style="width:24px;text-align:center" aria-hidden="true"></i> {* crmv@102879 *}
									{/if}
									&nbsp;{$nfield.label}
								</a></td></tr>
		...
		<table border="0" cellpadding="5" cellspacing="1" width="97%" align="center">
			...
			{if $PROCESSMAKERMODE eq true}
				...
				<tr class="fieldprop_default"><td colspan="2">
					<div class="editoptions" id="defaultValueContainer" style="float:right;"></div>
					<div class="dvtCellInfo"><input type="text" id="fieldprop_default" class="detailedViewTextBox" value="" /></div>
				</td></tr>
...
(function() {
	...
	if (!window.Drag) {
		console.log('No Drag library found');
		return;
	}

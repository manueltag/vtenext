		...
		{foreach key=fieldno item=drow from=$DEFAULT_CREATE}
			<tr>
				...
				<td class="listTableRow" align="right">
					<img style="cursor:pointer;" onClick="DataImporterFields.removeDefaultField('create', '{$fieldno}')" src="{'delete.gif'|@vtiger_imageurl:$THEME}" border="0"  alt="{$APP.LBL_DELETE_BUTTON}" title="{$APP.LBL_DELETE_BUTTON}"/>&nbsp;&nbsp;
				</td>
			</tr>
		{/foreach}
	</table>
	...
	<table border="0" width="100%" class="small listTable" id="dimport_table_dfield_u" cellspacing="0" cellpadding="5">
		...
		{foreach key=fieldno item=drow from=$DEFAULT_UPDATE}
			<tr>
				...
				<td class="listTableRow" align="right">
					<img style="cursor:pointer;" onClick="DataImporterFields.removeDefaultField('update', '{$fieldno}')" src="{'delete.gif'|@vtiger_imageurl:$THEME}" border="0"  alt="{$APP.LBL_DELETE_BUTTON}" title="{$APP.LBL_DELETE_BUTTON}"/>&nbsp;&nbsp;
				</td>
			</tr>
		{/foreach}
	</table>

...
{elseif $keyid eq '1' || $keyid eq '7' || $keyid eq '9' || $keyid eq '55' || $keyid eq '71' || $keyid eq '72' || $keyid eq '103' || $keyid eq '255' || $keyid eq 1112} <!--TextBox-->
	...
	<div id="editarea_{$label}" style="display:none;">
		<input class="detailedViewTextBox" type="text" id="txtbox_{$label}" name="{$keyfldname}" maxlength='100' value="{$keyval}"></input>
	</div>
	</div>
	{if $keyid eq '71' && $keyfldname eq 'unit_price' && $PRICE_DETAILS|@count > 0}
...
{elseif $keyid eq '17'} <!--WebSite-->
	...
	<div class="{$DIVCLASS} detailCellInfo" {if $AJAXEDITTABLEPERM}onclick="hndMobileClick(this);" ondblclick="{if !empty($AJAXONCLICKFUNCT)}{$AJAXONCLICKFUNCT}{else}hndMouseClick{/if}({$keyid},'{$label}','{$keyfldname}',this);"{/if}> {* crmv@63001 *}
		...
		<div id="editarea_{$label}" style="display:none;">
			<input class="detailedViewTextBox"  onkeyup="validateUrl('{$keyfldname}');" type="text" id="txtbox_{$label}" name="{$keyfldname}" maxlength='100' value="{$keyval}"></input>
		</div>
	</div>
{elseif $keyid eq '85'}<!--Skype-->
	{include file="FieldHeader.tpl" uitype=$keyid mandatory=$keymandatory label=$label ajaxEditablePerm=$AJAXEDITTABLEPERM}
	<div class="{$DIVCLASS} detailCellInfo" {if $AJAXEDITTABLEPERM}onclick="hndMobileClick(this);" ondblclick="{if !empty($AJAXONCLICKFUNCT)}{$AJAXONCLICKFUNCT}{else}hndMouseClick{/if}({$keyid},'{$label}','{$keyfldname}',this);"{/if}> {* crmv@63001 *}
		<img src="{$IMAGE_PATH}skype.gif" alt="{$APP.LBL_SKYPE}" title="{$APP.LBL_SKYPE}" LANGUAGE=javascript align="absmiddle"></img>
	...
	<div class="{$DIVCLASS} detailCellInfo" {if $AJAXEDITTABLEPERM}onclick="hndMobileClick(this);" ondblclick="{if !empty($AJAXONCLICKFUNCT)}{$AJAXONCLICKFUNCT}{else}hndMouseClick{/if}({$keyid},'{$label}','{$keyfldname}',this);"{/if}> {* crmv@63001 *}
		...
		<div id="editarea_{$label}" style="display:none;">
			...
			<table border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td>
						<input class="form-control" size="11" maxlength="10" type="text" id="txtbox_{$label}" name="{$keyfldname}" maxlength='100' value="{$keyval|regex_replace:'/[^-]*(--)[^-]*$/':''}"></input>
					</td>
					<td>
				...
				<tr>
					<td colspan=2>
						<font size=1><em old="(yyyy-mm-dd)">({php}global $current_user; echo getTranslatedString($current_user->date_format,'Users');{/php})</em></font>
					</td>	
				</tr>										
			</table>
			<script type="text/javascript">
...
{elseif $keyid eq '199'}
	...
	<div class="{$DIVCLASS} detailCellInfo" {if $AJAXEDITTABLEPERM}onclick="hndMobileClick(this);" ondblclick="{if !empty($AJAXONCLICKFUNCT)}{$AJAXONCLICKFUNCT}{else}hndMouseClick{/if}({$keyid},'{$label}','{$keyfldname}',this);"{/if}> {* crmv@63001 *}
		<span id="dtlview_{$label}">{$keyval}</span>
		<div id="editarea_{$label}" style="display:none;">
			<input class="detailedViewTextBox" type="password" id="txtbox_{$label}" name="{$keyfldname}" maxlength='100' value=""></input>
		</div>
	</div>
...
{elseif $keyid eq 1020}	<!-- crmv@18338 -->
	...
	<div class="{$DIVCLASS} detailCellInfo" {if $AJAXEDITTABLEPERM}onclick="hndMobileClick(this);" ondblclick="{if !empty($AJAXONCLICKFUNCT)}{$AJAXONCLICKFUNCT}{else}hndMouseClick{/if}({$keyid},'{$label}','{$keyfldname}',this);"{/if}> {* crmv@63001 *}
		...
		<div id="editarea_{$label}" style="display:none;">
			<input class="detailedViewTextBox"  type="input" id="txtbox_{$label}" name="{$keyfldname}" maxlength='100' value="{$keyval}"></input>
		</div>
	</div>
{elseif !empty($keyid)}
	{include file="FieldHeader.tpl" uitype=$keyid mandatory=$keymandatory label=$label}

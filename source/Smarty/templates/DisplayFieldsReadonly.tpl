...
{elseif $uitype eq 1 || $uitype eq 13 || $uitype eq 7 || $uitype eq 9 || $uitype eq 1112 || $uitype eq 1013}
	...
	<div class="{$DIVCLASS}">
		{if $fldname eq 'tickersymbol' && $MODULE eq 'Accounts'}
			<input type="hidden" name="{$fldname}" tabindex="{$vt_tab}" id ="{$fldname}" value="{$fldvalue}" class="detailedViewTextBox" {if $fldname eq 'tickersymbol' && $MODULE eq 'Accounts'}sensex_info(){/if}">
			{$fldvalue}
		<!--   //ds@26 -->
		{elseif $fldname eq 'if_yes_products' && $MODULE eq 'Visitreport'}
...
{elseif $uitype eq 85}
	{include file="FieldHeader.tpl" uitype=$uitype mandatory=$keymandatory label=$fldlabel}
	<div class="{$DIVCLASS}">
		<img src="{$IMAGE_PATH}skype.gif" alt="Skype" title="Skype" LANGUAGE=javascript align="absmiddle"></img><input type="hidden" tabindex="{$vt_tab}" name="{$fldname}" style="border:1px solid #bababa;" size="27" value="{$fldvalue}">
		{$fldvalue}
	</div>

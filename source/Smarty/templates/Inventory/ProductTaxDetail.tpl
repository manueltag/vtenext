...
{foreach key=tax_row_no item=tax_data from=$data.taxes}
	...
	<tr>
		<td align="left" class="lineOnTop">
			<input type="text" class="small" size="5" name="{$taxname}" id="{$taxname}" value="{$tax_data.percentage|formatUserNumber}" onChange="calcCurrentTax('{$taxname}',{$row_no},{$tax_row_no});calcTotal();">&nbsp;% {*//crmv@43358*}

...
<script type="text/javascript">
        function getUserSelectionBox()
        {
                ...
                var s = '<input type="hidden" id="get_users_list">' +
						...
						'src="{/literal}{$IMAGE_PATH}{literal}select.gif" alt="Select" title="Select" ' +
						'LANGUAGE=javascript ' +
						'onclick=\'openPopup("index.php?"+url)\' ' +	//crmv@29190
						'align="absmiddle" style="cursor:hand;cursor:pointer">&nbsp;' +
						'<input type="image" tabindex="3" ' +
						'src="{/literal}{$IMAGE_PATH}{literal}clear_field.gif" alt="Clear" title="Clear" ' +
...
<tbody><tr>
		...
		<form name="FpofvEditView" method="POST" action="index.php" ENCTYPE="multipart/form-data">
	        ...
	        <tr><td align="left">
	                ...
	                <tr>
                        <td rowspan="2" style="width: 50px;"><img src="{$IMAGE_PATH}workflow.gif" align="absmiddle"></td>
                        <td class="heading2">
                            ...
                            <td align="left" valign="top">
						                	...
						                	<table width="100%" cellspacing="0" cellpadding="5" border="0" class="tableHeading">
												<tr class="tableHeading">
										        	...
										        	<td nowrap align="right">
														<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="small crmbutton save"  name="button" value="{$APP.LBL_SAVE_BUTTON_LABEL}" onclick="this.form.action.value='Save'; if(verify_data_conditionals(FpofvEditView)) FpofvEditView.submit();" style="width: 100px;" type="button" />
											...
											<table cellspacing="0" cellpadding="0" border="0" width="100%" id="proTab">   
						                        {if $MODE eq 'edit'}
					                        		{foreach from=$Rules key=key item=rule}
					                        			<script>
					                        				fnAddProductRow(getObj('moduleName').value,'{$rule.chk_fieldname}','{$rule.chk_criteria_id}','{$rule.chk_field_value}');
					                        			</script>
					                        		{foreachelse}	
					                        			<script>
								...
								<table width="100%" cellspacing="0" cellpadding="5" border="0" class="tableHeading">
									<tr class="tableHeading">
										...
										{/if}
		                            	<input title="{$UMOD.$label}" accesskey="{$UMOD.$label}" class="small crmbutton save"  name="button" value="{$UMOD.$label}"  onclick="if(verify_data_conditionals(FpofvEditView)) {ldelim}load_field_permissions_table();change_label(this,'{$UMOD.LBL_ST_SHOW}','{$UMOD.LBL_ST_RESET}');{rdelim}" style="width: 100px;" type="button" />
			                            </td>
									</tr>
								</table>

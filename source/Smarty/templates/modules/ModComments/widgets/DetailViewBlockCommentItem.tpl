 ...
 * All Rights Reserved.
 ************************************************************************************}
{* crmv@43050 crmv@43448 crmv@59626 crmv@104853 *}
...
<table id="tbl{$UIKEY}_{$COMMENTID}" cellpadding="0" cellspacing="0" width="100%"  class="small" style="font-size:14px">
	<tr>
		<td class="dataImg" style="padding: 5px;">
			...
			{if $SEEN eq 'true'}
				<a href="javascript:;" title="{'LBL_SEEN_ACTION'|getTranslatedString:'Messages'}" onclick="ModCommentsCommon.setAsUnread('{$UIKEY}','{$COMMENTID}',{$INDICATOR});">
					<i class="vteicon md-sm">panorama_fish_eye</i>
				</a>
			{else}
				<a href="javascript:;" title="{'LBL_UNSEEN_ACTION'|getTranslatedString:'Messages'}" onclick="ModCommentsCommon.setAsRead('{$UIKEY}','{$COMMENTID}',{$INDICATOR});">
		...
		<td>
			...
			<div class="dataField{$UNSEEN_CLASS}" style="position: relative;" valign="top" onmouseover="{if $ISMINE}jQuery(this).find('.dataFieldReplyDel').show();{/if}" onmouseout="{if $ISMINE}jQuery(this).find('.dataFieldReplyDel').hide();{/if}" onclick="ModCommentsCommon.checkAndSetAsRead(this, '{$UIKEY}','{$COMMENTID}',{$INDICATOR});">
				{if $COMMENTMODEL->canDeletePost()} {* crmv@101967 *}
					<div class="dataFieldReplyDel" style="display: none; position: absolute; top: 3px; right: 3px;"><a href="javascript:ModCommentsCommon.deleteComment('{$UIKEY}_{$COMMENTID}','{$COMMENTID}',{$INDICATOR});">
						<i class="vteicon md-sm" title="{'LBL_DELETE'|getTranslatedString:$MODULE}">clear</i>
					</a></div>
				{/if}
				...
				{if $SHOWPREVIEW eq true}
					...
					{if $TRUNCATE eq $CONTENT}
						{$CONTENT}
					{else}
						<span id="contentSmall{$UIKEY}_{$COMMENTID}">{$TRUNCATE}</span><span id="contentShowFull{$UIKEY}_{$COMMENTID}">&nbsp;<a href="javascript:ModCommentsCommon.showFullContent('{$UIKEY}_{$COMMENTID}',{$SEEN},'{$UIKEY}','{$COMMENTID}',{$INDICATOR});">{'LBL_SHOW_FULL_COMMENT'|getTranslatedString:'ModComments'}</a></span>
			...
			<div class="dataLabel{$UNSEEN_CLASS}" valign="top" onclick="ModCommentsCommon.checkAndSetAsRead(this, '{$UIKEY}','{$COMMENTID}',{$INDICATOR});">
				<a href="javascript:;" title="{$COMMENTMODEL->timestamp()}" style="color: gray; text-decoration: none;">{$COMMENTMODEL->timestampAgo()}</a>
				{* crmv@98819 *}
					{if $relatedToStr neq ''}
						{'LBL_ABOUT'|getTranslatedString:'ModComments'}&nbsp;<a href="{$relatedToStr.url}" {if $SEEN eq 'false'}onClick="return ModCommentsCommon.setAsRead('{$UIKEY}','{$COMMENTID}',{$INDICATOR});"{/if} title="{$relatedToStr.entityType}" target="_parent">{$relatedToStr.displayValue}</a> ({$relatedToStr.entityType})
						<a href="{$relatedToStr.url}" {if $SEEN eq 'false'}onClick="return ModCommentsCommon.setAsRead('{$UIKEY}','{$COMMENTID}',{$INDICATOR});"{/if} title="{'LBL_DETACH_MESSAGE'|getTranslatedString:'Messages'}" target="_blank">
			...
			<div class="ModCommAnswerBox" valign="top" style="padding: 5px 5px 5px 0px;">
				<table cellpadding="0" cellspacing="0" width="100%">
					<tr>
						<td align="right" width="100%" class="dvtCellInfo" style="background-color: #FFFFFF;">
							<textarea id="txtbox_{$UIKEY}_{$COMMENTMODEL->id()}" class="detailedViewTextBox detailedViewModCommTextBox" style="resize:vertical" onFocus="onModCommTextBoxFocus('txtbox_{$UIKEY}_{$COMMENTMODEL->id()}','{$UIKEY}_{$COMMENTMODEL->id()}','reply');" onBlur="onModCommTextBoxBlur('txtbox_{$UIKEY}_{$COMMENTMODEL->id()}','{$UIKEY}_{$COMMENTMODEL->id()}','reply');" cols="90" rows="8">{$DEFAULT_REPLY_TEXT}</textarea>
						</td>
					</tr>
					<tr>
						<td align="right">
							<div id="saveButtonRow_{$UIKEY}_{$COMMENTID}" style="display: none;">
								...
								{/if}
								<input type="button" class="crmbutton small save" value="{$MOD.LBL_PUBLISH}" onclick="ModCommentsCommon.addReply('{$UIKEY}_{$COMMENTID}', '{$RELATED_TO}', '{$COMMENTID}', {$INDICATOR});"/>
							</div>
						</td>
					</tr>
				</table>

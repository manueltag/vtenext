	...
	{else}
		...
		<div class="{$DIVCLASS}" {if $AJAXEDITTABLEPERM}onclick="hndMobileClick(this);" ondblclick="{if !empty($AJAXONCLICKFUNCT)}{$AJAXONCLICKFUNCT}{else}hndMouseClick{/if}({$keyid},'{$label}','{$keyfldname}',this);"{/if}> {* crmv@63001 *}
			...
			<div id="editarea_{$label}" style="display:none;">
				<input class="detailedViewTextBox" onblur="validateGenericUrl(this, '{$keyfldname}');" onpaste="var me = this; setTimeout(function() {ldelim} validateGenericUrl(me, '{$keyfldname}');{rdelim}, 100); return true;" type="text" id="txtbox_{$label}" name="{$keyfldname}" maxlength='200' value="{$keyval}"></input>
			</div>
		</div>
	{/if}

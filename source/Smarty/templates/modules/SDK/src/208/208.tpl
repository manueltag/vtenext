...
{if $sdk_mode eq 'detail'}
	...
	<div id="uitype208_td_{$keyfldid}">
		...
		{/if}
		<div class="{$DIVCLASS}" name="uitype208_cont_{$keyfldid}" {if $uitype208show eq false}style="display:none"{/if} {if $AJAXEDITTABLEPERM}ondblclick="{if !empty($AJAXONCLICKFUNCT)}{$AJAXONCLICKFUNCT}{else}hndMouseClick{/if}({$keyid},'{$label}','{$keyfldname}',this);"{/if}>
			...
			<div id="editarea_{$label}" style="display:none;">
				...
				{else}
					<input id="txtbox_{$label}" name="{$keyfldname}" class="detailedViewTextBox" onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'" type="text" maxlength='100' value="{$keyval}"></input>
				{/if}
			</div>
		</div>
	</div>

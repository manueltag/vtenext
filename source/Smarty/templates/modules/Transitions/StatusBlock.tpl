    ...
    {foreach from=$PERMITTED_STATUS key=num item=status name=permitted_status_foreach}
    	<tr>
    		<td class='lvtColData' align='left'>
    			...
    			{else}
    				<a href="javascript:void(0);" onClick="query_change_state_motivation('{$status_label}','{$MODULENAME}','{$UITYPE}','vtiger_{$FIELDNAME}','{$FIELDNAME}',{$ACTUAL_ID},'{$status}')">{$status_label}</a>
    			{/if}
    		</td>
    	</tr>
    {/foreach}

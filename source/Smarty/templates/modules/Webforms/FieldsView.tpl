...
<table id="field_table" class="small" border="0" cellpadding="5" cellspacing="1" width="100%">
	<tr height="25px">
		<td style="width:2%;" class="lvtCol"></td>
	...
	<tr style="height:30px" id="field_row">
		...
		{else}
			{if $WEBFORMID}
				{if $WEBFORM->isWebformField($WEBFORMID,$field.name) eq true}
					<input type="checkbox" name="fields[]" id="check_{$field.name}" record="false" checked="checked" value="{$field.name}" onClick=Webforms.showHideElement('value[{$field.name}]','required[{$field.name}]','jscal_trigger_{$field.name}','mincal_{$field.name}','hidden[{$field.name}]')>
				{else}
					<input type="checkbox" name="fields[]" id="check_{$field.name}" record="false" value="{$field.name}" onClick=Webforms.showHideElement('value[{$field.name}]','required[{$field.name}]','jscal_trigger_{$field.name}','mincal_{$field.name}','hidden[{$field.name}]')>
				{/if}
			{else}
				<input type="checkbox" name="fields[]" id="check_{$field.name}" record="false" value="{$field.name}" onClick=Webforms.showHideElement('value[{$field.name}]','required[{$field.name}]','jscal_trigger_{$field.name}','mincal_{$field.name}','hidden[{$field.name}]')>
			{/if}

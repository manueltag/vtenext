...
{if $FOLDER eq $SPECIAL_FOLDERS.Sent or $FOLDER eq $SPECIAL_FOLDERS.Drafts}
	...
	<tr valign="top">
		...
		<td>
			...
			<tr>
				<td>
					{$FOCUS->getAddressName($HEADER_BLOCK.mfrom.value,$HEADER_BLOCK.mfrom_n.value,$HEADER_BLOCK.mfrom_f.value)}
				</td>
			</tr>
			<tr>
				<td colspan="2">
					...
					{foreach name=BUF item=BUC from=$BUSINESS_CARD}
						...
						{/if}
					{foreachelse}
						{$FOCUS->getAddressName($HEADER_BLOCK.mto.value,$HEADER_BLOCK.mto_n.value,$HEADER_BLOCK.mto_f.value)}
					{/foreach}
					{if $HEADER_BLOCK.mcc.value neq ''}
						, <span style="color: gray;">{$MOD.Cc}:</span> {$FOCUS->getAddressName($HEADER_BLOCK.mcc.value,$HEADER_BLOCK.mcc_n.value,$HEADER_BLOCK.mcc_f.value)}
					{/if}
				</td>
			</tr>
			</table>
...
{else}
	...
	<tr valign="top">
		...
		<td>
			...
			<tr>
				<td colspan="2">
					<span style="color: gray;">{$HEADER_BLOCK.mto_n.label}:</span> {$FOCUS->getAddressName($HEADER_BLOCK.mto.value,$HEADER_BLOCK.mto_n.value,$HEADER_BLOCK.mto_f.value)}
					{if $HEADER_BLOCK.mcc.value neq ''}
						, <span style="color: gray;">{$MOD.Cc}:</span> {$FOCUS->getAddressName($HEADER_BLOCK.mcc.value,$HEADER_BLOCK.mcc_n.value,$HEADER_BLOCK.mcc_f.value)}
					{/if}
				</td>

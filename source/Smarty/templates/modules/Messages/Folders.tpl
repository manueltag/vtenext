			...
			<td colspan="2" class="folderMessageRow listMessageFrom"
				{if $entity.selectable eq true}
					{if $VIEW eq 'list'}
						...
						style="cursor: pointer;" onClick="selectFolder('{$DIV_DIMENSION.Folders}','{$DIV_DIMENSION.ListViewContents}','{$entity_id}','{$entity.label}')"
					...
					{elseif $VIEW eq 'create'}
						style="cursor: pointer;" onClick="selectCreateFolder('{$entity_id}');"
					{/if}
				{/if}
			>
					...
					<tr>
						<td width="100%"
							...
							{if $entity_id|in_array:$FOCUS->fakeFolders neq true}
								...
								{/if}
							{/if}
							{if !empty($entity.img)}<img src="{$entity.img}" style="padding-right:10px;" />{/if}{$entity.label}
						</td>
						{if $VIEW eq 'list'}

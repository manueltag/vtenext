...
{* crmv@21048m e*}
{literal}
<form name="EditView" method="POST" ENCTYPE="multipart/form-data" action="index.php" onSubmit="if(fax_validate(this.form,'')) { VtigerJS_DialogBox.block();} else { return false; }">
{/literal}
...
<tbody>
   ...
   <tr>
    ...
    <span class="mailClientCSSButton" >
      <i class="vteicon md-link" title="{$APP.LBL_CLEAR}" onClick="$('parent_id').value=''; $('hidden_toid').value='';$('parent_name').value=''; return false;">highlight_off</i>
    </span>
	</td>
   </tr>
   <tr>
...
function fax_validate(oform,mode)
{
    ...
    //ds@6 send a fax to external receiver
    if(!oform.check_to_fax.checked || oform.to_fax.value.replace(/^\s+/g, '').replace(/\s+$/g, '').length==0)
		{
        ...
        new Ajax.Request(
        	'index.php',
                {queue: {position: 'end', scope: 'command'},
                	method: 'post',
                        postBody:"module=Fax&action=FaxAjax&file=Save&ajax=true&server_check=true",
			onComplete: function(response) {
			//ds@31 bugfix spaces
			var trimed_response = trim(response.responseText);

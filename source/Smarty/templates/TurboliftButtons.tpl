...
{if $MODULE eq 'Messages' || $MODULE eq 'Emails' || $MODULE eq 'MyNotes'}
	...
	{foreach key=entity_id item=info from=$LINKS}
		{if $MODULE neq 'Emails'}
			{assign var=CARDCLICK value="preView('"|cat:$info.module|cat:"','"|cat:$entity_id|cat:"');"}
		{else}
			{assign var=CARDCLICK value=""}
		{/if}
		{include file='Card.tpl' CARDID="preView"|cat:$entity_id CARDMODULE=$info.module CARDMODULE_LBL=$info.modulelbl CARDNAME=$info.name CARDDETAILS=$info.details CARDONCLICK=$CARDCLICK IMG=$info.img}
	{/foreach}
{/if}
	...
	<div id="turbolift_back_button" class="turboliftEntry1 turboliftEntryWithImage btn" onClick="releaseOverAll('detailViewActionsContainer'); DynaFormScript.popup({$RELATED_PROCESS});">
		<div class="row no-gutter">
			<div class="col-sm-12">
				...
				<div class="col-sm-6 vcenter text-right">
					<i class="vteicon md-text icon-module icon-processes" data-first-letter="P"></i>
				</div>
			</div>
		</div>
	</div>

	...
	<tr>
		{if $CALENDAR_DISPLAY eq 'true' && $CHECK.Calendar eq 'yes'}
			{if $CATEGORY eq 'Settings' || $CATEGORY eq 'Tools' || $CATEGORY eq 'Analytics'}
				<td><a href="javascript:;" onClick='fnvshobj(this,"miniCal");getMiniCal("parenttab=My Home Page");'><i class="vteicon {$minIcon}" title="{$APP.LBL_CALENDAR_TITLE}">event</i></a></td>
			{else}
				<td><a href="javascript:;" onClick='fnvshobj(this,"miniCal");getMiniCal("parenttab={$CATEGORY}");'><i class="vteicon {$minIcon}" title="{$APP.LBL_CALENDAR_TITLE}">event</i></a></td>
			{/if}
		{/if}
		...
		{if $CALCULATOR_DISPLAY eq 'true'}
			<td><a href="javascript:;"><i class="vteicon2 fa-calculator" data-toggle="tooltip" data-placement="top" title="{$APP.LBL_CALCULATOR_TITLE}" onClick="fnvshobj(this,'calculator_cont');fetch_calc();"></i></a></td> {* crmv@82419 *}
		{/if}
		<!-- All Menu -->
		...
		{if $MODULE neq 'Messages' && 'Messages'|vtlib_isModuleActive}
			<td>
				<div style="position:relative;">
					<a href="javascript:;">
						<i data-toggle="tooltip" data-placement="top" id="MessagesCheckChangesImg" class="vteicon {$minIcon}" title="{'Messages'|getTranslatedString:'Messages'}" onmouseover="fnDropDown(this,'MessagesNotification_sub',-10);" onmouseout="fnHideDrop('MessagesNotification_sub');">email</i>
					</a>
					<span class="badge vte-top-badge" id="MessagesCheckChangesDivCount" onmouseover="fnDropDown(getObj('MessagesCheckChangesImg'),'MessagesNotification_sub',-10);" onmouseout="fnHideDrop('MessagesNotification_sub');"></span>
				</div>
...
<div id="Buttons_List_Contestual_Container" style="display:none;">
	<table id="Buttons_List_Contestual_Container_Table" border=0 cellspacing=0
			cellpadding={php}echo ($_COOKIE['crmvWinMaxStatus'] == 'close' ? "4" : "6");{/php}
			class="small" style="height: {php}echo ($_COOKIE['crmvWinMaxStatus'] == 'close' ? "38px" : "57px");{/php};">
	<tr>
		{if $MODULE eq 'Home' && $REQUEST_ACTION eq 'index'}
			<td>
				<i data-toggle="tooltip" data-placement="top" class="vteicon {$minIcon}" onClick='fnAddWindow(this,"addWidgetDropDown",-4);' onMouseOut='fnRemoveWindow();' title="{'LBL_HOME_ADDWINDOW'|getTranslatedString:$MODULE}" style="cursor:pointer;">add</i>
			</td>{*crmv@23264*}
		{elseif $CHECK.EditView eq 'yes' || ($MODULE eq 'Projects' && ( $ISPROJECTADMIN eq 'yes' || $ISPROJECTLEADER eq 'yes'))}
			{* crmv@2963m *}
			{if $MODULE eq 'Messages'}
				<td><a href="javascript:;" onclick="OpenCompose('','create');" style="text-decoration:none;"><i class="vteicon {$minIcon}" style="vertical-align:middle">add</i>&nbsp;{'LBL_COMPOSE'|getTranslatedString:'Messages'}</a></td>
		...
		{if $MODULE eq 'Home' && $REQUEST_ACTION eq 'index'}
			<td>
				<i data-toggle="tooltip" data-placement="top" class="vteicon {$minIcon}" onClick='showOptions("changeLayoutDiv");' title="{'LBL_HOME_LAYOUT'|getTranslatedString:$MODULE}">create</i>
			</td>
		...
		{if $MODULE eq 'Calendar' && $REQUEST_ACTION eq 'index'}
			...
			{/if}
			<td><a href='index.php?module=Users&action=EditView&record={$CURRENT_USER_ID}&scroll={$scroll}&return_module=Calendar&return_action=index'>
		...
		{if $CHECK.moduleSettings eq 'yes' && ($IS_ADINN == 1 || $CAN_EDIT_HOMEVIEW == 'yes')}
        	<td id="moduleSettingsTd">
				{if $IS_ADMIN eq 1}
					<i data-toggle="tooltip" data-placement="top" class="vteicon {$minIcon} md-link" title="{"LBL_CONFIGURATION"|getTranslatedString:"Settings"}" onmouseover="fnDropDown(this,'moduleSettings_sub',-10);" onmouseout="fnHideDrop('moduleSettings_sub');">settings_applications</i>
				{else}

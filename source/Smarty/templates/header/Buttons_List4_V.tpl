{*<!-- crmv@18592 -->*}
<table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" class="level3Bg" id="Buttons_List_4">  {* crmv@30356 *}
<tr>
	...
	<td style="padding:5px" nowrap>
		...
		{else}
            ...
            {elseif $OP_MODE eq 'dynaform_popup'}
				<button title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmButton small save" onclick="document.forms['EditView'].action.value='Save';displaydeleted(); SubmitForm(document.forms['EditView'],{$smarty.request.record},'{$smarty.request.module}',true,function(){ldelim}top.location.reload();{rdelim});" type="button" name="button">{$APP.LBL_SAVE_BUTTON_LABEL}</button>
				<button title="{$APP.LBL_CANCEL_BUTTON_TITLE}" accessKey="{$APP.LBL_CANCEL_BUTTON_KEY}" class="crmbutton small cancel" onclick="closePopup()" type="button" name="button">{$APP.LBL_CANCEL_BUTTON_LABEL}</button>
            {/if}
            {* crmv@93990e *}
		{/if}

	...
	<table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			...
			<td align="left" width="100%">
					...
					{/if}
		 		<span class="dvHeaderText"><span class="recordTitle1">{$SINGLE_MOD|@getTranslatedString}:</span> {$REPNAME}</span>
				{* crmv@25620 *}
			...
			{if $EXPORT_PERMITTED eq 'YES'}
				<td align="right"><input class="crmbutton small create" id="btnExport" name="btnExport" value="{$MOD.LBL_EXPORTPDF_BUTTON}" type="button" onclick="Reports.showExportOptions('{$REPORTID}','pdf');" title="{$MOD.LBL_EXPORTPDF_BUTTON}"></td>
				<td align="right"><input class="crmbutton small create" id="btnExport" name="btnExport" value="{$MOD.LBL_EXPORTXL_BUTTON}" type="button" onclick="Reports.showExportOptions('{$REPORTID}','xls');" title="{$MOD.LBL_EXPORTXL_BUTTON}" ></td>
			{* crmv@sdk-25785 *}
 			...
 			{/if}
 			{if $ENABLE_PRINT}
  				<td align="right"><input name="PrintReport" value="{$APP.LNK_PRINT}" onclick="Reports.showExportOptions('{$REPORTID}', 'print');" class="crmbutton small create" type="button"></td>
 			{/if}
			{* crmv@sdk-25785e *}
		</tr>
	</table>
...
{capture assign="FLOAT_CONTENT"}
	...
	<table border="0" cellpadding="5" cellspacing="0" width="250" class="hdrNameBg" id="report_export_list">
		<tr><td>
			...
			{/if}
		</td><td><label for="export_report_summary">{$MOD.LBL_REPORT_SUMMARY}</label></td></tr>
		<tr><td>
			<input checked="" type="checkbox" name="export_report_main" id="export_report_main" value="1" />
		</td><td><label for="export_report_main">{$APP.Report}</label></td></tr>
		<tr><td>
			...
			{/if}
		</td><td><label for="export_report_totals">{$MOD.LBL_REPORT_TOTALS}</label></td></tr>
	</table>
	</form>
{/capture}

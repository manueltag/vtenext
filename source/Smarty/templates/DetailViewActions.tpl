...
function detailviewLinksToggle(title,load){ldelim}
	...
	if(getObj(load).style.display == 'none'){ldelim}
		...
		image_button = 'themes/images/activate.gif';
	{rdelim}else{ldelim}
		label_button = '{$APP.LBL_MORE}';
		image_button = 'themes/images/inactivate.gif';
	{rdelim}
	jQuery('#'+load).toggle();
...
{if $MODULE eq 'Users'}
	{if $IS_ADMIN eq 'true'}
		<div class="turboliftEntry1 btn" onclick="releaseOverAll('detailViewActionsContainer'); showAuditTrail();">
			<div>
	...
	{if $CATEGORY eq 'Settings' && $ID neq 1 && $ID neq $CURRENT_USERID}
		<input title="{$APP.LBL_DELETE_BUTTON_TITLE}" accessKey="{$APP.LBL_DELETE_BUTTON_KEY}" class="crmbutton small delete" style="margin-top:5px;width:100%;text-align:left;" onclick="releaseOverAll('detailViewActionsContainer'); deleteUser({$ID});" type="button" name="Delete" value="{$APP.LBL_DELETE_BUTTON_LABEL}">
	{/if}
{/if}
{* crmv@20054e *}

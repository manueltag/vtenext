...
function QueryLdap(sCommand)
{
	...
	new Ajax.Request(
		...
		{queue: {position: 'end', scope: 'command'},
			method: 'post',
			postBody: 'module=Users&action=UsersAjax&file=QueryLdap&command='+sCommand+'&user='+sUser,
			onComplete: function(response)
			{
				 // alert(response.responseText);
				if (response.responseText.indexOf("Warn=") == 0)
				{
					sError = response.responseText.substring(6);
					alert (sError);
				}
				else if (response.responseText.indexOf("Error=") == 0)
				{
					sError = response.responseText.substring(6);
					alert (sError);
				}
				else if (response.responseText.indexOf("Options=") == 0)
				{
					sOptions = response.responseText.substring(8).split("\n");
				...
				else if (response.responseText.indexOf("Values=") == 0)
				{
					sValues = response.responseText.substring(7).split("\n");
...
<tbody><tr>
	...
	<tr><td align="left">
		...
		<tr>
			<td rowspan="2" width="50"><img src="{$IMAGE_PATH}ico-users.gif" align="absmiddle"></td>	{* crmv@20054 *}
			<td>	
		...
		<tr><td colspan="2">
			...
			<tr>
				...
				<table id="home_page_components" class="tableHeading" border="0" cellpadding="5" cellspacing="0" width="100%"> {* crmv@20054 *}
					<tr>
						<td class="big"><strong>5. {$UMOD.LBL_HOME_PAGE_COMP}</strong>
						</td>
						<td class="small" align="right">&nbsp;</td>
					</tr>
				</table>
				...
				<table class="tableHeading" border="0" cellpadding="5" cellspacing="0" width="100%">
					<tr>
						<td class="big"><strong>6. {$UMOD.LBL_TAGCLOUD_DISPLAY}</strong>
						</td>
						<td class="small" align="right">&nbsp;</td>
					</tr>
				</table>
				...
				<table id="home_page_components" class="tableHeading" border="0" cellpadding="5" cellspacing="0" width="100%">
					<tr>
						<td class="big"><strong>7. {'LBL_NOTIFICATION_MODULE_SETTINGS'|getTranslatedString:'ModNotifications'}</strong></td>
						<td class="small" align="right">&nbsp;</td>

...
<table class="table table-hover table-striped" width="100%" cellspacing="0">
	...
	<tr class="popupLinkListTitleRow">
		...
		{if $SHOW_CHECKBOXES}
			<td class="popupLinkListTitleCell" nowrap="" >&nbsp;</td>
		{/if}
		{foreach name=listrowtitlefor item=headcol from=$LIST_HEADER}
			...
			{/if}
		{/foreach}
		<!-- td class="popupLinkListTitleCell" nowrap="" >{$MOD.LBL_LINK_ACTION}</td-->
	</tr>
	</thead>
	...
	<tr onclick="{$SELECT_FUNC}('{$LISTID}', '{$MODULE}', '{$crmid}', '{$listrow.entityname|addslashes}')" {if $hasextrarow eq true}onmouseover="jQuery(this).next().addClass('hovered')" onmouseout="jQuery(this).next().removeClass('hovered')"{/if}> {* crmv@49389 *}
	{* crmv@66773e *}
		{if $SHOW_CHECKBOXES}
		...
		{foreach name=listrowfor item=listcell key=kk from=$listrow}
			{if $smarty.foreach.listrowfor.iteration <= $LIST_MAXROWFIELDS && $kk|is_numeric}
				<td class="popupLinkListDataCell" style="position:relative;">{$listcell}</td>	{* crmv@59091 *}
			{/if}
		{/foreach}

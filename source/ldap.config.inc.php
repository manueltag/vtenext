...
function get_config_ldap(){
	...
	if ($result){
		...
		$ldap_basedn = $adb->query_result($result,0,'ldap_basedn');
		$server_username = $adb->query_result($result,0,'ldap_username');
		$server_password = $adb->query_result($result,0,'ldap_pass');
		$objclass = $adb->query_result($result,0,'ldap_objclass');
		$account = $adb->query_result($result,0,'ldap_account');
		$fullname = $adb->query_result($result,0,'ldap_fullname');
		$userfilter = $adb->query_result($result,0,'ldap_userfilter');
		$role = $adb->query_result($result,0,'user_role');

...
//		echo $adb->convert2Sql($query,$adb->flatten_array(array_values(Array($this->modulename))));die;
		...
		if ($res){
			...
			if ($field == "" && count($this->all_status_field >=1)){
...
//		    echo $adb->convert2Sql($query,$adb->flatten_array($params));die;
			...
			$state_num = count($ret_val[$status]);
			return $ret_val;
...
//			echo $adb->convert2Sql($query,$adb->flatten_array(array_values($params)));die;
	...
	function getFieldPicklist(){
		...
		WHERE (
		       ...
		       AND ".$table_prefix."_field.uitype = ?
		       AND ".$table_prefix."_def_org_field.visible = 0
		)
...
//		echo $adb->convert2Sql($query,$adb->flatten_array(array_values(array($tabid, $tabid))));die;
		$result = $adb->pquery($query, array($tabid,'15'));
		//crmv@18586e
		if ($result){
	...
	private function updateHandler(){
		...
		$res = $adb->query($sql);
		if ($res && $adb->num_rows($res)>0){
			$em->registerHandler('vtiger.entity.beforesave','modules/Transitions/TransitionHandler.php','TransitionHandler'); 				
			$em->registerHandler('vtiger.entity.aftersave','modules/Transitions/TransitionHandler.php','TransitionHandler'); 				
		}
	}
...
//			echo $adb->convert2Sql($sql,$adb->flatten_array(array_values(Array($this->modulename,$field))));die;
			...
			$instancemodule = Vtiger_Module::getInstance($this->modulename);
			$params = Array(

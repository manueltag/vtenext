...
class MessagesRelationManager extends MessagesUtilities {
		...
		if ($hasrelation) {
			...
			if ($hash) {
				foreach ($with_crmid as $relcrmid) {
					...
					$adb->pquery("UPDATE {$table_prefix}_crmentity SET modifiedtime = ? WHERE crmid IN (?,?)", array($this->db->formatDate(date('Y-m-d H:i:s'), true), intval($crmid), intval($relcrmid))); // crmv@54449	crmv@56655 crmv@69690
				}
			}
		}
	}
	...
	function save_related_module_small($messageid, $with_module, $with_crmid, $subject = null) {
		...
		if ($hash) {
			foreach ($with_crmid as $relcrmid) {
				...
				if ($checkpresence && $adb->num_rows($checkpresence) > 0) continue;
				$adb->pquery("INSERT INTO {$this->relation_table} ({$this->relation_table_id}, crmid, module) VALUES (?,?,?)", array($hash, $relcrmid, $with_module));
				$adb->pquery("UPDATE {$table_prefix}_crmentity SET modifiedtime = ? WHERE crmid IN (?,?)", array($this->db->formatDate(date('Y-m-d H:i:s'), true), $this->id, intval($relcrmid))); // crmv@54449	crmv@56655 crmv@69690	crmv@82688
			}
		}
	}
	// crmv@81338e
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if ($mhash) {
			...
			//crmv@56829e
			$this->db->pquery("UPDATE {$table_prefix}_crmentity SET modifiedtime = ? WHERE crmid IN (?,?)", array($this->db->formatDate(date('Y-m-d H:i:s'), true), intval($id), intval($return_id))); // crmv@49398	crmv@56655 crmv@69690
		}
	}
	...
	function haveRelations($id,$modules='',$skip_modules='',$thread='') {
		...
		if (!empty($thread)) {
			...
			$mhash = getSingleFieldValue($this->table_name, 'messagehash', $this->table_index, $id);
			$query .= " and {$this->relation_table_id} = ?";
			$params = array($mhash);
		}
		if (!empty($modules)) {
			if (!is_array($modules)) {
	...
	function getCrmidFromMessageId($messageid) {
		...
		$res = $adb->query(
			...
			where {$table_prefix}_crmentity.deleted = 0 AND {$this->table_name}.messageid = ?",
			array($messageid)
		);
		if ($res && $adb->num_rows($res) > 0) {
			return $adb->query_result_no_html($res, 0, 'messagesid');
		}
		return null;
	}
	...
	function rebuildAllHashes() {
		...
		if ($adb->isMysql()) {
			...
			$res = $adb->query("update {$this->table_name} inner join {$table_prefix}_crmentity on {$table_prefix}_crmentity.crmid = {$this->table_name}.{$this->table_index} set messagehash = SHA1(CONCAT(messageid, subject)) where {$table_prefix}_crmentity.deleted = 0");
		} else {
			// general query
			$res = $adb->query("select {$this->table_index}, messageid, subject from {$this->table_name} inner join {$table_prefix}_crmentity on {$table_prefix}_crmentity.crmid = {$this->table_name}.{$this->table_index} where {$table_prefix}_crmentity.deleted = 0");
			if ($res) {
	...
	function getExcludedDomainsBySearch() {
		return array(
			'google', 'gmail', 'yahoo', 'hotmail', 'aol', 'msn', 'hotmail', 'aim\.', 'zoho\.com', 'outlook\.com', 'live\.com',
			'alice\.it', 'vodafone\.it', 'tiscali\.it', 'infostrada\.it', 'libero\.it', 'wind\.it', 'tele2\.it', 'fastweb', 'virgilio\.it', 'aruba\.it', 'mail\.it',
			'pec\.it',
		);
	}
	...
	function getPopupLinkModules($from_module, $from_crmid = '', $mode='') { // crmv@43864
		...
		foreach ($mods as $k=>$mod) {
			if (isPermitted($mod, 'EditView') != 'yes') unset($mods[$k]);
		}
	...
	function getMessagesWithSameHash($id) {
		global $adb, $table_prefix;
		$query = "select {$table_prefix}_messages.messagesid

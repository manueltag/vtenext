	...
	function handleEvent($eventName, $data) {
		...
		*/
		...
		if ($data->focus instanceof HelpDesk) {
			...
			if ($eventName == 'vtiger.entity.aftersave' && !in_array($column_fields['assigned_user_id'],$already_notified_users)) {
				if ($mode == 'create') {
					$type = 'Ticket created';
				} else {
					$type = 'Ticket changed';
		...
		*/
		if ($data->focus instanceof Products) {
			...
			if ($eventName == 'vtiger.entity.aftersave' && $data->stock_level_check) {

...
if (isModuleInstalled('HelpDesk')) {
	...
	$adb->pquery("UPDATE {$table_prefix}_links SET linkicon = ?, cond = ? WHERE linktype = ? AND tabid = ? AND linklabel LIKE 'Answer by mail%'",
		array('themes/images/reply_min.png',$cond,'DETAILVIEWBASIC',$HelpDeskInstance->id)
	);
	
	// add links
	Vtiger_Link::addLink($HelpDeskInstance->id, 'DETAILVIEWBASIC', 'Answer by mail', 'javascript:ReplyMailConverter($RECORD$,\'current\');', 'themes/images/reply_min.png', 0, $cond);
	Vtiger_Link::addLink($HelpDeskInstance->id, 'DETAILVIEWBASIC', 'Answer by mail (info)', 'javascript:ReplyMailConverter($RECORD$,\'mailconverter\');', 'themes/images/reply_min.png', 0, $cond);
	Vtiger_Link::addLink($HelpDeskInstance->id, 'HEADERSCRIPT', 'ReplyMailConverter', 'modules/SDK/src/modules/HelpDesk/ReplyMailConverter.js');
...
$trans = array(
	...
	'ALERT_ARR' => array(
		...
		'pt_br' => array(
			'HelpDeskFromMail' => 'Mail Remetente VTE',
		),
	)
);

...
function get_account_name($acc_id)
{
	...
	$acc_qry="select accountname from ".$table_prefix."_account where accountid =?";
...
function module_Chart_HomePageDashboard($userinfo) {
	...
	// Leads module
	$leadcountres = $adb->query("SELECT count(*) as count FROM ".$table_prefix."_crmentity se INNER JOIN ".$table_prefix."_leaddetails le on le.leadid = se.crmid
	...
	// HelpDesk module
	$helpdeskcountres = $adb->query("SELECT count(*) as count FROM ".$table_prefix."_crmentity se INNER JOIN ".$table_prefix."_troubletickets tt ON tt.ticketid = se.crmid
	...
	// Potentials module
	$potcountres = $adb->query("SELECT count(*) as count FROM ".$table_prefix."_crmentity se INNER JOIN ".$table_prefix."_potential pot ON pot.potentialid = se.crmid
	...
	// Calendar moudule
	//crmv@25055
	$calcountres = $adb->query("SELECT count(*) as count FROM ".$table_prefix."_crmentity se INNER JOIN ".$table_prefix."_activity act ON act.activityid = se.crmid
		WHERE se.deleted = 0 AND se.setype = 'Calendar' AND se.smownerid = $user_id AND ((act.status!='Completed' AND act.status!='Deferred') OR act.status IS NULL)
	...
	$query = "SELECT setype, count(setype) setype_count FROM ".$table_prefix."_crmentity se WHERE 
	...
	$cvidres = $adb->query("SELECT cvid,entitytype FROM ".$table_prefix."_customview WHERE viewname='All' AND entitytype in ('".
...
function module_Chart($user_id,$date_start="2000-01-01",$end_date="2017-01-01",$query,$graph_for,$title,$added_qry="",$module="",$graph_type)
{
	...
	global $adb,$current_user,$mod_strings, $default_charset,$table_prefix;
	...
	if($no_of_rows!=0)
	{
		...
		if($mod_by_mod_cnt!=0)
		{
			...
			for ($i=0;$i<count($mod_name_array); $i++)
			{
				...
				{
					$query = "SELECT productname FROM ".$table_prefix."_products WHERE productid=?";
				...
				{
					$query = "SELECT subject FROM ".$table_prefix."_purchaseorder WHERE purchaseorderid=?";
					$result = $adb->pquery($query, array($mod_name));
				...
				{
					$query = "SELECT subject FROM ".$table_prefix."_quotes WHERE quoteid=?";
					$result = $adb->pquery($query, array($mod_name));
					$name_val = $adb->query_result($result,0,"subject");
					$id_name = $mod_name;
					if($name_val!="")
				...
				{
					$query = "SELECT subject FROM ".$table_prefix."_invoice WHERE invoiceid=?";
					$result = $adb->pquery($query, array($mod_name));
					$name_val = $adb->query_result($result,0,"subject");
					$id_name = $mod_name;
					if($name_val!="")
				...
				if($graph_for =="campaignid")
				{
					...
					$query = "SELECT campaignname FROM ".$table_prefix."_campaign WHERE campaignid=?";
					$result = $adb->pquery($query, array($mod_name));
				...
				if($graph_for =="parent_id" || $graph_for =="related_to")
				{
					...
					if($seType == 'Contacts') {
						$query = "SELECT lastname, firstname FROM ".$table_prefix."_contactdetails
							WHERE contactid=?";
						$result = $adb->pquery($query, array($mod_name));
						...
						$query = "SELECT accountname FROM ".$table_prefix."_account WHERE accountid=?";
						$result = $adb->pquery($query, array($mod_name));

	...
	*/
	...
	function set_import_assigned_user()
	{
		...
		if($record_user != $current_user->id){
			$sqlresult = $adb->pquery("select id from ".$table_prefix."_users where id = ?", array($record_user));
	...
	function getDuplicatesQuery($module,$table_cols,$field_values,$ui_type_arr,$select_cols='') {
		...
		$from_clause .= " LEFT JOIN ".$table_prefix."_users ON ".$table_prefix."_users.id = {$table_prefix}_crmentity.smownerid"; // crmv@109663
	...
	function get_service_pricebooks($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $currentModule,$log,$singlepane_view,$mod_strings,$table_prefix;
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		if (isProductModule($module) && $with_module == 'PriceBooks') {
			...
			return parent::save_related_module($module, $crmid, $with_module, $with_crmid);
		}
	}
	// crmv@43864e
	...
	function getPriceBookRelatedServices($query,$focus,$returnset='')
	{
		...
		for($i=0; $i<$numRows; $i++) {
			...
			if(isPermitted("PriceBooks","EditView","") == 'yes')
				$action .= '<img style="cursor:pointer;" src="'.vtiger_imageurl('small_edit.png', $theme).'" border="0" onClick="fnvshobj(this,\'editlistprice\'),editProductListPrice(\''.$entity_id.'\',\''.$pricebook_id.'\',\''.$listprice.'\',\''.$relatedmodule.'\')" alt="'.$app_strings["LBL_EDIT_BUTTON"].'" title="'.$app_strings["LBL_EDIT_BUTTON"].'"/>';
			if(isPermitted("PriceBooks","Delete","") == 'yes')
			{
				if($action != "")
					$action .= '&nbsp;&nbsp;';
				$action .= '<img src="'.vtiger_imageurl('small_delete.png', $theme).'" onclick="if(confirm(\''.$app_strings['ARE_YOU_SURE'].'\')) deletePriceBookProductRel('.$entity_id.','.$pricebook_id.');" alt="'.$app_strings["LBL_DELETE"].'" title="'.$app_strings["LBL_DELETE"].'" style="cursor:pointer;" border="0">';
			}
			if($action != "")
				$entries[] = $action;
	 ...
	 */
	// crmv@63349
	function generateReportsQuery($module, $reportid = 0){
	...
	function generateReportsQuery_tmp($module){ // crmv@63349
		global $current_user,$table_prefix;
			...
			if (!$adb->table_exist($tmptable,true)){
				Vtiger_Utils::CreateTable($tmptable,"serviceid I(11) NOTNULL PRIMARY,\"ACTUAL_UNIT_PRICE\" N(25,2)",true,true);
			}
			else{
	 ...
	 */
	//crmv@38798
	function generateReportsSecQuery($module,$secmodule,$report_type='', $hasInventoryColumns = true) {
		global $current_user,$table_prefix;
		// crmv@29686

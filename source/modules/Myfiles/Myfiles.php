...
class Myfiles extends CRMEntity {
				...
				if($errCode == 0) {
					foreach($_FILES as $fileindex => $files) {
						if($files['name'] != '' && $files['size'] > 0){
							...
							$filename = str_replace(':', '', $filename); // crmv@95157
	...
	*/
	...
	function getSortOrderForFolder($folderId) {
		...
		} elseif(is_array($_SESSION['MYFILES_FOLDER_SORT_ORDER']) &&
					!empty($_SESSION['MYFILES_FOLDER_SORT_ORDER'][$folderId])) {
				$sorder = $_SESSION['MYFILES_FOLDER_SORT_ORDER'][$folderId];
		} else {
	...
	function getOrderByForFolder($folderId) {
		...
		} elseif(is_array($_SESSION['MYFILES_FOLDER_ORDER_BY']) &&
				!empty($_SESSION['MYFILES_FOLDER_ORDER_BY'][$folderId])) {
			$order_by = $_SESSION['MYFILES_FOLDER_ORDER_BY'][$folderId];
		} else {

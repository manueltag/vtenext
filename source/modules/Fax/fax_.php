...
/**   Function used to send fax 
  ...
  *   $from_fax	-- currently loggedin vtiger_users's fax id. you can give as '' if you are not in HelpDesk module
  ...
  *   $faxid		-- id of the fax object which will be used to get the vtiger_attachments
  */
...
/**	Function to set all the Faxer properties
  ...
  *	$faxid	-- id of the fax object which will be used to get the vtiger_attachments - optional
  */
...
//				$fax->addAddress($_tmp[$j]);
	...
	if($attachment == 'current' && $faxid != '')
	{
		...
		addAttachment($fax,$file_name,$faxid);
	}

	//This will add all the vtiger_files which are related to this record or fax
	if($attachment == 'all' && $faxid != '')
	{
		addAllAttachments($fax,$faxid);
	}
...
function setFaxServerProperties($fax)
{
	...
	$res = $adb->pquery("select * from ".$table_prefix."_systems where server_type=?", array('fax'));
...
/**	Function to add the file as attachment with the mail object
  ...
  */
function addAttachment($fax,$filename,$record)
{
	...
	//This is the file which has been selected in Email EditView
        ...
        {
                $fax->AddAttachment($root_directory."test/upload/".$filename);
        }
}

/**     Function to add all the vtiger_files as attachment with the mail object
  *     $fax -- reference of the mail object
  *     $record -- fax id ie., record id which is used to get the all vtiger_attachments from database
  */
function addAllAttachments($fax,$record)
{
...
function FaxSend($fax)
{
	...
	{
		 $log->info("Fax has been sent from the vtigerCRM system : Status : '".$fax->ErrorInfo."'");
		return 1;
	}
}
...
function parseFaxErrorString($fax_error_str)
{
	...
	foreach($fax_status as $key => $val)
	{
		...
		if($status_str[1] != 1 && $status_str[1] != '')
		{
			...
			{
				$adb->println("first elseif part - status will be 0 which is the case of assigned to vtiger_users's fax is empty.");

...
$query='select * from '.$table_prefix.'_troubletickets';
...
for($i=0;$i<count($idlist);$i++)
{
	...
	{
		$resultset=$adb->pquery('select * from '.$table_prefix.'_users where user_name=?', array($_REQUEST['assigned_user_id']));
		$changevalue=$adb->query_result($resultset,0,"id");
		$changekey='assigned_user_id';
	}
	$sql="update ".$table_prefix."_troubletickets set $changekey = ? where id = ?";

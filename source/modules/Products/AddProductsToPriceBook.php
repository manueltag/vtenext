...
{
	$list_query .= ' and '.$table_prefix.'_products.discontinued<>0  ORDER BY '.$order_by.' '.$sorder;
}
...
$sql1 = "select productid from ".$table_prefix."_pricebookproductrel
...
for($i=0; $i<count($new_prod_array); $i++)
{
	...
	if(isPermitted("PriceBooks","EditView","") == 'yes')
		$list_body .= '<input class="dvtCellInfo" type="text" name="'.$field_name.'" style="visibility:hidden;" id="'.$field_name.'">';
	else
		$list_body .= '<input class="dvtCellInfoOff" type="text" name="'.$field_name.'" style="visibility:hidden;" readonly id="'.$field_name.'">';
	$list_body .= '</td></tr>';
}

...
global $vtiger_current_version,$table_prefix;
$res = $adb->pquery("select * from ".$table_prefix."_version", array());
if($adb->num_rows($res))
{
	$res = $adb->pquery("update ".$table_prefix."_version set old_version=?,current_version=?", array($versions[$source_version], $vtiger_current_version));
}
else
{
	$adb->pquery("insert into ".$table_prefix."_version (id, old_version, current_version) values ('', ?, ?);", array($versions[$source_version], $vtiger_current_version));
}
...
$mig_currency = $adb->query_result($adb->pquery("select currency_name from ".$table_prefix."_currency_info", array()),0,'currency_name');

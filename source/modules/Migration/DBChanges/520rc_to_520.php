...
global $adb;
$query=$adb->pquery("select * from ".$table_prefix."_selectcolumn",array());
...
if($numOfRows>0){
	for($i=0;$i<$numOfRows;$i++){
		...
		if(!empty($matches)){
			...
			$query1=$adb->pquery("update ".$table_prefix."_selectcolumn set columnname=? where columnname = ? ",array($columnname1,$columnname));
		}
	}
}
$query2=$adb->pquery("select * from ".$table_prefix."_reportsortcol",array());
...
if($numOfRows>0){
	for($i=0;$i<$numOfRows;$i++){
		...
		if(!empty($matches)){
			$columnname1=str_replace('&amp;', '&', $columnname);
			$query3=$adb->pquery("update ".$table_prefix."_reportsortcol set columnname=? where columnname = ? ",array($columnname1,$columnname));
		}
	}
}
...
function vt520_updateCurrencyInfo() {
	...
	ExecuteQuery("DELETE FROM ".$table_prefix."_currencies;");
	ExecuteQuery('UPDATE '.$table_prefix.'_currencies_seq set id=1;');
...
function VT520GA_webserviceMigrate(){
	...
	ExecuteQuery("delete from ".$table_prefix."_ws_fieldtype where uitype=116;");
	ExecuteQuery("update ".$table_prefix."_field set uitype=117 where tabid=29 and fieldname='currency_id';");
}

VT520GA_webserviceMigrate();

ExecuteQuery("UPDATE ".$table_prefix."_tab SET customized=1 WHERE name='ProjectTeam'");

function VT520GA_picklistMigrate(){
	global $adb,$table_prefix;
	$columnList = $adb->getColumnNames($table_prefix.'_invoicestatus');
	//fix the typo, where invoice is spelled as inovice
	if(in_array('inovicestatusid', $columnList)) {
		$sql = 'alter table '.$table_prefix.'_invoicestatus change inovicestatusid invoicestatusid int(19)';
		ExecuteQuery($sql);
	}
}

...
if ($_REQUEST['morph_mode'] == 'installation' || $_SESSION['morph_mode'] == 'installation') {
	...
	session_start();
	$_SESSION['morph_mode'] = 'installation';
}
$update_mode = false;
						...
						<div class="col-xs-12 content-padding">	
							...
							--><div class="col-xs-4 nopadding vcenter text-right">
								<a href="http://www.vtecrm.com" target="_blank">
									<img src="<?php echo $path; ?>themes/logos/vtecrm.png" />
								</a>
							</div>
						</div>
...
function validateUser() {
	...
	<?php if ($installation_mode) { ?>
		var res = getFile('<?php echo $path; ?>modules/Users/CheckPasswordCriteria?record=&password='+getObj('password_std').value+'&row='+encodeURIComponent(JSON.stringify(checkPasswValues)));
	<?php } else { ?>
...
function sendEmail(params) {
	...
	<?php if ($installation_mode) { ?>
		var url = '<?php echo $path; ?>modules/Morphsuit/SendMailMorphsuit.php';
	<?php } else { ?>
		var url = 'index.php?module=Morphsuit&action=MorphsuitAjax&file=SendMailMorphsuit';
	<?php } ?>
	params['email_std'] = getObj('email_std').value;
	jQuery.ajax({
		url : url,
		type: 'POST',
		data: params,

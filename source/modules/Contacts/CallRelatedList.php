...
if($singlepane_view == 'true' && $_REQUEST['action'] == 'CallRelatedList') {
	...
	if(isset($_REQUEST['record']) && $_REQUEST['record']!='') {
		...
		$sql = "select fieldname from ".$table_prefix."_field where uitype = '13' and tabid = 4 and ".$table_prefix."_field.presence in (0,2)";
		...
		//crmv@7216
		$sql = "select fieldname from ".$table_prefix."_field where uitype = 1013 and tabid = 4";
		$result = $adb->pquery($sql, array());
		$num_fieldnames = $adb->num_rows($result);
		for($i = 0; $i < $num_fieldnames; $i++)
		{
			$fieldname = $adb->query_result($result,$i,"fieldname");
			$permit_fax= getFieldVisibilityPermission("Contacts",$userid,$fieldname);
		}
		//crmv@7216e			
	}

...
$sql_user="SELECT distinct(userid) from ".$table_prefix."_user2mergefields where tabid=?";
...
if($selected_col_string != "") {
	if($user == "true") {
		$sql="UPDATE ".$table_prefix."_user2mergefields SET visible=2 WHERE userid=? and tabid=?";
		$adb->pquery($sql,array($current_user->id,$tabid));
		
		$sql1="UPDATE ".$table_prefix."_user2mergefields SET visible=1 WHERE fieldid IN(". generateQuestionMarks($merge_criteria_cols) .") AND userid=?";
		...
		if ($fld_result != null) {
	    	...
	    	for($i=0; $i<$num_rows; $i++) {
	    		...
	    		$adb->pquery("insert into ".$table_prefix."_user2mergefields values (?,?,?,?)", $params);
			}
			$sql1="UPDATE ".$table_prefix."_user2mergefields SET visible=1 WHERE fieldid IN(". generateQuestionMarks($merge_criteria_cols) .") AND userid=?";
			$adb->pquery($sql1,array($merge_criteria_cols,$current_user->id));
		}

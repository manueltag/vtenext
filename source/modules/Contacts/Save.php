...
if($image_error=="false")
{
	...
	elseif($_REQUEST['portal'] != '' && $_REQUEST['email'] != '')// && $_REQUEST['mode'] != 'edit')
	{
		...
		$sql = "select id,user_name,user_password,isactive from ".$table_prefix."_portalinfo";
		...
		if($insert == 'true')
		{
			...
			$sql = "INSERT INTO {$table_prefix}_portalinfo (id, user_name, user_password, type, last_login_time, login_time, logout_time, isactive) VALUES (?,?,?,?,?,?,?,?)";

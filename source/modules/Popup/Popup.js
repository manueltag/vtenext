...
var LPOP = {
	...
	setGlobalVars: function() {
		var vars = ['userDateFormat', 'SDKValidate', 'alert_arr'];
			...
			jQuery.ajax({
				...
				success: function(data, status, xhr) {
					//if (!data.match(/error/i)) {
						...
						window.top.jQuery('#relation').val(window.top.jQuery('#relation').val()+'|'+crmid);
					...
					if (!data.substr(0,100).match(/error/i)) {
						...
						//crmv@55506e
							...
							if (mode == 'linkdocument') {
								window.top.saveDocument(parentid,jQuery('#contentid').val(),crmid,module);
							} else if (attList.length > 0) {
								for (var i=0; i<attList.length; ++i) {
									window.top.saveDocument(parentid,attList[i],crmid,module);
								}
							}
							//crmv@44775
	...
	create: function(module, callback) {
		var me = LPOP,
			descrCont = jQuery('#linkMsgDescrCont'),
			listCont = jQuery('#linkMsgListCont'),
			editCont = jQuery('#linkMsgEditCont');
	...
	create_cancel: function() {
		...
		if (action == 'create') {
			...
			listCont.height(msgRightTopHeight+'px').show();
		}
	},

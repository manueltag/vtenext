...
function getStdContactFlds(&$queryFields, $adb, $valueArray)
{
  ...
  $query = "SELECT fieldid, columnname, fieldlabel FROM ".$table_prefix."_field WHERE tablename='".$table_prefix."_contactdetails' AND uitype='56' and ".$table_prefix."_field.presence in (0,2)";
...
if ($step == "ask")
{
  ...
  $queryFields = Array();
	// get the Contacts CF fields
 	$cfquery = "SELECT columnname,fieldlabel,uitype FROM ".$table_prefix."_field WHERE tablename='".$table_prefix."_contactscf' and ".$table_prefix."_field.presence in (0,2)";
    ...
    elseif ($cfTmp['uitype'] == 15)
    {
      ...
      $cfValues = "SELECT ".$cfColName.",".$cfColName."id FROM ".$table_prefix."_".$cfColName;
  ...
  if (($export_type == "email") || ($export_type == "emailplus") )
  {
	...
	if(count($fields) > 0)
	 ...
	 $exquery[0] = "SELECT crmentity.crmid, contactdetails.contactid,
	   ...
	   contactdetails.lastname, contactdetails.email  FROM ".$table_prefix."_account
	   INNER JOIN ".$table_prefix."_crmentity crmentity on crmentity.crmid=".$table_prefix."_account.accountid
	   ...
	   INNER JOIN ".$table_prefix."_contactdetails contactdetails ON ".$table_prefix."_account.accountid = contactdetails.accountid
	   INNER JOIN ".$table_prefix."_contactscf contactscf ON contactscf.contactid = contactdetails.contactid
   ...
   if ($export_type == "emailplus")
   {
		  $exquery[1] = "SELECT crmentity.crmid, contactdetails.contactid,
		    contactdetails.salutation, contactdetails.firstname,
		    contactdetails.lastname, ".$table_prefix."_account.email1  FROM ".$table_prefix."_account
		    INNER JOIN ".$table_prefix."_crmentity crmentity ON crmentity.crmid=".$table_prefix."_account.accountid
		    INNER JOIN ".$table_prefix."_accountbillads ON ".$table_prefix."_account.accountid=".$table_prefix."_accountbillads.accountaddressid
		    INNER JOIN ".$table_prefix."_accountshipads ON ".$table_prefix."_account.accountid=".$table_prefix."_accountshipads.accountaddressid
		    INNER JOIN ".$table_prefix."_accountscf ON ".$table_prefix."_account.accountid = ".$table_prefix."_accountscf.accountid
		    ...
		    AND contactdetails.email = '' AND ".$table_prefix."_account.email1 != '' ".$where;
	...
	else if ($export_type == "full")
	{
	  ...
	  $exquery[0] = "select crmentity.crmid, contactdetails.contactid,
	    contactdetails.salutation, contactdetails.firstname,
	    contactdetails.lastname, contactdetails.email, ".$table_prefix."_account.accountname,"
	      ." ".$table_prefix."_account.phone, ".$table_prefix."_account.website, ".$table_prefix."_accountshipads.ship_street,"
	      ." ".$table_prefix."_accountshipads.ship_code,"
	      ." ".$table_prefix."_accountshipads.ship_city, ".$table_prefix."_accountshipads.ship_state,"
	      ." ".$table_prefix."_accountshipads.ship_country,"
	      ." ".$table_prefix."_accountbillads.bill_street, ".$table_prefix."_accountbillads.bill_code,"
	      ." ".$table_prefix."_accountbillads.bill_city, ".$table_prefix."_accountbillads.bill_state,"
	      ." ".$table_prefix."_accountbillads.bill_country"
	      ." FROM ".$table_prefix."_account INNER JOIN ".$table_prefix."_crmentity crmentity ON crmentity.crmid=".$table_prefix."_account.accountid"
  ...
  for ($temp = 0; $temp < count($exquery); $temp++)
  {
	  ...
	  if ($temp == 0)  // We only need the headers for first query
	  {
	    ...
	    for ($arraywalk = 0; $arraywalk < count($fields_array); $arraywalk++)
	    {
	      ...
	      if (strstr ($fields_array[$arraywalk], $table_prefix."_cf_"))
	      {

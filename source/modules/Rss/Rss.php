...
class vtigerRSS extends CRMEntity
{
	...
	function getCRMRssFeeds()
	{
		...
		while($allrssrow = $adb->fetch_array($result))
		{
			...
			{
				$shtml .= "<td width=\"15\">
					<i class=\"vteicon md-sm md-link\" id=\"star-$allrssrow[rssid]\" onClick=\"makedefaultRss($allrssrow[rssid])\">star_border</i></td>";
			}
	...
	function getRssFeedsbyCategory()
	{
		...
		while($allrssrow = $adb->fetch_array($result))
		{
			...
			if($allrssrow["starred"] == 1)
			{
				$shtml .= "<i class=\"vteicon md-sm nohover\" id=\"star-$allrssrow[rssid]\">star</i>";

			}else
			{
				$shtml .= "<i class=\"vteicon md-sm md-link\" id=\"star-$allrssrow[rssid]\" onClick=\"makedefaultRss($allrssrow[rssid])\">star_border</i>";
			}
			$shtml .= "</td>";

...
class Area extends SDKExtendableClass {
	...
	function search($search,$orberby='',$header='',$hide_other_button=false) {
		...
		if (!empty($this->modules)) {
			foreach ($this->modules as $module) {
				...
				$queryGenerator = QueryGenerator::getInstance($module['name'],$current_user);
				...
				if (!empty($search)) {
					...
					$listview_header_search_new = $SearchUtils->getUnifiedSearchFieldInfoList($module['name']);
				...
				if (!empty($orberby)) {
					...
					$customView = new CustomView($module['name']);
					$viewid = $customView->getViewId($module['name']);
					...
					if(!empty($order_by) && $order_by != '' && $order_by != null) {
						if($order_by == 'smownerid') {
							...
							$list_query .= $focus->getFixedOrderBy($module['name'],$order_by,$sorder);
						}
					}
				}
				...
				$listview_entries = $controller->getListViewEntries($focus,$module['name'],$list_result,null,true);
				
				$nameFields = $queryGenerator->getModuleNameFields($module['name']);
				...
				$return[$module['tabid']] = array('header'=>$header,'entries'=>$listview_entries,'name_field_position'=>array_keys($nameFieldListPosition),'show_other_button'=>$show_other_button);
			}
		}
		return $return;
	}
	...
	function setSessionVars() {
		if (!empty($this->modules)) {
			foreach ($this->modules as $module) {
				$_SESSION['areas'][$module['name']] = $this->id;
			}
		}
	}
	...
	function getModuleList($area=false,$module=false) {
		...
		while($info=$adb->fetchByAssoc($res)) {
			...
			if (!file_exists($info['img'])) {
				$info['img'] = "themes/images/modulesimg/Generic.png";
			}

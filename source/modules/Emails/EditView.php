...
if(isset($message) && $message != '') {
	...
	switch ($send_action) {
		case 'forward':
			...
			break;
		case 'reply_all':
			$smarty->assign('CC_MAIL', implode(', ',$recipients['mcc']));	//crmv@44429
		case 'reply':
			$focus->mode = '';
			...
			if (!empty($specialFolders['Sent']) && $focusMessages->column_fields['folder'] == $specialFolders['Sent']) {
				...
				$smarty->assign('OTHER_TO_MAIL', $focusMessages->column_fields['mfrom']);
				...
				if (!empty($cc_tmp)) {
					$smarty->assign('CC_MAIL', trim(implode(', ',$cc_tmp)));
				}
			}
	...
	if (!empty($mailids['mailds'])) {
		...
		if (!empty($relatedPersons)) {
			foreach($relatedPersons as $rp) {
				...
				$rp_instance = Vtiger_Module::getInstance($rp_module);
...
//crmv@2043m
	...
	saveListViewCheck(getSalesEntityType($HelpDeskFocus->column_fields['parent_id']),$HelpDeskFocus->column_fields['parent_id']);
...
if($_REQUEST["internal_mailer"] == "true") {
	...
	if($rec_type == "record_id") {
		...
		elseif ($type == "Vendors")
			...
			$tablename = getTableNameForField($type,$fieldname);
			$tablename_index = $module_focus->tab_name_index[$tablename];
			$q = "select $fieldname from $tablename where $tablename_index = ?";
			//crmv@21474e
			// END
		}
...
$smarty->assign('CUSTOM_LINKS', Vtiger_Link::getAllByType(getTabid('Messages'), Array('DETAILVIEWWIDGET'), $customlink_params));
// END

...
function setMailServerProperties($mail, $from_email)	//crmv@46572
{
	...
	if(isset($_REQUEST['server_password'])) {
		...
		//crmv@43764
		if ($password == '' && $res && $adb->num_rows($res) > 0) {
			$result = $adb->pquery("select server_password from {$table_prefix}_systems where id = ?",array($adb->query_result($res,0,'id')));
	...
	//crmv@32079e
	//crmv@46572
	(isset($_REQUEST['account_smtp'])) ? $account = $_REQUEST['account_smtp'] : $account = $adb->query_result($res,0,'account');	// smtp account type
	//crmv@94605

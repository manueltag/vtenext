...
class PHPMailer
{
	...
	var $AltBoundaries = array(); // crmv@68357
    ...
    */
    ...
    function Send() {
        ...
        switch($this->Mailer)
        {
            ...
            case "mail":
                ...
                break;
            case "smtp":
				$result = $this->SmtpSend($header, $body); // crmv@56409
                break;
            default:
    ...
    function SendmailSend($header, $body) {
		//crmv@securityfocus
		if (!empty($this->Sender)) { // crmv@112560
    ...
    function MailSend($header, $body) {
        ...
        if (!empty($this->Sender) && strlen(ini_get("safe_mode"))< 1 && $this->validateAddress($this->Sender)) // crmv@112560
        {
    ...
    function SmtpConnect() {
            ...
            if(strstr($hosts[$index], ":"))
	    {
            ...
            if($this->smtp->Connect($host, $port, $this->Timeout))
            {
                ...
                {
                	//crmv@32079
                	if ($host == 'smtp.live.com' || $host == 'smtp.mail.me.com') {
                		$this->smtp->StartTLS();
                	}
                	//crmv@32079e
					//crmv@office365
     ...
     * @return string
     */
    function CreateHeader($message_id='') {	//crmv@2963m
        $result = "";
        ...
        //crmv@2963m
        if (empty($message_id)) {
			$this->message_id = sprintf('<%s@%s>', $uniq_id, $this->ServerHostname());
        } else {
        	$this->message_id = $message_id;
        }
        $result .= sprintf("Message-ID: {$this->message_id}%s", $this->LE);
        //crmv@2963me
        $result .= $this->HeaderLine("X-Priority", $this->Priority);
        //crmv@32904
        //$result .= $this->HeaderLine("X-Mailer", "PHPMailer [version " . $this->Version . "]");
        $result .= $this->HeaderLine("X-Mailer", 'VTECRM-WEBMAIL');
        //crmv@32904e
    ...
    function CreateBody() {
        ...
        switch($this->message_type)
        {
            case "alt":
                ...
                $result .= html_entity_decode($this->EncodeString($this->AltBody, $this->Encoding), ENT_COMPAT, $this->CharSet); // crmv@56409
                $result .= $this->LE.$this->LE;
				...
				$result .= $this->AppendAltBoundaries(); //crmv@68357
     ...
     * @access private
     */
    function GetBoundary($boundary, $charSet, $contentType, $encoding, $name='', $method = '') { // crmv@68357
        $result = "";
        ...
        // crmv@68357 - support for name and method
        $ctypes = array('charset="'.$charSet.'"');
        if (!empty($name)) $ctypes[] = 'name="'.$name.'"';
        if (!empty($method)) $ctypes[] = 'method='.strtoupper($method);
        $result .= "Content-Type: {$contentType}; ".implode('; ', $ctypes);
        // crmv@68357e
        $result .= $this->LE;
	...
	// crmv@68357
    public function AddAltBoundary($content, $name = '', $charset = '', $contentType = '', $encoding = '', $method = '') {
		if ($charset == "") { $charset = $this->CharSet; }
        if ($contentType == "") { $contentType = $this->ContentType; }
        if ($encoding == "") { $encoding = $this->Encoding; }

        $cur = count($this->AltBoundaries);
        $this->AltBoundaries[$cur] = array(
			'content' => $content,
			'charset' => $charset,
    ...
    protected function AppendAltBoundaries() {
		$result = '';
		foreach ($this->AltBoundaries as $binfo) {
			$result .= $this->GetBoundary($this->boundary[2], $binfo['charset'], $binfo['contenttype'], $binfo['encoding'], $binfo['name'], $binfo['method']);
			$result .= $this->LE;
			$result .= $this->EncodeString($binfo['content'], $binfo['encoding']);
			$result .= $this->LE.$this->LE;
		}
		return $result;
    }
    ...
    function AttachAll() {
        ...
        for($i = 0; $i < count($this->attachment); $i++)
        {
            ...
            if (empty($encoding)) $encoding = 'base64';	//crmv@63246

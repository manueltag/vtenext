...
foreach($modArr as $fld_module => $fld_label)
{
	...
	for($i=0; $i<$noofrows; $i++)
	{
		...
		if ($check_res && $adb->num_rows($check_res) > 0){
			$update_query = "update ".$table_prefix."_profile2field set visible=?, mandatory=? where fieldid=? and profileid=? and tabid=?";
			$adb->pquery($update_query, array($visible_value, $mandatory_value, $fieldid, $profileid, $tab_id));
		}
		else{
...
// crmv@49398
global $metaLogs;
if ($metaLogs) $metaLogs->log($metaLogs::OPERATION_EDITPROFILE, $profileid);
// crmv@49398e

...
//		$this->loadPreferencesFromDB($row['user_preferences']);
		...
		// crmv@26485
		$_SESSION['__UnifiedSearch_SelectedModules__'] = $this->getSearchModules();
		// crmv@26485e
				...
				*/
				...
				if (!empty($free_login)) {
					if ($free_login == 'CONNECTION_FAILED' && $this->time_to_block_user != 0 && !in_array($last_login,$empty_date_values) && ($now > strtotime($last_login.' + '.$this->time_to_block_user.' months'))) {
						...
						$_SESSION['login_alert'] = getTranslatedString('LBL_OTHER_FREE_VERSION','Morphsuit');
						...
						if (!$this->checkPasswordCriteria($user_password,$row)) {
							$_SESSION['login_confirm'] = array('text'=>sprintf(getTranslatedString('LBL_NOT_SAFETY_PASSWORD','Users'),$this->password_length_min).' '.getTranslatedString('LBL_CLICK_OK_TO_RECOVER','Users'),'action'=>"window.location = 'modules/Users/Recover.php';");
						}
	...
	*/
	...
	function getNonAdminAccessControlQuery($module,$user,$scope=''){
		CRMEntity::getNonAdminAccessControlQuery($module,$user,$scope);
	}
	//crmv@18917e
	...
	function getCookieForSavelogin() {
		...
		if (isset($_SESSION['savelogincookie']) && !empty($_SESSION['savelogincookie'])) return $_SESSION['savelogincookie'];
		$r = getUserAuthtokenKey($token_type, $this->id, $login_expire_time, true);
		$_SESSION['savelogincookie'] = $r;
		return $r;
	}
	...
	//crmv@104988
	function formatUserName($userid, $info, $with_name=false) {
		$full_name = '';
		if ($with_name) {
			$full_name = trim($info['first_name'].' '.$info['last_name']);
		}
		if ($full_name != '')
			$user_name = $info['user_name'].' ('.$full_name.')';
		else

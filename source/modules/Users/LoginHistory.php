...
class LoginHistory extends SDKExtendableUniqueClass {
	...
	public function user_login($usname, $usip = null, $intime = null, $type = 'auto') {
		...
		//crmv@33867
		$params = array($id,$usname,$type,$sessionid,$deviceid,$usip,$this->db->formatDate($intime, true),'Signed in');
		$query = "INSERT INTO {$this->table_name} (login_id,user_name, type, sessionid, deviceid, user_ip, login_time, status) VALUES (".generateQuestionMarks($params).")";
		//crmv@33867e
		$result = $adb->pquery($query, $params);
		
		// save it also in the current session, for faster retrieval
		if (!isset($_SESSION['login_time'])) $_SESSION['login_time'] = $intime;
	...
	public function getLoginTime($username, $type = 'auto') {
		...
		if (!empty($_SESSION['login_time'])) return $_SESSION['login_time']; 
		...
		if (empty($_SESSION['login_time']) && !empty($ltime)) $_SESSION['login_time'] = $ltime;

...
if($enable_ftp_backup == 'true' && is_admin($current_user) == true)
{
	...
	$ftpuser = '';
	$ftppassword = '';
	$query = "select * from ".$table_prefix."_systems where server_type=?";
...
if($enable_local_backup == 'true' && is_admin($current_user) == true)
{
		...
		$path_query = $adb->pquery("SELECT * FROM ".$table_prefix."_systems WHERE server_type = ?",array('local_backup'));
...
// crmv@91082
// clear out the authentication flag

...
function getConvertTTicketToSoObject($focus,$tticket_focus,$tticketid)
{
    ...
    $query ="DELETE FROM ".$table_prefix."_inventoryproductrel where id=$tticketid";
    $adb->query($query);
    // now we insert a new record for each InvoiceLine with a product
    //crmv@fix isnull
    $query ="SELECT product_id,tcunits,".$table_prefix."_crmentity.description
             FROM ".$table_prefix."_timecards
             INNER JOIN ".$table_prefix."_crmentity on crmid=timecardsid
             WHERE ticket_id=$tticketid and timecardtype='InvoiceLine' and product_id is not null and deleted=0";
    //crmv@fix isnull end
    $rs=$adb->query($query);
    for ($r=0;$r<$adb->num_rows($rs);$r) {
        ...
        if (isset($prodid) and is_numeric($prodid)) {
            ...
            $query.=" VALUES($tticketid, ".$prodid.", $nunit, ".getUnitPrice($prodid).','.$InventoryUtils->getProductTaxPercentage('tax1',$prodid,'default').", '".$adb->query_result($rs,$r,2)."')";
            $adb->query($query);
        }
    }

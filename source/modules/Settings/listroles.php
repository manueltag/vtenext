...
function indent($hrarray,$roleout,$role_det) {
	...
	foreach($hrarray as $roleid => $value) {
		...
		if ($roledepth == 0 ) {
			...
			$roleout .='<a href="javascript:;" class="small" onClick="get_parent_ID(this,\'user_'.$roleid.'\')"><i class="vteicon md-sm" title="'.$mod_strings['LBL_MOVE_ROLE'].'">open_with</i></a></div></td></tr></table>';
		}
		$roleout .=  '</li>';

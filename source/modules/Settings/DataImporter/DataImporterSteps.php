...
class DataImporterSteps {
	...
	public function saveStepVars($step, &$request) {
		$_SESSION['dimport']['step_vars'][$step] = $this->extractStepVars($request);
	}
	...
	protected function guessMapField($column, $fields, $content = null, $fuzzyness = 2) {
		...
		foreach ($fields as $finfo) {
			$fnames = array($finfo['fieldname'], $finfo['fieldlabel']);
			foreach ($fnames as $fname) {
				switch ($fuzzyness) {
					case 0:
						if (strcmp($column, $fname) == 0) return $fname;
						break;
					case 1:
						if (strcasecmp($column, $fname) == 0) return $fname;
						break;
					case 2:
						if (strcasecmp($column, $fname) == 0) return $fname;
						$nameFuzzy = preg_replace('/[^a-z0-9]/i', '', $fname);
						if (strcasecmp($columnFuzzy, $nameFuzzy) == 0) return $fname;
						break;
					case 3:
						if (strcasecmp($column, $fname) == 0) return $fname;
						$nameFuzzy = preg_replace('/[^a-z0-9]/i', '', $fname);
						...
						if ($dist >= 0 && $dist <= 1) return $fname;
						$sim = similar_text(strtolower($column), strtolower($fname), $perc);
						if ($sim > 3 && $perc > 70) return $fname;
						break;
					default:
						break;
				}
				if ($fname == 'assigned_user_id' && preg_match('/.*owner.*/', $column)) return $fname;
			}
		}
	...
	public function clearStepVars($step) {
		unset($_SESSION['dimport']['step_vars'][$step]);
	}
	...
	public function clearAllStepsVars() {
		unset($_SESSION['dimport']['step_vars']);
	}
	...
	public function getStepVars($step) {
		...
		if (isset($_SESSION['dimport']['step_vars'][$step])) {
			$vars = $_SESSION['dimport']['step_vars'][$step];
		}
	...
	public function saveAllVarsFromDb($dbdata) {
		$_SESSION['dimport']['step_vars'] = array();

		$_SESSION['dimport']['step_vars'][1]['dimport_module'] = $dbdata['module'];
		$_SESSION['dimport']['step_vars'][1]['dimport_invmodule'] = $dbdata['invmodule'];
		
		$_SESSION['dimport']['step_vars'][2]['dimport_sourcetype'] = $dbdata['srcinfo']['dimport_sourcetype'];
		$_SESSION['dimport']['step_vars'][3] = $dbdata['srcinfo'];
		if ($dbdata['srcinfo']['dimport_sourcetype'] == 'csv') {
			$_SESSION['dimport']['step_vars'][4] = array();
		} else {
			$_SESSION['dimport']['step_vars'][4] = $dbdata['srcinfo'];
		}

		$_SESSION['dimport']['step_vars'][5]['dimport_mapping'] = $dbdata['mapping']['fields'];
		$_SESSION['dimport']['step_vars'][5]['dimport_mapping_keycol'] = $dbdata['mapping']['dimport_mapping_keycol'];
		$_SESSION['dimport']['step_vars'][5]['dimport_deffields'] = $dbdata['mapping']['deffields'];
		
		$_SESSION['dimport']['step_vars'][6] = $dbdata['scheduling'];
		$_SESSION['dimport']['step_vars'][7]['dimport_notifyto'] = $dbdata['notifyto'];
	...
	public function getAllVarsForSave() {
		$out = array();
		if (is_array($_SESSION['dimport']['step_vars'])) {
			$srcinfo = array();
			foreach ($_SESSION['dimport']['step_vars'] as $step=>$svars) {
				if ($step == 1) {

...
switch($page_action){
	case 'index':
		...
		//js stuff
echo <<<EOT
		<link rel="stylesheet" type="text/css" media="screen" href="include/js/jquery_plugins/jqgrid/css/jquery-ui-1.7.2.custom.css" />
		<link rel="stylesheet" type="text/css" media="screen" href="include/js/jquery_plugins/jqgrid/css/ui.jqgrid.css" />
		<script src="include/js/jquery_plugins/jqgrid/i18n/grid.locale-{$lang}.js" type="text/javascript"></script>
		<script language="JavaScript" type="text/javascript" src="include/js/jquery_plugins/jqgrid/jqgrid.js"></script>
		<script language="JavaScript" type="text/javascript" src="include/js/jquery_plugins/jqgrid/jquery-ui-1.7.2.custom.min.js"></script>
		<script>
...
EOT;
	...
	case 'edit':
		...
		switch($_POST['oper']){
			...
			case 'edit':
				SDK::setLanguageEntries($_POST['modulename'],$_POST['label'],$update_languages);
				//controllo se è legato ad un campo o un valore di picklist
				$params_check = Array($_POST['modulename'],$_POST['label']);
...
//			$arr['APP_LIST_STRINGS'] = getTranslatedString('LBL_TRANS_APP_LIST_STRINGS',$currentModule); //tolto perchè immodificabile, sono array in JSON
//			$arr['APP_CURRENCY_STRINGS'] = getTranslatedString('LBL_TRANS_APP_CURRENCY_STRINGS',$currentModule); //tolto perchè sempre vuoto...
...
function construct_table($languages){
	...
	//crmv@100548e
	Vtiger_Utils::CreateTable('sdk_language_translate',implode(",",$fields));
}

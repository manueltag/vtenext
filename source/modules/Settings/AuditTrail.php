...
**/
	...
	function getAuditTrailHeader()
	{
		...
		$table_name = $table_prefix."_audit_trial";
		$list_fields = Array(
					'Module'=>Array($table_prefix.'_audit_trial'=>'module'), 
					'Action'=>Array($table_prefix.'_audit_trial'=>'action'), 
					'Record'=>Array($table_prefix.'_audit_trial'=>'recordid'),
				        'Action Date'=>Array($table_prefix.'_audit_trial'=>'actiondate'), 
		);
...
**/
	function getAuditTrailEntries($userid, $navigation_array, $sorder='', $orderby='')
	{
		...
		if($sorder != '' && $order_by != '')
			$list_query = "Select * from ".$table_prefix."_audit_trial where userid =? order by ".$order_by." ".$sorder; 
		else

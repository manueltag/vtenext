...
class WizardMakerSteps {
	...
	public function saveStepVars($step, &$request) {
		$_SESSION['wmaker']['step_vars'][$step] = $this->extractStepVars($request);
	}
	...
	public function clearStepVars($step) {
		unset($_SESSION['wmaker']['step_vars'][$step]);
	}
	...
	public function clearAllStepsVars() {
		unset($_SESSION['wmaker']['step_vars']);
	}
	...
	public function getStepVars($step) {
		...
		if (isset($_SESSION['wmaker']['step_vars'][$step])) {
			$vars = $_SESSION['wmaker']['step_vars'][$step];
		}
	...
	public function saveAllVarsFromDb($dbdata) {
		...
		if (!empty($fields)) {
			...
			$_SESSION['wmaker']['step_vars'][3]['wmaker_fields'] = $flds;
		}
		...
		if (!empty($relations)) {
			...
			$_SESSION['wmaker']['step_vars'][4]['wmaker_relations'] = $rels;
		}
	...
	public function getAllVarsForSave() {
		$out = array();
		$cfg = array();
		if (is_array($_SESSION['wmaker']['step_vars'])) {
		
			$module = $_SESSION['wmaker']['step_vars'][2]['wmaker_module'];
			
			$cfg['type'] = 'create';
			$cfg['module'] = $module;
			
			foreach ($_SESSION['wmaker']['step_vars'] as $step=>$svars) {
				if ($step == 1) {

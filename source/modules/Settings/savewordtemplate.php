...
global $upload_badext,$adb,$table_prefix;
...
if(move_uploaded_file($_FILES["binFile"]["tmp_name"],$uploaddir.$_FILES["binFile"]["name"])) 
{
  ...
  $file_type_value = strtolower($filetype_array[1]);
    ...
    if($filesize != 0)	
    {
		...
		$genQueryId = $adb->getUniqueID($table_prefix."_wordtemplates");
		if($genQueryId != '') {
			if($result!=false && $savefile=="true") {
				$module = vtlib_purify($_REQUEST['target_module']);
				$sql = "INSERT INTO ".$table_prefix."_wordtemplates ";
				...
				$result = $adb->updateBlob($table_prefix.'_wordtemplates','data',"templateid=$genQueryId",$data);

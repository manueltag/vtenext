...
$PMUtils->unsetReloadAdvancedFieldAssignment();	//crmv@106856
...
while($row=$adb->fetchByASsoc($result,-1,false)) {
	...
	} elseif ($field->getFieldDataType() == 'date' || $field->getFieldDataType() == 'datetime' || (in_array($tabid,array(9,16)) && in_array($row['fieldname'],array('time_start','time_end')))) {
		//crmv@120769
...
$COMMONHDRLINKS = Vtiger_Link::getAllByType($tabid, Array('HEADERSCRIPT'), $hdrcustomlink_params);

 ...
 * All Rights Reserved.
 ***************************************************************************************/
/* crmv@96450 crmv@105685 crmv@105937 crmv@106857 crmv@112297 crmv@115268 */
...
class ProcessDynaForm extends SDKExtendableClass {
	...
	function getStructure($processmaker, $elementid=false, $metaid=false) {
		$PMUtils = ProcessMakerUtils::getInstance();
		$data = $PMUtils->retrieve($processmaker);
		$helper = Zend_Json::decode($data['helper']);
		if (!$elementid) {
			$meta = $this->getMeta($processmaker, $elementid, $metaid);
	...
	function getCurrentDynaForm($focus, &$elementid='') {
		...
		$blocks = array();
		$elementid = '';
		$result = $adb->pquery("select current_dynaform from {$table_prefix}_process_gateway_conn where processesid = ?", array($focus->id));
		...
		if (!empty($elementid)) {
			if (!isset($current_dynaform[$elementid])) {
				$current_dynaform[$elementid] = $this->getStructure($focus->column_fields['processmaker'], $elementid);
			}
			$blocks = $current_dynaform[$elementid];
		}
		return $blocks;
	}
	...
	function getMeta($processmaker, $elementid=false, $metaid=false) {
		global $adb, $table_prefix;
	...
	function getProcessesId($running_process, $metaid) {
		...
		$result = $adb->pquery("SELECT p.processesid FROM {$table_prefix}_processes p
			...
			WHERE c.deleted = ? and p.running_process = ? AND d.metaid = ?", array(0, $running_process, $metaid));
		if ($result && $adb->num_rows($result) > 0) {
	...
	function retrieveDynaform(&$focus,&$request) {
		...
		$blocks = $this->getCurrentDynaForm($focus,$elementid);
		if (!empty($blocks)) {
			$meta = $this->getMeta($focus->column_fields['processmaker'], $elementid);
	...
	function resetDynaForm($metaid, $engine) {
		global $adb, $table_prefix;
		$save = array();
		...
		if (!empty($processesid)) {
			...
			$blocks = $this->getStructure($focus->column_fields['processmaker'], $elementid);
	...
	// crmv@102879
	function categorizeFieldByType(&$return, $field, $groupLabel, $value) {
				...
				if ($h['active'] == 'on' && isset($h['dynaform'])) {
					...
					if (!empty($blocks)) {
						...
						if ($meta) {
							...
							$return['reference'][$groupLabel][$value] = 'ID Processo';
							$return['all'][$groupLabel][$value] = 'ID Processo';
							//crmv@105312e
							foreach($blocks as $block) {
								if (!empty($block['fields'])) {
									foreach($block['fields'] as $field) {
	...
	function getDefaultValues($engine,$blocks) {
		...
		foreach($blocks as $block) {
			foreach($block['fields'] as $field) {
				...
				$form[$field['fieldname']] = $engine->replaceTags($field['fieldname'],$field['default'],$referenceFields,$ownerFields);
			}
		}
		return $form;
	}
	...
	function addBlockInformation($col_fields, $mode, &$return_fields, &$blockdata, &$aBlockStatus, $dynaFormValues=array(), $conditionalPermissions=false) { // crmv@104568
		...
		$blocks = $this->getCurrentDynaForm($focus,$elementid);
		if (!empty($blocks)) {
			$meta = $this->getMeta($col_fields['processmaker'], $elementid);
			...
			foreach($blocks as $blockid => $block) {
				if (!empty($block['fields'])) {
					...
					foreach($block['fields'] as $field) {
						...
						if ($mode == 'detail') {
							$custfld = getDetailViewOutputHtml($field['uitype'], $field['fieldname'], $field['label'], $col_fields, 1, '', $currentModule, $field);
							if (is_array($custfld)) {
								$fieldtablename = '';
								$fieldname = $field['fieldname'];
							...
							$fld = getOutputHtml($field['uitype'], $field['fieldname'], $field['label'], $field['length'], $col_fields, 1, $currentModule, $mode, $readonly, $typeofdata, $field);
							$fld[] = $field['fieldname'];
	...
	function existsConditionalPermissions($focus) {
		...
		$PMUtils = ProcessMakerUtils::getInstance();
		$data = $PMUtils->retrieve($focus->column_fields['processmaker']);
	...
	function existsConditionalFpovValueActive($focus) {
		$elementid = '';
		$fields = $this->getFields($focus, $elementid);
...
//			}
	...
	function replaceTags($value, $dynaform) {
		$PMUtils = ProcessMakerUtils::getInstance();
		return $PMUtils->replaceTags($value, $dynaform, '/(\$vcf_([0-9:]+))/');
	}
						...
						foreach($value as $row => $tab_fields) {
							foreach($tab_fields as $tab_fieldname => $tab_value) {
								if (isset($permissions[$fieldname.'_'.$tab_fieldname.'_'.$row]['value'])) {
									$dynaform[$fieldname][$row][$tab_fieldname] = $this->replaceTags($permissions[$fieldname.'_'.$tab_fieldname.'_'.$row]['value'],$dynaform_tmp);
								}
							}
						}
					}
	...
	function propagateParallelsChangeLog($processesid,$changeLogFocus) {
		...
		if ($result && $adb->num_rows($result) > 0) {
			...
			$changeLogFocus->save('ChangeLog',false,false,false);
		}
	}
	//crmv@103534e
}
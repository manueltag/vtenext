...
class ModuleMakerSteps {
	...
	public function saveStepVars($step, &$request) {
		$_SESSION['mmaker']['step_vars'][$step] = $this->extractStepVars($request);
	}
	...
	public function clearStepVars($step) {
		unset($_SESSION['mmaker']['step_vars'][$step]);
	}
	...
	public function clearAllStepsVars() {
		unset($_SESSION['mmaker']['step_vars']);
	}
	...
	public function getStepVars($step) {
		...
		if (isset($_SESSION['mmaker']['step_vars'][$step])) {
			$vars = $_SESSION['mmaker']['step_vars'][$step];
		}
	...
	public function processStepVars($mode, $step, $prevstep, &$stepVars) {
		...
		if ($mode == 'create' && $step == 2) {
			...
			$stepVars['mmaker_enable_import'] = 1;
			$stepVars['mmaker_enable_export'] = 1;
			$stepVars['mmaker_enable_quickcreate'] = 1;
	...
	public function saveAllVarsFromDb($dbdata) {
		$_SESSION['mmaker']['step_vars'] = array();
		
		$step1 = $dbdata['moduleinfo'];
		$step1['mmaker_modname'] = $dbdata['modulename'];
		$_SESSION['mmaker']['step_vars'][1] = $step1;
		...
		$_SESSION['mmaker']['step_vars'][2]['mmaker_blocks'] = $dbdata['fields'];
		$_SESSION['mmaker']['step_vars'][2]['mmaker_panels'] = $dbdata['panels'];
		$_SESSION['mmaker']['step_vars'][2]['mmaker_currentpanelno'] = 0;
		$_SESSION['mmaker']['step_vars'][2]['mmaker_lastfieldid'] = $this->calculateLastFieldId($dbdata['fields']);
		$_SESSION['mmaker']['step_vars'][3]['mmaker_filters'] = $dbdata['filters'];
		$_SESSION['mmaker']['step_vars'][4]['mmaker_relations'] = $dbdata['relations'];
		$_SESSION['mmaker']['step_vars'][5]['mmaker_labels'] = $dbdata['labels'];
		
		$_SESSION['mmaker']['step_vars'][6]['mmaker_enable_quickcreate'] = $dbdata['moduleinfo']['enable_quickcreate'];
		$_SESSION['mmaker']['step_vars'][6]['mmaker_enable_import'] = $dbdata['moduleinfo']['enable_import'];
		$_SESSION['mmaker']['step_vars'][6]['mmaker_enable_export'] = $dbdata['moduleinfo']['enable_export'];
		$_SESSION['mmaker']['step_vars'][6]['mmaker_enable_dupcheck'] = $dbdata['moduleinfo']['enable_dupcheck'];
		$_SESSION['mmaker']['step_vars'][6]['mmaker_sharing_access'] = $dbdata['moduleinfo']['sharing_access'] ?: 'Private';
	...
	public function getAllVarsForSave() {
		$out = array();
		if (is_array($_SESSION['mmaker']['step_vars'])) {
			foreach ($_SESSION['mmaker']['step_vars'] as $step=>$svars) {
				if ($step == 1) {
	...
	public function getNewFieldDefinition($fieldno, $properties, $fieldid = 0, $tableField = false) {
		...
		$allFieldProps = array('length', 'decimals', 'picklistvalues', 'relatedmods', 'autoprefix', 'onclick', 'code', 'users', 'newline');	//crmv@98570 crmv@101683
	...
	public function getRelationModules() {
		...
		$skipMods = array('PBXManager', 'Messages', 'ChangeLog', 'ModComments', 'Charts', 'ModNotifications', 'Emails', 'MyNotes', 'Sms', 'Fax', 'Calendar');
	...
	public function getNewFieldNoByUitype($uitype) {
		static $fields = array();

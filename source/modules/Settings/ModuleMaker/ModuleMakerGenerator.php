...
class ModuleMakerGenerator {
	...
	function copyBaseFiles() {
		...
		$r = $this->rcopy($this->base_module_dir, $this->moduleDir);
		if (!$r) return false;
	...
	function calculateTableStructure($skip_check_table=false) {
		...
		$crmentityFields = array('creator', 'assigned_user_id', 'description', 'modifiedtime', 'createdtime');
		...
		if (!$skip_check_table) {
			$i = 1;
			while (Vtiger_Utils::CheckTable($propTable) && $i <= 5) {

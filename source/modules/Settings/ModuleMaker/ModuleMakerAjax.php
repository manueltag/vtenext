...
$fieldops = array(
	...
	'addblock', 'delblock', 'moveblock', 'moveblockpanel',
...
if (in_array($action, $fieldops)) {
	...
	if ($step == 2) {
		...
		if ($processMakerMode) {
			...
			$smarty->assign("USETABLEFIELDS", $PMUtils->todoFunctions);
		}
		// crmv@104180e
	}
...
class ModuleMakerAjax {
	...
	public function addField(&$vars, $blockno, $addfieldno, $properties) {
	...
	public function getLabelsList(&$vars, $opts) {
		...
		if (!empty($sortid)) {
			$sign = ($sortord == 'asc' ? '+' : '-');
			usort($labels, create_function('$a, $b', 'return '.$sign.'strcmp($a["'.$sortid.'"], $b["'.$sortid.'"]);'));
		}

...
class StorageBackendUtils extends SDKExtendableUniqueClass {
	...
	public function uploadRevision($backend, $module, &$focus = null, $userEmail = null) {
		...
		if ($doc_precedente > 0){
			...
			$params = Array($record, $doc_precedente, $revision,$current_user->id,$now,$userEmail);
	 ...
	 */
	public function downloadFile($module, $crmid, $attid) {
		global $adb, $table_prefix;
		...
		if ($adb->num_rows($result) == 1) {
			...
			$bclass->retrieveFile($attid, $ekey);
	...
	public function incrementDownloadCount($module, $crmid, $attid = null) {
		...
		if ($backend && $key) {
			$bclass = $this->getBackendClass($backend);
			$bclass->incrementDownloadCount(key);
		}
		
		// now increment it locally
		if ($module == 'Documents') {
			$sql = "UPDATE {$table_prefix}_notes SET filedownloadcount = filedownloadcount+1 WHERE notesid= ?";
			$res = $adb->pquery($sql,array($crmid));
		} elseif ($module == 'Myfiles') {
			$sql = "UPDATE {$table_prefix}_myfiles SET filedownloadcount = filedownloadcount+1 WHERE myfilesid= ?";
			$res = $adb->pquery($sql,array($crmid));
		}

...
/**   Function used to send sms 
  ...
  *   $from_sms	-- currently loggedin vtiger_users's sms id. you can give as '' if you are not in HelpDesk module
  ...
  *   $smsid		-- id of the sms object which will be used to get the vtiger_attachments
  */
...
/**	Function to set all the Smser properties
  ...
  *	$smsid	-- id of the sms object which will be used to get the vtiger_attachments - optional
  */
...
function setSmsServerProperties($sms)
{
	...
	$res = $adb->pquery("select * from ".$table_prefix."_systems where server_type=?", array('sms'));
	...
	if(isset($_REQUEST['server_password']))
		...
		$sms_server_type = $adb->query_result($res,0,'service_type');
...
function SmsSend($sms)
{
	...
	{
		 $log->info("Sms has been sent from the vtigerCRM system : Status : '".$sms->ErrorInfo."'");
		return 1;
	}
}
...
function parseSmsErrorString($sms_error_str)
{
	...
	foreach($sms_status as $key => $val)
	{
		...
		if($status_str[1] != 1 && $status_str[1] != '')
		{
			...
			{
				$adb->println("first elseif part - status will be 0 which is the case of assigned to vtiger_users's sms is empty.");

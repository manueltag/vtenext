...
function getNewLeads($maxval,$calCnt)
{
	...
	$list_query = 'select '.$table_prefix.'_leaddetails.firstname, '.$table_prefix.'_leaddetails.lastname, '.$table_prefix.'_leaddetails.leadid, '.$table_prefix.'_leaddetails.company 
	...
	$params = Array("Lost Lead","Junk Lead",$current_module_strings['Lost Lead'],$current_module_strings['Junk Lead']);
	$params[] = $start_date;
	$params[] = $current_user->id;
	if($calCnt == 'calculateCnt') {
		$list_result_rows = $adb->pquery(mkCountQuery($list_query),$params);
		return $adb->query_result($list_result_rows, 0, 'count');
	}
	$list_result = $adb->limitpQuery($list_query,0,$adb->sql_escape_string($maxval),$params);	
	...
	$search_qry = "&query=true&Fields0=".$table_prefix."_leaddetails.leadstatus&Condition0=isn&Srch_value0=Lost+Lead&Fields1=".$table_prefix."_leaddetails.leadstatus&Condition1=isn&Srch_value1=Junk+Lead&Fields2=".$table_prefix."_crmentity.smownerid&Condition2=is&Srch_value2=".$current_user->column_fields['user_name']."&Fields3=".$table_prefix."_crmentity.createdtime&Condition3=grteq&Srch_value3=".$start_date."&searchtype=advance&search_cnt=4&matchtype=all";

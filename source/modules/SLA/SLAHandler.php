...
class SLAHandler extends VTEventHandler {
	function handleEvent($eventName, $entityData) {
		...
		if($eventName == 'vtiger.entity.beforesave.modifiable') {
			...
			if(!$entityData->isNew()) { //entità modificata
		...
		if($eventName == 'vtiger.entity.aftersave.last') {
...
class CalcTime {
	...
	function __construct($config,&$entityData,$now){
		...
		$this->entityData->trans_obj->Initialize('HelpDesk');		
		$this->config = $config;
	...
	function convert_to_seconds($value){
		switch ($this->config[time_measure]){
			case 'minutes':
				$value = $value*60;
				break;
			case 'hours':
	...
	function process($mode,$savepoint){
		...
		switch($mode){
			case 'create':
				$this->time_refresh = $this->now_time;
				//Il now_time è sempre tipo timestamp
				$this->start_sla = $this->now_time;
				...
				if (in_array($this->entityData->newStatus_SLA,$this->config['status_close_value']) && ($this->close_time || $this->config['auto_set_closing_datetime'])){
					// se lo stato è chiuso e ho settato data e ora chiusura devo calcolare lo sla fino all'ora indicata						
					$end_sla = true;	
				}
		...
		if ($calculate){
			if ((($changed_status || $reset_sla) && $savepoint == 'aftersave') || $savepoint == 'beforesave'){
				...
				elseif ($changed_status){
					$this->reset(true);
					//se lo sla è cambiato me lo resetta...patch
					if ($this->entityData->oldSLA != $this->entityData->newSLA){
						$this->data['sla_time'] = $this->entityData->newSLA;
						$this->sla_time = $this->convert_to_seconds($this->entityData->newSLA);
					}
...
//		if ($this->count_window[$dayofweek] < 2 ) return false; //torno false, poichè faccio già i controlli sui tempi massimo/minimo, quindi salto
	...
	function set_sec_elapsed(){
		...
		$sec_elapsed = 0;
		$exit_while =false;
		//loop di conteggio finchè le ore rimanenti sono finite
		$cnt = 0;
...
//		$this->debug_print("refresh time = ".date("Y-m-d H:i:s", $this->time_refresh));
		...
		while ($time_start <= $time_now){
			...
			$this->debug_print("ciclo $cc e seconds elapsed ".$sec_elapsed);
				...
				//se la data di fine conteggio è vacanza allora riporto indietro di un giorno e la la uniformo al limite massimo giornaliero
				if ((in_array($dayofweek1,$this->jump_days) || in_array(date("d-m",$time_now),$this->holidays)) && !in_array(date("d-m",$time_now),$this->force_days_keys)) {
					...
					$min1 = 59;
					$sec1 = 59;
					//ricalcolo la data attuale (che verrà controllata nel while principale)
					$time_now = mktime($hour1,$min1,$sec1,$month1,$day1,$year1); //ora minuto secondo mese giorno anno
					//ricomincio il ciclo
					$this->debug_print("sposto time now a ".date("d-m-Y H:i",$time_now)." causa vacanza");		
					continue;
				}
				//se la data di fine conteggio è > del limite massimo giornaliero, la uniformo al limite massimo giornaliero
				...
				//se la data di fine conteggio è minore del limite minimo giornaliero,la la uniformo al limite massimo del giorno prima
				...
				//se la data di fine conteggio è al di fuori delle finestre temporali, la uniformo al limite massimo/minimo della finestra temporale precendente / successiva
...
//				$this->debug_print("1uniformo data attuale da ".date("d-m-Y H:i",$time_start_backup)." a ".date("d-m-Y H:i",$time_start));		
				...
				//se la data di inizio conteggio è > del limite massimo giornaliero, la sposto al minimo giornaliero del giorno dopo
				if ($hour > $this->hour_w_end[$dayofweek] || ($hour == $this->hour_w_end[$dayofweek] && $min > $this->min_w_end[$dayofweek])) {
					$day++;
					$hour = 0;
					$min = 0;
					$sec = 0;
					//ricalcolo la data di partenza (che verrà controllata nel while principale)
					$time_start = mktime($hour,$min,$sec,$month,$day,$year); //ora minuto secondo mese giorno anno
					//ricomincio il ciclo
					$this->debug_print("sposto time start a ".date("d-m-Y H:i",$time_start)." causa > limite massimo giornaliero");
					continue;
				}
				//se la data di inizio conteggio è vacanza allora vado avanti di un giorno e la la uniformo al limite minimo giornaliero
				...
				//se la data di inizio conteggio è minore del limite minimo giornaliero,la la uniformo al limite minimo dello stesso giorno
				...
				//se la data di inizio conteggio è al di fuori delle finestre temporali, la uniformo al limite massimo/minimo della finestra temporale precendente / successiva
...
//				$this->debug_print("2uniformo data attuale da ".date("d-m-Y H:i",$time_start_backup)." a ".date("d-m-Y H:i",$time_start));	
				...
				$window=$this->is_in_a_window($hour,$min,$sec,$month,$day,$year,$dayofweek);
				if ($window){ //se sto in una finestra temporale, $window sarà uguale alla chiave dell'array di configurazione
...
//				$entity->set('time_elapsed', $this->time_elapsed+$sec_elapsed);
	...
	function set_fine_sla_date(){
		...
		//aggiungo anche il tempo fuori sla così da spostare la fine sla
		...
		while ($seconds_remaining > 0){
			...
			//se la data attuale è > del limite massimo giornaliero, la sposto al minimo giornaliero del giorno dopo
			if ($hour > $this->hour_w_end[$dayofweek] || ($hour == $this->hour_w_end[$dayofweek] && $min > $this->min_w_end[$dayofweek])) {
				$day++;
				$hour = 0;
				$min = 0;
			...
			//se la data attuale è vacanza allora vado avanti di un giorno e la la uniformo al limite minimo giornaliero
			...
			//se la data attuale è minore del limite minimo giornaliero,la la uniformo al limite minimo dello stesso giorno
			if ($hour < $this->hour_w_start[$dayofweek] || ($hour == $this->hour_w_start[$dayofweek] && $min < $this->min_w_start[$dayofweek])){		
				$hour = $this->hour_w_start[$dayofweek];
				$min = $this->min_w_start[$dayofweek];
				$sec = 0;
			...
			//se la data attuale è al di fuori delle finestre temporali, la uniformo al limite massimo/minimo della finestra temporale precendente / successiva

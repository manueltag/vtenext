...
if(!empty($modval)){
	...
	if($adb->num_rows($result)==0){
	  ...
	  die;
		...
		$html = '<select id=selFilterid name=selFiltername onchange=setPrimaryFld(this) class="detailedViewTextBox" onfocus="this.className=\'detailedViewTextBoxOn\'" onblur="this.className=\'detailedViewTextBox\'">';
...
if(!empty($dash)){
	...
	if (is_array($sdk_dashes) && count($sdk_dashes) > 0) {
		foreach ($sdk_dashes as $dname => $data) {
			$graph_array[$dname] = getTranslatedString($dname, 'Dashboard');
		}
	}
	//crmv@sdk-28873e

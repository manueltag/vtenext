...
if(!is_admin($current_user)) {
	...
	if(vtlib_isModuleActive('Touch')) {
		...
		$menu_array['LayoutEditorMobile']['image_src'] = vtiger_imageurl('orgshar.gif',$theme);
	...
	if (isset($picklistModules[$module])) {
		...
		if($adb->num_rows($sql_result) > 0) {
			...
			$menu_array['PickList']['image_src'] = vtiger_imageurl($adb->query_result($sql_result, 0, 'iconpath'), $theme);
	...
	if (isset($pick_obj->module_list[$module])) {
		...
		if($adb->num_rows($sql_result) > 0) {
			...
			$menu_array['Picklistmulti']['image_src'] = vtiger_imageurl($adb->query_result($sql_result, 0, 'iconpath'), $theme);
	...
	if (in_array($module,$picklistModules)) {
		...
		if($adb->num_rows($sql_result) > 0) {
			...
			$menu_array['LinkedPicklist']['image_src'] = vtiger_imageurl($adb->query_result($sql_result, 0, 'iconpath'), $theme);

...
if ($_REQUEST['calc_nav'] == 'true'){
	...
	if ($_REQUEST['get_all_ids'] == 'true'){
		...
		echo '&#&#&#';
		echo get_navigation_values($_SESSION[$currentModule.'_listquery'],$url_string,$currentModule,$_REQUEST['folderid'],false,$viewid);	//crmv@83034
	}
	die();
}
...
$list_buttons = Array();
if (empty($homepage)) $smarty = new vtigerCRM_Smarty();	//crmv@102334
...
if(isset($_REQUEST['lv_user_id'])) {
	$_SESSION['lv_user_id_'.$currentModule] = $_REQUEST['lv_user_id']; // crmv@107328
} else {
	$_REQUEST['lv_user_id'] = $_SESSION['lv_user_id_'.$currentModule]; // crmv@107328
}
...
if(!empty($order_by) && $order_by != '' && $order_by != null) {
	if($order_by == 'smownerid') {
		$list_query .= " ORDER BY {$table_prefix}_users.user_name ".$sorder; // crmv@110302
	} else {

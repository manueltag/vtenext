...
class Touch extends SDKExtendableUniqueClass { // crmv@42024
	 ...
	 */
	public $excluded_modules = array('Fax', 'Sms', 'Emails', 'ChangeLog', 'PriceBooks', 'PBXManager', 'WebMails', 'Projects', 'Newsletter', 'Targets', 'Campaigns', 'Telemarketing', 'Transitions'); // crmv@33311
	...
	public $modules_list_order = array(
		'Calendar', 'Events', 'Messages', 'MyNotes', 'Myfiles', 'Recents', 'Favourites', 'Search', 'PageBreak',
		'Areas',
	...
	protected function checkTables() {
		...
		if(!Vtiger_Utils::CheckTable($this->table_name_prop)) {
		...
		if(!Vtiger_Utils::CheckTable($this->table_name_user_prop)) {
			$schema_obj = new adoSchema($adb->database);
			$schema_obj->ExecuteSchema($schema_obj->ParseSchemaString($schema_table));
		}
		...
		if(!Vtiger_Utils::CheckTable($this->table_name_user_dev_prop)) {
			$schema_obj = new adoSchema($adb->database);
			$schema_obj->ExecuteSchema($schema_obj->ParseSchemaString($schema_table));
		}
		...
		if(!Vtiger_Utils::CheckTable($this->table_requests)) {
			$schema_obj = new adoSchema($adb->database);
			$schema_obj->ExecuteSchema($schema_obj->ParseSchemaString($schema_table));
		}
		// crmv@95788e
		...
		if(!Vtiger_Utils::CheckTable($this->table_tempids)) {
			$schema_obj = new adoSchema($adb->database);
			$schema_obj->ExecuteSchema($schema_obj->ParseSchemaString($schema_table));
		}
		// crmv@106521e
	...
	public function processLoginData($data) {
		...
		if (is_array($data) && is_array($data['s']) && $data['d']) {
			...
			$_SESSION['touch_device_id'] = $data['d'];
			// crmv@91082e
		}
	}
	...
	public function getCurrentDeviceId() {
		$devid = $_SESSION['touch_device_id'];
		if (empty($devid) && !empty($_REQUEST['deviceid'])) {
			$devid = $_SESSION['touch_device_id'] = $_REQUEST['deviceid'];
		}
		return $devid;
	}
	// crmv@91082e
	...
	public function startWSSession() {
		...
		if (!empty($_SERVER['HTTP_TOUCH_SESSION_ID'])) {
			session_id($_SERVER['HTTP_TOUCH_SESSION_ID']);
		} elseif (!empty($_REQUEST['touch_session_id'])) {
			session_id($_REQUEST['touch_session_id']);
		}
		session_start();
		header('Touch-Session-Id: '.session_id());
	...
	public function executeWS($wsname, $wsversion, &$request) {
		...
		if (!class_exists($classname)) {
			...
			if (!$wsclass->preLogin && !$this->executeWSLogin($request)) {
				...
				if ($wsname == 'Login' || $wsname == 'Logout' || empty($_SESSION['touch_clean_tempids_ts']) || $_SESSION['touch_clean_tempids_ts'] < time()-7200) {
					global $current_user;
					$this->cleanTempIds($current_user->id);
					$_SESSION['touch_clean_tempids_ts'] = time();
				}
	...
	public function outputRaw($data) {
		...
		$sessid = session_id() ?: $this->currentSessionId; // crmv@93148
	...
	protected function checkReplay(&$request, &$return_data = null) { // crmv@106521
		...
		if ($wsname == 'Login' || $wsname == 'Logout' || empty($_SESSION['touch_clean_replay_ts']) || $_SESSION['touch_clean_replay_ts'] < time()-3600) {
			$this->cleanReplayTable($current_user->id);
			$_SESSION['touch_clean_replay_ts'] = time();
		}
		...
		$res = $adb->pquery(
			"SELECT * FROM {$this->table_requests} WHERE userid = ? AND deviceid = ? AND requestid = ? AND status = ?",
			array($current_user->id, $deviceId, $requestId, 'COMPLETED')
		);
		if ($res && $adb->num_rows($res) > 0) {
			// already processed
	...
	public function field2Touch($module, $fieldname, $fieldvalue, $onlydisplay = false, &$focus = null, $fieldinfo = array()) {
		...
		if (is_array($fieldinfo)) {
			...
			switch ($type) {
				...
				case 'owner':
					if ($onlydisplay) {
						$fieldvalue = $touchUtils->getUserName($fieldvalue);
					}
					break;
				case 'file':
					// fix internal link
			...
			// crmv@67656 - calendario, eventi privati
			if ($module == 'Events' && $focus && !empty($focus->column_fields['visibility']) && !empty($focus->column_fields['smownerid'])) {
				$owner = $focus->column_fields['smownerid'];
				$visibility = $focus->column_fields['visibility'];
				$showFields = array('date_start', 'due_date', 'time_start', 'time_end', 'visibility', 'is_all_day_event', 'assigned_user_id');
				$notAccessFields = array();
				if (!is_admin($current_user) && $owner != $current_user->id && $visibility == 'Private') {
					if ($fieldname == 'subject') {
						$fieldvalue = getTranslatedString('Private Event', 'Calendar');
					} elseif (in_array($fieldname, $notAccessFields)) {
						$fieldvalue = getTranslatedString('LBL_NOT_ACCESSIBLE');
					} elseif (!in_array($fieldname, $showFields)) {
						$fieldvalue = '';
					}
				}
			}

...
class TouchModulesList extends TouchWSClass {
	...
	function process(&$request) {
		...
		uasort($unsortedMods, create_function('$m1, $m2', 'return strcmp($m1[0]["label"], $m2[0]["label"]);'));

		$unsortedMods = array_map(create_function('$e', 'return $e[0];'), $unsortedMods);
		...
		foreach ($sortedMods as &$m) {
			if (in_array($m['view'], array('PageBreak', 'Search', 'Settings', 'Favourites', 'Recents'))) {
				$m['tabid'] = $tabid++;
			}
			$m['sequence'] = $seq++;
		}

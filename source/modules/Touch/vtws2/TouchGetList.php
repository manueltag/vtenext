	...
	function process(&$request) {
		...
		$tabid = GetTabid($module);
			...
			if (!empty($order_by)) {
				...
				$_SESSION[$module.'_ORDER_BY'] = $order_by;
			...
			if ($module == 'Calendar') {
				...
				$list_query .= " AND {$table_prefix}_activity.activitytype not in ('Task', 'Emails', 'Sms', 'Fax') ";
			...
			if (!empty($order_by) && !empty($sorder)) {
				...
				} elseif (count($unionConditions) > 0) {
			...
			if ($result && $adb->num_rows($result) > 0)	{
				while ($row = $adb->fetchByAssoc($result)) {
					...
					if ($module == 'Events') {
						// crmv@92772
						if ($row['visibility'] == 'Private' && $row['smownerid'] != $current_user->id) {
						...
						foreach ($extraFields as $fname) {
							...
							if (is_null($rawval) || $rawval === '') {
								...
								if (is_null($procval) || $procval === '')
									$procval = $emptyValue;
								elseif (!$extraFieldsRaw && strlen($procval) > 60)
									$procval = substr($procval, 0, 57).'...';

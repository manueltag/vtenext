...
class TouchGetRecord extends TouchWSClass {
	...
	function process(&$request) {
		...
		if ($recordid > 0) {
			...
			if ($module == 'ModNotifications') {
				...
				// retrieve a record
				$record = $this->retrieveRecord($module, $recordid, $prodblock);
	...
	function retrieveRecord($module, $recordid, $prodblock = 0) {
		...
		$ret = $focus->retrieve_entity_info_no_html($recordid, $permModule, false);
		$focus->id = $recordid;
	...
	function addInfoToRecord($record, $options = array()) {
		...
		if ($module == 'Messages') {
			...
			*/
			...
			$dynaform = $processDynaFormObj->getCurrentDynaForm($runningProcess, $processmaker, $elementid);
			if (!empty($dynaform)) {
	...
	protected function formatDynaFormField($field) {
		...
		if (!empty($field)) {
			$type = $touchUtils->getFieldTypebyUIType($field['uitype']);
	
			if ($field['uitype'] == 7 || $field['uitype'] == 9) {
				$type = isset($field['decimals']) ? 'double' : 'integer';
			} else if ($field['uitype'] == 5) {
				$type = 'date';
			} else if ($field['uitype'] == 52) {
				$type = 'reference';
			}
	
			$formattedField['type']['name'] = $type;
			...
			switch ($name) {
				case 'reference':
					$relatedmods = $field['relatedmods'] ?: array();
					if ($field['uitype'] == 52) {
						$formattedField['type']['refersTo'] = array('Users');
					} else {
						$formattedField['type']['refersTo'] = explode(',', $relatedmods);
					}
					break;
				case 'multipicklist':
				case 'picklist':
					...
					foreach ($tmp as $value) {
						$picklistvalues[] = array('label' => $value, 'value' => $value);
					}
					$formattedField['type']['picklistValues'] = $picklistvalues;
					$formattedField['type']['defaultValue'] = $newField['type']['picklistValues'][0]['value'];
					break;
				case 'currency':
					$currency_id = $current_user->currency_id;
					$curSymCrate = getCurrencySymbolandCRate($currency_id);
					$formattedField['type']['symbol'] = $curSymCrate['symbol'];
					break;
			}
		}

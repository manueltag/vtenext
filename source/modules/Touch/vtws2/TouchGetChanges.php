...
class TouchGetChanges extends TouchWSClass {
	...
	public function process(&$request) {
		...
		$sinceDate = ($since > 0 ? date('Y-m-d H:i:s', $since) : $this->zeroDate);
		...
		$changes = $this->getChangedRecords($modules, $sinceDate, !$skip_content, !$skip_filter, !$skip_folder, !$skip_related);
	...
	function getChangedRecords($modules = null, $sinceDate = null, $content = false, $filters = false, $folders = false, $related = false) {
		...
		$res = $adb->query("SELECT MAX(modifiedtime) AS maxmod FROM {$table_prefix}_crmentity");
		if ($res && $adb->num_rows($res) > 0) {
			$maxmod = $adb->query_result_no_html($res, 0, 'maxmod');
			if ($maxmod && $maxmod < $sinceDate) {
		...
		$eventsBadType = array('Task', 'Emails', 'Sms', 'Fax', 'Webmails');
		...
		foreach ($modulesList as $elementType) {
			...
			if ($elementType == 'Messages') {
				...
				if ($current_account != 'all')
					$specialFolders = $moduleInstance->getSpecialFolders();
			...
			if ($elementType == 'Calendar') {
				...
				$queryGenerator->appendToWhereClause(" AND {$table_prefix}_crmentity.smownerid = ".$current_user->id);
			}
			...
			$query = preg_replace('/^select /i', 'select CASE WHEN ('.$deletedQueryCondition.") THEN 0 ELSE 1 END AS is_deleted, {$table_prefix}_crmentity.modifiedtime, ", $query);
			...
			if ($this->rawRequest['latest']) {
				...
				$query .= " ORDER BY {$table_prefix}_crmentity.modifiedtime ASC, {$table_prefix}_crmentity.crmid ASC";
			}
		...
		return array('changes'=>$changes, 'total'=>$total, 'last_timestamp' => time());
	}
	...
	protected function getAllRelated($module, $crmid) {
		global $touchInst, $touchUtils;
		global $adb, $table_prefix;

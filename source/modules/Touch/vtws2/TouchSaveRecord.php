...
class TouchSaveRecord extends TouchWSClass {
	...
	protected function getRecordTs($module, $crmid) {
		...
		if ($this->useChangelog && isModuleInstalled('ChangeLog') && vtlib_isModuleActive('ChangeLog')) {
			...
			if ($clfocus && $clfocus->isEnabled($module)) {
				$changes = array();
				$res = $adb->limitPquery(
					"SELECT modifiedtime, description 
					FROM {$clfocus->table_name} clog 
					INNER JOIN {$table_prefix}_crmentity c ON clog.{$clfocus->table_index} = c.crmid
					WHERE c.deleted = 0 AND clog.parent_id = ?
				...
				if ($res && $adb->num_rows($res) > 0) {
					while ($row = $adb->FetchByAssoc($res, -1, false)) {
						...
						$change['timestamp'] = strtotime($row['modifiedtime']);
						$changes[] = $change;
					}
				}
	...
	public function preSaveRecord($module, $crmid, $mode, &$values) {
		...
		if ($module == 'Events') {
			...
			if (isset($values['signature'])) {
				...
				$file = $path . 'signature_' . md5($recordid) . '.png';
	...
	public function updateRecord($module, $crmid, $newRecord, &$returndata = null) {
		...
		$uploads = $this->checkUploadedFile($newRecord, $record, $wsclass);
	...
	public function createRecord($module, $tempCrmid, $newRecord, &$returndata = null) {
		...
		$wsclass = $touchInst->getWSClassInstance('UploadFile', $this->requestedVersion);
		$uploads = $this->checkUploadedFile($newRecord, $updateRecord, $wsclass);
	...
	protected function checkUploadedFile($inputValues, &$outputValues, $wsclass) {

		$uploads = array_filter(array_unique(explode(',', $inputValues['upload_ids'])), create_function('$v', 'return $v !== "" && $v >= 0;'));

...
class TouchGetBlocks extends TouchWSClass {
	...
	protected function getBlocks($module) {
		...
		if ($module == 'ModNotifications') {
			$keepNotificationFields = array('notification_no', 'related_to', 'mod_not_type', 'assigned_user_id', 'seen', 'createdtime', 'modifiedtime', 'description');
			// remove extraneous fields
			foreach ($response['fields'] as $k=>$nf) {
				if (!in_array($nf['name'], $keepNotificationFields)) unset($response['fields'][$k]);
			}
		...
		// ordino i blocchi
		usort($blocks, create_function('$v1,$v2', 'return ($v1["sequence"] > $v2["sequence"] ? +1 : ($v1["sequence"] < $v2["sequence"] ? -1 : 0));'));

...
class TouchSendMail extends TouchWSClass {
	...
	function process(&$request) {
		...
		if (is_readable('modules/Emails/mailsend.php') && vtlib_isModuleActive("Emails")) {
			...
			$attach = array_filter(array_unique(explode(',', $values['attachments_ids'])), create_function('$v', 'return $v !== "" && $v >= 0;'));
			...
			$uploads = array_filter(array_unique(explode(',', $values['upload_ids'])), create_function('$v', 'return $v !== "" && $v >= 0;'));
			if (count($uploads) > 0) {

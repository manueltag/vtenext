...
class Invoice extends CRMEntity {
	...
	*/
	...
	function generateReportsSecQuery($module,$secmodule,$reporttype,$useProductJoin = true){
		...
		$vtiger_inventoryproductrelInvoice = substr($table_prefix.'_inventoryproductrelInvoice',0,29);

		if ($reporttype != 'COLUMNSTOTOTAL' && $useProductJoin) {
			$productjoins = " left join {$table_prefix}_inventoryproductrel $vtiger_inventoryproductrelInvoice on {$table_prefix}_invoice.invoiceid = $vtiger_inventoryproductrelInvoice.id

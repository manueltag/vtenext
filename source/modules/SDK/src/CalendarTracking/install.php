...
if(!Vtiger_Utils::CheckTable($table_prefix.'_cal_tracker')) {
...
if(!Vtiger_Utils::CheckTable($table_prefix.'_cal_tracker_log')) {
	$schema = '<?xml version="1.0"?>
		<schema version="0.3">
...
$sdkkInstance = Vtiger_Module::getInstance('SDK');
Vtiger_Link::addLink($sdkkInstance->id, 'HEADERSCRIPT', 'CalendarTrackingScript', 'modules/SDK/src/CalendarTracking/CalendarTracking.js');

SDK::setMenuButton('fixed','LBL_TRACK_MANAGER',"openPopup('index.php?module=SDK&action=SDKAjax&file=src/CalendarTracking/TrackerManager','','','auto','60','70','','nospinner');",'timer');

$res = $adb->pquery("SELECT activitytypeid FROM {$table_prefix}_activitytype WHERE activitytype = ?", array("Tracked"));
if ($res && $adb->num_rows($res) == 0) {
	$fieldInstance = Vtiger_Field::getInstance('activitytype',Vtiger_Module::getInstance('Calendar'));
...
$trans = array(
	'APP_STRINGS' => array(
		'it_it' => array(
			'LBL_TRACKER_MODULE' => 'Tracciamento Attività',
			...
			'LBL_TRACKING_MSG_START' => 'Gentile Cliente, la segnalazione è stata presa in carico dal reparto competente. La terremo aggiornata sullo stato di avanzamento.',
			'LBL_TRACKING_MSG_PAUSE' => 'Gentile Cliente, la risoluzione della sua segnalazione richiede più tempo del previsto, proseguiremo con l\'indagine e la terremo aggiornata sullo stato di avanzamento.',
			...
			'LBL_TRACKING_ALREADY_RUNNING' => 'Già in esecuzione per',
		...
		'de_de' => array(
			...
			'LBL_TRACK_START_FOR' => 'Start Verfolgung dafür ',
			'LBL_TRACK_PAUSE_FOR' => 'Pause Verfolgung dafür ',
			'LBL_TRACK_STOP_FOR' => 'Stopp Verfolgung dafür ',
			'LBL_START' => 'Start',
			'LBL_PAUSE' => 'Pause',
			...
			'LBL_TRACKING_MSG_START' => 'Sehr geehrter Kunde, das Problem wurde betreut von der betroffenen Abteilung übernommen. Wir werden Sie über den Fortschritt zu informieren.',
			'LBL_TRACKING_MSG_PAUSE' => 'Sehr geehrter Kunde, das Problem erfordert mehr Zeit als erwartet, werden wir auch weiterhin und wir werden Sie über den Fortschritt zu informieren.',
			'LBL_TRACKING_MSG_STOP' => 'Sehr geehrter Kunde, beendeten wir die Auflösung des Ausgabe, stellen Sie bitte sicher, dass das Problem ist nun behoben.',
			'LBL_TRACKING_ALREADY_RUNNING' => 'Bereits läuft für',
			'LBL_TRACKING_ALREADY_RUNNING_BY_USER' => 'Zur Zeit auch zurück',
			'Tracking' => 'Verfolgung',
		),
		'nl_nl' => array(
			'LBL_TRACKER_MODULE' => 'Activity Tracker',
		...
		'pt_br' => array(
			...
			'LBL_DO_TRACK' => 'Traço',
			'LBL_DO_TRACK_AND_TICKET' => 'Traço e Ticket',
			'LBL_DO_TRACK_AND' => 'Traço e ',
			'LBL_TRACK_AND_COMMENT' => 'Traço e comenta',
			...
			'LBL_TRACKING_MSG_START' => 'Prezado Cliente, o problema foi cuidado pelo departamento envolvido. Vamos informá-lo sobre o andamento.',
			'LBL_TRACKING_MSG_PAUSE' => 'Prezado Cliente, a questão requer mais tempo do que o esperado, vamos continuar e vamos informá-lo sobre o andamento.',
			'LBL_TRACKING_MSG_STOP' => 'Prezado Cliente, nós terminamos a resolução do seu problema, por favor, verifique se o problema já está resolvido.',
			'LBL_TRACKING_ALREADY_RUNNING' => 'Já em execução para',
			'LBL_TRACKING_ALREADY_RUNNING_BY_USER' => 'Atualmente também traçada por',

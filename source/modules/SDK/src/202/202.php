...
switch($sdk_mode) {
	case 'insert':
		...
		if (!empty($value)) {
			if ($current_user->id == $this->id && array_key_exists($value, $languages)) {	//crmv@30166
				$default_language = $value;
				if (array_key_exists('authenticated_user_language', $_SESSION)) $_SESSION['authenticated_user_language'] = $value;
				unset($_SESSION['sdk_js_lang']);
			}
			//crmv@35153
			if (is_admin($this)) {
				$priv_file = $root_directory.'config.inc.php';
				$userfile = file_get_contents($priv_file);
				$userfile = preg_replace("/default_language\s*=\s*['\"][^'\"]+['\"]\s*;/", "default_language = '$value';", $userfile);

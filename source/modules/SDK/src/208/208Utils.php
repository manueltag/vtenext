...
class EncryptedUitype {
	...
	protected function initTables() {
		if(!Vtiger_Utils::CheckTable($this->config_table)) {
	...
	public function registerUitype() {
		...
		Vtiger_Link::addLink($moduleInstance->id,'HEADERSCRIPT','SDKUitype',$basedir."{$this->uitype}Utils.js");
	}
	...
	public function getCachedPassword($fieldid) {
		...
		//check expiration
		$expired = false;
		if ($fconfig['pwd_timeout'] > 0 && $_SESSION['uitype208'][$fieldid]['last_insert_time'] > 0) {
			$now = time();
			$delta = $now - $_SESSION['uitype208'][$fieldid]['last_insert_time'];
			if ($delta > $fconfig['pwd_timeout']) $expired = true;
		}

		// retrieve
		$pwd = $_SESSION['uitype208'][$fieldid]['password'];

		// validate
		if ($expired || empty($pwd) || !$this->validatePassword($pwd, $fconfig['pwd_hash'])) {
			$pwd = false;
			$_SESSION['uitype208'][$fieldid] = array(
				'expired' =>  true,
				'password' => false,
			...
			$_SESSION['uitype208'][$fieldid]['expired'] = false;
			$_SESSION['uitype208'][$fieldid]['old_uitype'] = $fconfig['old_uitype'];
		}
	...
	public function setCachedPassword($fieldid, $password, $permitted = true) {
		...
		// set password
		$_SESSION['uitype208'][$fieldid]['password'] = $password;

		// set insert time
		$_SESSION['uitype208'][$fieldid]['last_insert_time'] = time();

		// permitted
		$_SESSION['uitype208'][$fieldid]['permitted'] = $permitted;
	}

	public function clearCachedPassword($fieldid = null) {
		if ($fieldid > 0) {
			unset($_SESSION['uitype208'][$fieldid]);
		} else {
			unset($_SESSION['uitype208']);
		}
	}
	...
	public function getConvertibleFields($modules = array()) {
		...
		$skipModules = array('Projects', 'ChangeLog', 'ModComments', 'ModNotifications', 'Charts', 'Fax', 'Sms', 'Newsletter', 'Emails', 'Calendar');

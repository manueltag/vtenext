...
class Notifications {
	...
	function getUserNotificationNo() {
		...
		if ($this->type == 'ModComments') {
			...
			$result = $adb->pquery('select id from vte_notifications where userid = ? and type = ?',array($this->user,$this->type));
			if ($result) {

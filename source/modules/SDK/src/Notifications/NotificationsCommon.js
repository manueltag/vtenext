...
if (typeof(NotificationsCommon) == 'undefined') {
	var NotificationsCommon = {
		...
		unseen : {'ModComments':0,'ModNotifications':0,'Todos':0},
		diff_default : 2,
		return_top : 0,
		return_left : 20,
		return_top_min : 2,
		...
		showChangesAndStorage : function(notificationDiv,notificationImg,plugin) {
		...
		//crmv@30850
		removeChange : function(plugin,id) {
			if (plugin == 'ModNotifications') {
				var res = getFile('index.php?module=SDK&action=SDKAjax&file=src/Notifications/DeleteChanges&plugin='+plugin+'&id='+id);
				var tmp = res.split('|##|');
				NotificationsCommon.drawChangesAndStorage(plugin+'CheckChangesDiv',plugin+'CheckChangesImg',tmp[1],plugin);
			}
		}

 ...
 */
function linkedListGetChanges($name, $sel, $mod) { // crmv@30528
	global $adb;
	global $current_user;
	...
	if($res && $adb->num_rows($res) > 0) {
		while ($row=$adb->fetch_array_no_html($res)) {
			...
			if (crmv_checkmandatory($row['pickdest'])){
			...
			if (is_array($values)) {
				...
				foreach ($pick_values as $k=>$v){
					if (in_array($v,$values)) {
						$real_values[] = $v;
						$values_trans[] = getTranslatedString($v);
					}
				}
				//crmv@83345e
			}
			...
			//crmv@67430e
			$ret[] = array($row[0], $real_values, $values_trans);
			//crmv@83345e
		}
		...
		foreach ($children as $c) {
			...
			if (crmv_checkmandatory($c)){
				array_unshift($values,'');
			}
			$values_trans = array();
			// translations
			if (is_array($values)) {
				foreach ($values as $v) $values_trans[] = getTranslatedString($v);
			}
			//crmv@67430
			if (crmv_checkmandatory($c)){
			...
			//crmv@67430e
			$ret[] = array($c, $values, $values_trans);
		}
	}
	//crmv@36522 e
	return $ret;
...
//crmv@36522
function crmv_checkmandatory($c){

...
function get_lang_strings($module,$language){
	...
	if (isset($_SESSION['vte_languages'][$module][$language])) return $_SESSION['vte_languages'][$module][$language];
	//crmv@27624e
	$strings = array();	//crmv@25117
	$sql = "select label,trans_label from sdk_language where module = ? and language = ?";
	$params = Array($module,$language);
	$res = $adb->pquery($sql,$params);
	if ($res && $adb->num_rows($res)>0) {	//crmv@25117
		//crmv@24862
		while ($row = $adb->fetchByAssoc($res,-1,false)){
			$len = strlen($row['trans_label'])-1;
			//if (($row['trans_label'][0] == '[' || $row['trans_label'][0] == '{') && ($row['trans_label'][$len] == ']' || $row['trans_label'][$len] == '}'))
			if ($row['trans_label'][0] == '{' && $row['trans_label'][$len] == '}')
				$strings[$row['label']] = Zend_Json::decode(html_entity_decode($row['trans_label']));
			elseif($module == 'APP_STRINGS')
				$strings[$row['label']] = replace_version_strings(html_entity_decode($row['trans_label']));
			else
				$strings[$row['label']] = html_entity_decode($row['trans_label']);
		}
		//crmv@24862e
	}
	$_SESSION['vte_languages'][$module][$language] = $cache_lang_strings[$module.$language] = $strings;	//crmv@27624
	*/
	//crmv@27624	crmv@47905bis
	static $cache_lang_strings = array();
	if (array_key_exists($module.$language, $cache_lang_strings)) return $cache_lang_strings[$module.$language];
	//if (isset($_SESSION['vte_languages'][$module][$language])) return $_SESSION['vte_languages'][$module][$language];
	//$_SESSION['vte_languages'][$module][$language] = $cache_lang_strings[$module.$language] = $strings;
	$strings = SDK::getCachedLanguage($module,$language);
	if ($strings === false && (isModuleInstalled($module) || in_array($module,array('APP_CURRENCY_STRINGS','APP_LIST_STRINGS','APP_STRINGS')))) {
		SDK::cacheLanguage($language);
...
function insert_language($module,$language,$mod_strings){
	...
	$sumlength = create_function('$sum, $item', 'return $sum + strlen($item);');

...
class PDFContent extends SDKExtendableClass { // crmv@42024
	...
	//crmv@25443e
      ...
      while($row = $this->db->fetchByAssoc($result))
      {
		...
		//crmv@97136
		elseif ($row["uitype"] == "77" || $row["uitype"] == "52")
	...
	//crmv@65492e - 28
      ...
      {
		if ($fieldname == "assigned_user_id" || $fieldname == "assigned_user_id1" || in_array($fieldname,$UserField))	//crmv@60349 crmv@97136
          $value = $this->getOwnerNameCustom($value);
        elseif($fieldname == "account_id") {
          //$account_no = $this->getAccountNo($value);
          //$this->replacements["$".$related.strtoupper($emodule."_ACCOUNT_NO")."$"]=$account_no;
          //$this->replaceContent();
    ...
    protected function replaceUserCompanyFields() {
	  ...
	  if ($result && $adb->num_rows($result) > 0) {
		  ...
		  foreach ($Org_Details AS $coll => $value) {
			if ($coll == "logo")
			  $value = "<img src=\"".$$root_dir."test/logo/".$Org_Details["logoname"]."\">";
	...
	//crmv@19166
      $query="select case when ".$table_prefix."_products.productid != '' then ".$table_prefix."_products.productname else ".$table_prefix."_service.servicename end as productname," .
            ...
            " case when ".$table_prefix."_products.productid != '' then c1.description else c2.description end as psdescription, ".$table_prefix."_inventoryproductrel.* " .
                              ...
                              if (count($RelBlock_Data) > 0)
                              {
                                  foreach ($RelBlock_Data AS $RelBlock_i => $RelBlock_Details)
                                  {
                                      foreach($ProductParts as $productpartid=>$productparttext)
                                      {
                                          ...
                                          {
												//crmv@25443
												if (in_array($coll,(array)$this->reference_fields[$secmodule]) && $value != '' && $value != '-') { // crmv@106527
												...
												//TODO: multipicklist_fields
												} elseif (in_array($coll,(array)$this->picklist_fields[$secmodule]) && $value != '' && $value != '-') { //crmv@97759 - array casting
													$value = getTranslatedString($value,$secmodule);
												}
												//crmv@97834
												elseif($coll == 'description'){
													$value = nl2br($value);
												}
												//crmv@97834e
												//crmv@25443e
                                              if (trim($value) != "-") $show_line = true;
                                              $productparttext = str_replace("$".$coll."$",$value,$productparttext);
                                          }

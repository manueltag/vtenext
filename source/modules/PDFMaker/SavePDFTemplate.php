...
$sql4 = "INSERT INTO ".$table_prefix."_pdfmaker_settings (templateid, margin_top, margin_bottom, margin_left, margin_right, format, orientation, decimals, decimal_point, thousands_separator, header, footer, encoding, file_name) 
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$params4 = array($templateid, $margin_top, $margin_bottom, $margin_left, $margin_right, $pdf_format, $pdf_orientation, $dec_decimals, $dec_point, $dec_thousands, $header, $footer, $encoding, $nameOfFile);
...
if ($_REQUEST["ignore_picklist_values"] != '') {
	...
	foreach($pvvalues as $value) {
		$adb->query("INSERT INTO ".$table_prefix."_pdfmaker_ignpickvals(value) VALUES('".trim($value)."')");
	}
}
//crmv@19166e
...
if ($num_rows7 == 0)
{
    ...
    $adb->pquery($sql8, array($linkid, $tabid, "LISTVIEWBASIC", "PDF Export", "getPDFListViewPopup2(this,'$"."MODULE$');",1));
}

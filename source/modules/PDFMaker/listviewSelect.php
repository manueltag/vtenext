...
if($adb->num_rows($temp_result)>0)
{
  ...
  $generate_pdf='
      <tr>
          ...
          <input type="button" class="crmbutton small save" value="'.$app_strings["LBL_EXPORT_TO_PDF"].'" onclick="if(getSelectedTemplates()==\'\') alert(\''.$pdf_strings["SELECT_TEMPLATE"].'\'); else document.location.href=\'index.php?module=PDFMaker&relmodule='.$_REQUEST["return_module"].'&action=CreatePDFFromTemplate&idslist=true&commontemplateid=\'+getSelectedTemplates()+\'&language=\'+document.getElementById(\'template_language\').value; hideFloatingDiv(\'PDFListViewDivCont\');" />            

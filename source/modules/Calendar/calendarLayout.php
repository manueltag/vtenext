...
function calendar_layout(& $param_arr,$viewBox='',$subtab='',$view_filter='',$activity_view='') //crmv@7633s // crmv@vte10usersFix
{
    global $mod_strings,$cal_log,$current_user;//crmv@vte10usersFix
	...
	echo "<table class='small calHdr' align='center' border='0' cellpadding='5' cellspacing='0' width='100%' style='display:none;'>";


    echo "<div style='display: none;' id='mnuTab'>";
    echo "<form name='EventViewOption' method='POST' action='index.php' style='display: none;'>";
    echo getEventViewOption($param_arr,$view_filter);
    echo "</form>";
	get_cal_header_data($param_arr,$viewBox,$subtab,$view_filter);//crmv@7633s
    //crmv@17001
	...
	// pass the ical info
	if (is_array($param_arr['icals']) && count($param_arr['icals']) > 0) {
	...
	if ($_REQUEST['related_add'])
		echo '<iframe id="wdCalendar" name="wdCalendar" src="about:blank" width="100%" frameborder="0" scrolling="auto"></iframe>'; //crmv@26807
	else
		echo '<iframe id="wdCalendar" name="wdCalendar" src="'.$calendarUrl.'" width="100%" frameborder="0" scrolling="auto"></iframe>'; //crmv@26807
    ...
    if($_REQUEST['related_add']){
	    echo "
	    	wdCalendar_h = wdCalendar_h - 10; 
			setTimeout(function() {
				jQuery('#vte_menu_white_1').height(jQuery('#Buttons_List_4').height());
				getObj('wdCalendar').height = jQuery(document).height() - jQuery('#vte_menu_white').height() - jQuery('#vte_footer').height() - wdCalendar_h - jQuery('#vte_menu_white_1').height() - jQuery('#Buttons_List_4').height();
				window.frames['wdCalendar'].document.location.href = '$calendarUrl';
			}, 500);
		";
	}else{
	    echo "
	    	// old code
			getObj('wdCalendar').height = document.documentElement.clientHeight - jQuery('#vte_menu_white').height() - jQuery('#vte_footer').height() - wdCalendar_h;
	    ";
	}
	echo " 
		jQuery('#fancybox-loading').hide();
	});
	</script>";	//crmv@18592 //crmv@20210
	...
	//crmv@62447e
	$hourview_layout .= '</div></div>';
//	$hourview_layout .= '</td></tr></table></td></tr>';
	$hourview_layout .= '		</table>
	        </td></tr></table>
	        </td></tr></table>
	        </td></tr></table>
	        </div>
...
function get_cal_header_data(& $cal_arr,$viewBox,$subtab,$view_filter='') //crmv@7633s
{
	...
	if(isPermitted("Calendar","EditView") == "yes")
	{
		...
		//crmv@vte10usersFix
		//crmv@23696 crmv@128159
        $headerdata .="<tr>
                <td id='addButton'>
					<div class=\"smallerCircleGreen iconCircle\">
						<i class=\"vteicon md-link\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".getTranslatedString('LBL_CREATE_BUTTON_LABEL')."\"
							onclick='fnAddEvent(this,\"addEventDropDown\",\"".$temp_date."\",\"".$temp_date."\",\"".$time_arr['starthour']."\",\"".$time_arr['startmin']."\",\"".$time_arr['startfmt']."\",\"".$time_arr['endhour']."\",\"".$time_arr['endmin']."\",\"".$time_arr['endfmt']."\",\"".$viewBox."\",\"".$subtab."\",\"".$eventlist."\",\"".$view_filter."\",-20,\"".$date_format."\");'
							onmouseout='fnRemoveEvent();'>add</i>
					</div>
                </td>";
        
        //crmv@23696e crmv@128159e
		//crmv@20480e	//crmv@22622e //crmv@vte10usersFix e
	}
	else
	{
		$headerdata .="<tr><td>&nbsp;</td>";
...
function crmvGetUserAssignedToHTML($assignedto,$toggletype,$only_contents=false,$view_filter='selected')
{
	...
	if (!$only_contents)
		$htmlStr .= '<div id="filterDivCalendar" style="border:none; overflow-x: hidden; overflow-y: auto; position: relative; padding-top:4px;">'; // crmv@43117
	if($toggletype == 'task')
	...
	foreach($assignedto as $key_one=>$name)
	{
		...
		if(in_array($key_one,array('all','mine','others','none'))) {//crmv@26030m
			...
			if ($key_one == 'mine') {
				...
				$otherUsers = '<tr><td><div class="checkbox"><label><input id="singleUserMine" class="checkbox" type="checkbox" name="singleUser" value="mine" '.$checked.' '.$checkFunction.'></label></div></td><td style="background-color:'.$bgColor.'; cursor: pointer; width:100%;"><label for="singleUserMine">'.$app_strings['LBL_ASSIGNED_TO_ME'].'</label></td></tr>';
			}
			elseif ($key_one == 'all')
			...
			$otherUsers = '<tr><td><div class="checkbox"><label><input id="singleUser'.$key_one.'" type="checkbox" name="singleUser" value="'.$key_one.'" '.$checked.' '.$checkFunction.'></label></div></td><td nowrap style="background-color:'.$bgColor.'; cursor: pointer;"><label for="singleUser'.$key_one.'">'.$name['name'].'</label></td></tr>'; //crmv@20874
		}

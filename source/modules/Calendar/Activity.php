...
class Activity extends CRMEntity {
	...
	function insertIntoActivityReminderPopup($cbmodule) {
		...
		if(isset($cbmodule) && isset($cbrecord)) {
			...
			unset($_SESSION['next_reminder_time']); // crmv@103354
		}
	}
...
//Function Call for Related List -- Start
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		//crmv@103400 - added join _contactscf
...
//calendarsync
    ...
    function get_calendars($user_name,$from_index,$offset)
    {
		...
		$query = "select ".$table_prefix."_activity.location as location,".$table_prefix."_activity.duration_hours as duehours, ".$table_prefix."_activity.duration_minutes as dueminutes,".$table_prefix."_activity.time_start as time_start, ".$table_prefix."_activity.subject as name,".$table_prefix."_crmentity.modifiedtime as date_modified, ".$table_prefix."_activity.date_start start_date,".$table_prefix."_activity.activityid as id,".$table_prefix."_activity.status as status, ".$table_prefix."_crmentity.description as description, ".$table_prefix."_activity.priority as ".$table_prefix."_priority, ".$table_prefix."_activity.due_date as date_due ,".$table_prefix."_contactdetails.firstname cfn, ".$table_prefix."_contactdetails.lastname cln from ".$table_prefix."_activity inner join ".$table_prefix."_salesmanactivityrel on ".$table_prefix."_salesmanactivityrel.activityid=".$table_prefix."_activity.activityid inner join ".$table_prefix."_users on ".$table_prefix."_users.id=".$table_prefix."_salesmanactivityrel.smid left
join ".$table_prefix."_cntactivityrel on ".$table_prefix."_cntactivityrel.activityid=".$table_prefix."_activity.activityid left join ".$table_prefix."_contactdetails on ".$table_prefix."_contactdetails.contactid=".$table_prefix."_cntactivityrel.contactid inner join ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid=".$table_prefix."_activity.activityid where user_name='" .$user_name ."' and ".$table_prefix."_crmentity.deleted=0 and ".$table_prefix."_activity.activitytype='Meeting' limit " .$from_index ."," .$offset;
...
//calendarsync
	...
	global $log,$table_prefix;
	 ...
	 $query = "select ".$table_prefix."_activity.subject as name,".$table_prefix."_crmentity.modifiedtime as date_modified, ".$table_prefix."_activity.date_start start_date,".$table_prefix."_activity.activityid as id,".$table_prefix."_activity.status as status, ".$table_prefix."_crmentity.description as description, ".$table_prefix."_activity.priority as priority, ".$table_prefix."_activity.due_date as date_due ,".$table_prefix."_contactdetails.firstname cfn, ".$table_prefix."_contactdetails.lastname cln from ".$table_prefix."_activity inner join ".$table_prefix."_salesmanactivityrel on ".$table_prefix."_salesmanactivityrel.activityid=".$table_prefix."_activity.activityid inner join ".$table_prefix."_users on ".$table_prefix."_users.id=".$table_prefix."_salesmanactivityrel.smid left join ".$table_prefix."_cntactivityrel on ".$table_prefix."_cntactivityrel.activityid=".$table_prefix."_activity.activityid left join ".$table_prefix."_contactdetails on ".$table_prefix."_contactdetails.contactid=".$table_prefix."_
cntactivityrel.contactid inner join ".$table_prefix."_crmentity on ".$table_prefix."_crmentity.crmid=".$table_prefix."_activity.activityid where user_name='" .$user_name ."' and ".$table_prefix."_crmentity.deleted=0 and ".$table_prefix."_activity.activitytype='Task' limit " .$from_index ."," .$offset;
...
function get_calendarsforol($user_name)
{
  ...
  for($i=0;$i < $adb->num_rows($result1);$i++)
  {
	...
	function getActivityMailInfo($return_id,$status,$activity_type)
	{
		...
		$desc_qry = "select description from ".$table_prefix."_crmentity where crmid=?";
	...
	function generateInvitationIcs($activityid, $fields = null) {
		...
		if (empty($fields)) {
			...
			$query = 
				"SELECT 
					a.*, 
					c.smownerid, c.createdtime, c.modifiedtime, c.description,
					ar.reminder_time
	...
	function getShownUserId($userid,$all=false){
		...
		for($j=0;$j<$rows;$j++)
		{
			$id = $adb->query_result($result,$j,'shownid');
			$selected = $adb->query_result($result,$j,'selected');
			$shownname = getUserName($id,$showfullusername);
			$shownid[$id] = array('name'=>$shownname,'selected'=>$selected);
		}
		return $shownid;
	}

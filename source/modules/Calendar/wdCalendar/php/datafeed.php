...
function listCalendarByRange($sd, $ed, $view_filter, $noCompletedTask){ //crmv@vte10usersFix
	...
	$ret['events'] = array();
  	...
  	try{
		...
		//Events - i
		$sql = Appointment::readAppointment($current_user->id,$start_date,$end_date,'',$view_filter,true);
	    $res = $adb->query($sql);
		...
		if ($adb->isOracle()) {
			$sql = str_replace($table_prefix.'_crmentity.description', "CAST({$table_prefix}_crmentity.description AS VARCHAR2(2000)) as description", $sql);
		}
	    $res = $adb->query($sql);
	    $rows_task = array();
	    unset($row_tmp);
	    ...
	    if (in_array('Contacts',array_keys($related_array)) && count($activityid_array) > 0) {
			...
			global $onlyquery;
			$onlyquery = true;
			$actClass = CRMEntity::getInstance('Activity');
			...
			$query = $_SESSION['contacts_listquery'];
	    ...
	    foreach ($rows_event as $row) { //crmv@26030m
			...
			//crmv@24883e
			if (!is_admin($current_user) && $row['smownerid'] != $current_user->id && $row['visibility'] == 'Private') {

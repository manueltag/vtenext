...
if((isset($_REQUEST['change_status']) && $_REQUEST['change_status']) && ($_REQUEST['status']!='' || $_REQUEST['eventstatus']!=''))
{
	...
	if($count != 0)
	{
		...
		$focus->sendInvitation($return_id,$invitees_ids_string,$activity_type,$mail_data['subject'],$mail_data);	//crmv@19555 //crmv@32334
	}
	//crmv@32334 e
}
else
...
if ($_REQUEST['ajaxCalendar'] != '') {
	global $returnid;
	$returnid = $return_id;
	if ($_REQUEST['ajaxCalendar'] == 'detailedAdd') exit;
	// crmv@42752

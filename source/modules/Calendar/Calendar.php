...
class Calendar 
{
        ...
        */
         ...
         */
        function add_Activities($current_user,$free_busy='',$view_filter='') //crmv@7633s
        {
                ...
                $activities = Array();
                $activities = Appointment::readAppointment($current_user->id,$start_datetime,$end_datetime,$this->view,$view_filter); //crmv@7633s
                if(!empty($activities))
                {
	...
	function crmv_extend_week_view($value) {
		list($data_,$ora_) = explode(":",$value->formatted_datetime);
		list($anno_,$mese_,$giorno_) = explode("-",$value->formatted_datetime);
		$day_diff = $value->end_time->day - $giorno_;
		if($day_diff != 0) {
			$count = 24 - $ora_;
		} else {
			$count = $value->end_time->hour - $ora_;
		...
		for($i=1;$i<$count;$i++) {
			...
			if(strlen($ora_2) < 2) 
				$ora_2 = "0".$ora_2;
			//crmv@32334
			if (!is_array($this->week_slice[$data_.":".$ora_2]->activities)){
				$this->week_slice[$data_.":".$ora_2]->activities = Array();	
			}
			//crmv@32334 e					
			array_push($this->week_slice[$data_.":".$ora_2]->activities, $value);
		}
	}
}

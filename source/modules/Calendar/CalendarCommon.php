...
function getSharedCalendarId($sharedid)
{
	...
	$query = "SELECT * from ".$table_prefix."_sharedcalendar where sharedid=?";
...
function getTimeCombo($format,$bimode,$hour='',$min='',$fmt='',$todocheck=false,$quickcreate=false) //crmv@31315
{
	...
	else
	{
		...
		$combo .= '<tr><td><label for="'.$bimode.'hr">'.$mod_strings[LBL_HR].'</label></td><td><select name="'.$bimode.'hr" id="'.$bimode.'hr" class="small detailedViewTextBox" '.$jsfn.'>';
		for($i=0;$i<=23;$i++)
		{
			$hrvalue = twoDigit($i);
			$hrsel = ($hour == $hrvalue)?'selected':'';
		...
		$combo .= '<td><label for="'.$bimode.'min">'.$mod_strings[LBL_MIN].'</label></td><td><select name="'.$bimode.'min" id="'.$bimode.'min" class="small detailedViewTextBox" '.$jsfn.'>';
		for($i=0;$i<12;$i++)
		{
			$value = $i*5;
			$value= twoDigit($value);
...
function getEmailInvitationDescription($description,$user_id,$record='',$answer='',$from='')
{
	...
	$confirm_url = $site_URL."/modules/Calendar/ConfirmInvitation.php?from=$from&app_key=$application_unique_key&record=$record&userid=".$user_id;
	$link_answer = '';

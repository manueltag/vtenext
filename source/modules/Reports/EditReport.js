...
var EditReport = {
	...
	blockFrame: function() {
		VtigerJS_DialogBox.block(undefined, 0.2);
	},
	...
	saveReport: function() {
		...
		// advfilters
		var advfilters = [];
		jQuery('#advFiltersContainer div.advFilterGroup').each(function() {
			var groupfields = [];
	...
	gotoStep: function(step) {
		...
		if (step != currStep) {
			...
			if (valid) {
				...
				jQuery(cells[currStep-1]).removeClass('reportStepCellSelected');
				jQuery(cells[step-1]).addClass('reportStepCellSelected');
				jQuery('#nextButton')[step == totalSteps ? 'hide' : 'show']();
				jQuery('#saveButton')[step == totalSteps ? 'show' : 'hide']();
	...
	alignFilterField: function(self, keepvalues) {
		...
		if (finfo.wstype == 'integer' || finfo.wstype == 'double' || finfo.wstype == 'currency') {
			tokeep = ['e','n','l','g','m','h'];
		} else if (finfo.wstype == 'date' || finfo.wstype == 'datetime') {
